		 ...
		 var drag = false,
			...
			pointerEventToXY = function( e ) {
				...
				if( e.type == 'touchstart' || e.type == 'touchmove' || e.type == 'touchend' || e.type == 'touchcancel' ) {
					...
					out.x = touch.clientX/scale;
					out.y = touch.clientY;
				}else if (e.type == 'mousedown' || e.type == 'mouseup' || e.type == 'mousemove' || e.type == 'mouseover'|| e.type=='mouseout' || e.type=='mouseenter' || e.type=='mouseleave') {
					out.x = e.clientX/scale;
					out.y = e.clientY;
				}
				return out;
			},
		...
		if( typeof(_options)=='string' ){
		 ...
		 return this.each(function(){
			...
			for(i=0;i<_this.values.length;i++){
				if( !_this.$runners[i] ){
					_this.$runners.push($runner = $('<div class="xdsoft_range2dslider_runner xdsoft_range2dslider_runner'+i+'"><div style="position:absolute;top:-29px;left:-29px;width:90px;height:90px;background:transparent;opacity:0.5;cursor:pointer;"></div></div>'));
			 ...
			 if(_this.options.showRadio){
				 ...
				 for(i=0;i<=lengthrange;i++){
					 if(i!= lengthrange){
						 ...
						 obj = $('<span class="rangee" style="left:'+parseInt(count*i-18)+'px;"></span>');
					 }
			...
			if( _this.options.showLegend && ( !_this.legends||_this.options.recalcLegends ) ){
				...
				if( _this.legends[0]&&_this.legends[0][0] ){
					_this.legends[0][0]
						.css(_this.options.y,'-10px')
						.css(_this.options.x,(offsets[0]-10)+'px')
