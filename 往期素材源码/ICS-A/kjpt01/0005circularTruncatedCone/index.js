...
var twoDimension={
    ...
    getJson:function(){
        ...
        arrayAll[2] = [polygon.topradius,0,-polygon.height];
        arrayAll[3] = [polygon.radius,0,0];
        polygon.angleJson = arrayAll;
    },
    getArcJson:function(){
        arcJson=[];
        var angle = -polygon.angle;
        var topradius = polygon.topradius;
        var radius = polygon.radius;
        arcJson[0] = [0,0,0];
        arcJson[1] = [0,polygon.height,0];
        if(topradius){
            arcJson[2] = [Math.round(topradius*Math.cos(angle)),polygon.height,Math.round(radius*Math.sin(angle))];
            arcJson[3] = [Math.round(radius*Math.cos(angle)),0,Math.round(radius*Math.sin(angle))];
        }else{
...
var threeDimensional ={
    ...
    createCylinder:function(){
        ...
        var meshG = new THREE.CylinderGeometry(polygon.topradius,polygon.radius,polygon.height,64,64);
        var meshM = new THREE.MeshBasicMaterial({color:'#F39800',opacity:0.2,transparent:true});
    ...
    createClyinderLine:function(){
        ...
        for(i=0;i<73;i++){
            ...
            vertices2.push(new THREE.Vector3(topradius*Math.cos(i*5*Math.PI/180),polygon.height,topradius*Math.sin(i*5*Math.PI/180)));
        }
        ...
        var lineMesh2 = new THREE.Line(geometryLine2, new THREE.LineBasicMaterial({color: '#F39800'}));
        sceneT.add(lineMesh1);
        sceneT.add(lineMesh2);
        threeDimensional.clyinderLines.push(lineMesh1);
        threeDimensional.clyinderLines.push(lineMesh2);
    },
    ...
    drawRectangle:function(){
        ...
        shape.moveTo(0,0,0);
        if(polygon.topradius == 0){
            shape.lineTo(0,polygon.height,0);
            shape.lineTo(polygon.radius,0,0);
        }else{
            shape.lineTo(0,polygon.height,0);
            shape.lineTo(polygon.topradius,polygon.height,0);
            shape.lineTo(polygon.radius,0,0);
        }
    ...
    dynamicMove:function(){
        ...
        if(polygon.angle > 6.28){
            ...
            dynamic = 0;
            setTimeout(function(){
                sceneT.remove(threeDimensional.circlrTop,threeDimensional.circlrBottom)
    ...
    createShape:function(angle){
        angle = -polygon.angle;
        sceneT.remove(threeDimensional.circlrTop,threeDimensional.circlrBottom);
        var g = new THREE.CircleGeometry(polygon.topradius,50,0,angle);
        var m = new THREE.MeshBasicMaterial({color:'#F39800',side:THREE.DoubleSide,opacity:0.8,transparent:false});
        threeDimensional.circlrTop = new THREE.Mesh(g,m);
        threeDimensional.circlrTop.rotation.x = Math.PI/2;
        threeDimensional.circlrTop.position.y = getParameter.height;
        sceneT.add(threeDimensional.circlrTop);
        g = new THREE.CircleGeometry(polygon.radius,50,0,angle);
        m = new THREE.MeshBasicMaterial({color:'#F39800',side:THREE.DoubleSide,opacity:0.8,transparent:false});
...
if(canWebgl){
    ...
    rendererT = new THREE.CanvasRenderer({alpha:true});
}
...
function renderAll(){
    ...
    /*if(changeFlag){
        if(changeFlag == 1){
            getParameter.topradius = getParameter.radius;
            polygon.topradius = getParameter.topradius;
            polygon.angleJson[2][0] =parseInt(getParameter.topradius);
            threeDimensional.repaintMesh();


            var value = polygon.topradius;
            var rate = value*410/150;
            $('.s1').find('.sliderLeft').css({'width':rate+'px'});
            $('.s1').find('.xdsoft_range2dslider_runner').css({'left':rate+'px'});
            $('.s1').find('.xdsoft_slider_label').text(''+value);
            $('#slider1').attr('value',''+value+'|0');

        }else{
            getParameter.radius = getParameter.topradius;
            polygon.radius = getParameter.radius;
            getParameter.radius= polygon.angleJson[3][0]=parseInt(getParameter.radius);
            threeDimensional.repaintMesh();
            changeRange(2,getParameter.radius);

            var value = polygon.radius;
            var rate = value*410/200;
            $('.s2').find('.sliderLeft').css({'width':rate+'px'});
            $('.s2').find('.xdsoft_range2dslider_runner').css({'left':rate+'px'});
            $('.s2').find('.xdsoft_slider_label').text(''+value);
            $('#slider2').attr('value',''+value+'|0');
...
$('#slider1').change(function(){
    ...
    if(value >value2){
        value = value2;
        flag = true;
        var left = parseInt($('.s1 .xdsoft_range2dslider_runner1').css('left'));
        left-=2.1*(value-value2);
        $('.s1 .xdsoft_range2dslider_runner0').css('left',left+'px');
        $(this).range2DSlider({
            template:'horizontal',
    ...
    getParameter.topradius = value;
    getParameter.radius=value2
    /*if(getParameter.topradius >= getParameter.radius){
        changeFlag = 1;
    }else{*/
        polygon.topradius = getParameter.topradius;
        polygon.angleJson[2][0] =parseInt(getParameter.topradius);
...
$('#slider3').change(function(){
    ...
    polygon.angleJson[1][2]=polygon.angleJson[2][2]=-parseInt(getParameter.height);
