...
var opened=1,selectNum=1,selectFun = 5,select1=0,select2=0,select3=0,select4=0;
...
function ThreeDimensional() {
    ...
    var axis={},funcs=[null,null,null],mousedownflag = false,changeLine = null,textA=null,textB=null;

    $('.verticalCenter').css('margin-top',($('#controlContainer').height() -119 - $('.verticalCenter').height() )/2);

    var thiz = this;
    var grid=null,sphere1=null,sphere2=null,selectobjs=[],selectobj=null,text=null;
    ...
    this.int = function () {
       ...
       // this.createGrid();
        ...
        sphere1 = this.createSphere([50,0,0],14,0x78cdf8);
        sphere1.name = 'a';
        thiz.scene.add(sphere1);
        textA = new SpriteText2D('(1,0)', textStyle);
        textA.position.x = 50+50;
        textA.position.y = 0;
        this.scene.add(textA);
        selectobjs.push(sphere1);
        sphere2 = this.createSphere([0,50,0],14,0x78cdf8);
        sphere2.name = 'b';
        thiz.scene.add(sphere2);
        textB = new SpriteText2D('(0,1)', textStyle);
        textB.position.x = 50;
        textB.position.y = 50;
        this.scene.add(textB);
        selectobjs.push(sphere2);
        sphere1.visible = sphere2.visible =textB.visible = textA.visible = false;
        ...
        if(mousedownflag){
            if ( raycaster.ray.intersectPlane( plane, intersection ) ) {
                ...
                if(selectobj.name =='a'){
                    ...
                    sphere1.position.x = x*50;
                    sphere1.position.y = y*50;

                    sphere2.position.x = y*50;
                    sphere2.position.y = x*50;

                    textA.position.x = x*50+50;
                    textA.position.y = y*50;
                    textA.text='('+x.toFixed(1)+','+y.toFixed(1)+')';

                    textB.position.x = y*50+50;
                    textB.position.y = x*50;
                    textB.text='('+y.toFixed(1)+','+x.toFixed(1)+')';

                }else{
                    if(x >0){
                        x =Math.log(y)/Math.log(valueA);
                    }else{
                        y = Math.pow(valueA,x);
                    }
                    sphere2.position.x = x*50;
                    sphere2.position.y = y*50;

                    sphere1.position.x = y*50;
                    sphere1.position.y = x*50;

                    textA.position.x = y*50 +50;
                    textA.position.y =  x*50;
                    textA.text='('+y.toFixed(1)+','+x.toFixed(1)+')';

                    textB.position.x = x*50 +50;
                    textB.position.y = y*50;
                    textB.text='('+x.toFixed(1)+','+y.toFixed(1)+')';
    ...
    function countFunction(){
        ...
        array = [];
        array.push(new THREE.Vector3(-10*50,-10*50,0));
        array.push(new THREE.Vector3(10*50,10*50,0));
        funcs[2] = createLineMesh(array,'#7ed321',2);
    ...
    function createChangeLine(){
        if(changeLine!=null){
            thiz.scene.remove(changeLine);
        }
        var vertices = [];
        vertices.push(new THREE.Vector3(sphere1.position.x,sphere1.position.y,0));
        vertices.push(new THREE.Vector3(sphere2.position.x,sphere2.position.y,0));
        changeLine = createLineMesh(vertices,'#000',2);
        thiz.scene.add(changeLine);
        changeLine.visible = false;
...
function turnRight(){
    ...
    if(!select1||!select2){
        select4 = 0;
        $('.turn4').removeClass('on').addClass('off');
        $('.turn4').find('.span2').text('' +'on');
    }
