...
function ThreeDimensional() {
    ...
    this.createLine=function(){
        ...
        var vertices1=[];
        if(B==0){
            vertices1.push(vec3((-C/A)*50,-500,0));
            vertices1.push(vec3((-C/A)*50,500,0));
        }
        else{
            vertices1.push(vec3(-5000,((-A*(-100)/B)-C/B)*50,0));
            vertices1.push(vec3(5000,((-A*(100)/B)-C/B)*50,0));
        }
    ...
    this.createAllLine=function(){
        ...
        vertices1.push(vec3(x1*50,((-A*x1/B)-C/B)*50,0));
        var geometryLine1 = new THREE.Geometry();
        geometryLine1.vertices = vertices1;
         var linemesh2 = new THREE.LineSegments(geometryLine1, new THREE.LineBasicMaterial({color: 0x000000}));
        var text1=createText("S",x1*50,((-A*x1/B)-C/B)*50+30,1,"#000000");
        line2.add(linemesh2,text1);
        this.scene.add(line2);

        var vertices2=[];
        vertices2.push(vec3(x1*50,y1*50,0));
        vertices2.push(vec3(((-B*y1/A)-C/A)*50,y1*50,0));
        var geometryLine2 = new THREE.Geometry();
        geometryLine2.vertices = vertices2;
        var linemesh3 = new THREE.LineSegments(geometryLine2, new THREE.LineBasicMaterial({color: 0x000000}));
        var text2=createText("R",((-B*y1/A)-C/A)*50+20,y1*50+10,1,"#000000");
        line3.add(linemesh3,text2);
        ...
        var b=y1-(B/A)*x1;
        var resultx=((-C/B)-b)/(B/A+A/B);
        var vertices3=[];
        vertices3.push(vec3(x1*50,y1*50,0));
        if(A==0){
            vertices3.push(vec3(x1*50,(-C/B)*50,0));
        }
        else if(B==0){
            vertices3.push(vec3((-C/A)*50,y1*50,0));
        }
        else{
            vertices3.push(vec3(resultx*50,((-A*resultx/B)-C/B)*50,0));
        }
        var geometryLine3 = new THREE.Geometry();
        geometryLine3.vertices = vertices3;
        geometryLine3.computeLineDistances();
        var dashlinemesh = new THREE.Line(geometryLine3,new THREE.LineDashedMaterial( { color: 0x000000,opacity:0.9, dashSize: 5, gapSize: 5 } ));
        var text3=createText("Q",resultx*50+20,((-A*resultx/B)-C/B)*50+20,1,"#000000");
        var text4=createText("P("+x1+","+y1+")",x1*50-15,y1*50-20,1,"#000000");
        var text5=createText("d",(resultx+x1)/2*50,(y1*50+(((-A*resultx/B)-C/B)*50))/2,1,"#000000");
        if(((-aa*(x1)/bb)-cc/bb)==y1){
            dashline.add(text4);
        }
        else{
            dashline.add(dashlinemesh,text3,text4,text5);
        }
        this.scene.add(dashline);

        if(((-aa*(x1)/bb)-cc/bb)==y1){
            this.scene.remove(line2,line3)
        }
    };
    this.showresult=function(){
        ...
        if(A==0&&B==0){
            ...
            if(A==0){
                var d=Math.abs(y1-(-C/B));
            }
            else{
                var d=(Math.abs(A*x1+B*y1+C))/Math.pow((Math.pow(A,2)+Math.pow(A,2)),1/2);
            }
            ...
            if(A==1){
                if(B==0){
                    ...
                    if(B==1){
                        ...
                        if(C==0){
                            $('#fangchen').html("x+"+B+"y=0");
                        }
                        else if(C>0){
                            $('#fangchen').html("x+"+B+"y+"+C+"=0");
                        }
                        else{
                            $('#fangchen').html("x+"+B+"y"+C+"=0");
                        }
                ...
                if(B==0){
                    ...
                    if(B==1){
                        if(C==0){
                            $('#fangchen').html(A+"x+"+"y=0");
                        }
                        else if(C>0){
                            $('#fangchen').html(A+"x+"+"y+"+C+"=0");
                        }
                        else{
                            $('#fangchen').html(A+"x+"+"y"+C+"=0");
                        }
                        ...
                        if(C==0){
                            $('#fangchen').html(A+"x+"+B+"y=0");
                        }
                        else if(C>0){
                            $('#fangchen').html(A+"x+"+B+"y+"+C+"=0");
                        }
                        else{
                            $('#fangchen').html(A+"x+"+B+"y"+C+"=0");
                        }
...
//        round:true,
        ...
        $('.slider3 .sliderLeft').css('width','206px');
    }
...
$("#slider3").change(function(){
    if(bb==0&&aa==0){
        $('.slider3 .xdsoft_range2dslider_runner').css('left','206px');
        $('.slider3 .sliderLeft').css('width','206px');
        $('#slider3').range2DSlider({
            template:'horizontal',
            value:[[0,0]],
