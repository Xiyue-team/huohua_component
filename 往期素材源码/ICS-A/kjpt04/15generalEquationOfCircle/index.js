...
var threeDimension = {
    ...
    createObj:function(){
        ...
        //圆环
        var R=(Math.pow(getParameter.D,2)+Math.pow(getParameter.E,2)-4*getParameter.F)/4
        if(R>0){
            var r=Math.sqrt(R)
            var x=-r*40*Math.cos(45/180*Math.PI)-getParameter.D/2*40;
            var y=-r*40*Math.sin(45/180*Math.PI)-getParameter.E/2*40;

            //半径
            vertices=[]
            vertices.push(threeDimension.vec3(-getParameter.D/2*40,-getParameter.E/2*40,0))
            vertices.push(threeDimension.vec3(x,y,0))
            var liner=threeDimension.createLineMesh(vertices, "#f00", 2)
            var textr=threeDimension.createText('r = '+r.toFixed(2), x-45,y+20,0,'#f00',18)
            var P=threeDimension.createText('P', x-5,y,0,'#000',18)

            //圆
            vertices=[]
            vertices.push(-getParameter.D/2*40,-getParameter.E/2*40,0)
            var line=threeDimension.createCircle(vertices,r*40,'#000',false);

            //圆心
            var O=threeDimension.createCircle(vertices,7,'#F35A27',true);
            var textO=threeDimension.createText('A('+(-getParameter.D/2).toFixed(1)+','+(-getParameter.E/2).toFixed(1)+')',-getParameter.D/2*40+50,-getParameter.E/2*40+30,0,'#000',24);

            threeDimension.Obj.add(line,O,textO,liner,textr,P);
            threeDimension.scene.add(threeDimension.Obj)
        }
    },
    createCircle:function(vertices,radius,color,k){
        var CircleG;
        if(k){
            CircleG = new THREE.CircleGeometry(radius, 50, 0, 2 * Math.PI);
        }else{
            CircleG = new THREE.TorusGeometry(radius, 0.6, 50,50, 2 * Math.PI);
        }
...
$("#slider1").change(function(){
    ...
    if(val == 0){
        ...
        getParameter.D=(val/10).toFixed(1);
    } 
    threeDimension.createObj();
});
$("#slider2").change(function(){
    ...
    if(val == 0){
        ...
        getParameter.E=(val/10).toFixed(1);
    } 
    threeDimension.createObj();
});
$("#slider3").change(function(){
    ...
    if(val == 0){
        ...
        getParameter.F=(val/10).toFixed(1);
    } 
    threeDimension.createObj();
});
function renew(){
    ...
    $('.slider1,.slider2').find('.sliderLeft').css({'width':'226px'});
    $('.slider1,.slider2').find('.xdsoft_range2dslider_runner').css({'left':'226px'});
    $('.slider1').find('.xdsoft_slider_label').text('D = 1.0');
    $('.slider2').find('.xdsoft_slider_label').text('E = 1.0');
    $('.slider1,.slider2').attr('value',''+1.0+'|0');
    $('.slider3').find('.sliderLeft').css({'width':'185px'});
    $('.slider3').find('.xdsoft_range2dslider_runner').css({'left':'185px'});
    $('.slider3').find('.xdsoft_slider_label').text('F = -1.0');
    $('.slider3').attr('value',''+-1.0+'|0');
    getParameter = {
        D:1,
        E:1,
        F:-1
