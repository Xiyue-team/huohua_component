...
var threeDimension = {
    ...
    createObj:function(){
        ...
        //直线l
        var line=threeDimension.createLine_text(getParameter.j,'l');

        //辅助线
        if(key==true){
            var Px=Math.max(getParameter.by,getParameter.ay)/Math.tan(getParameter.j/180*Math.PI);
            var Px1=Math.max(getParameter.bx,getParameter.ax);
            var Px2=Math.min(getParameter.bx,getParameter.ax);
            var Py=Math.min(getParameter.by,getParameter.ay);

            //辅助线
            vertices = [];
            if(getParameter.j!=0&&getParameter.j!=90&&getParameter.j!=180){
                vertices.push(threeDimension.vec3(getParameter.ax,getParameter.ay,-1));
                vertices.push(threeDimension.vec3(Px,Py,-1));
                vertices.push(threeDimension.vec3(getParameter.bx,getParameter.by,-1));
                var linef = threeDimension.createLineMesh(vertices,'#000',2);
                threeDimension.Obj.add(linef);
            }
            ...
            var textP;
            var P,P1;
            if(getParameter.j<90&&getParameter.j!=0){
                textP=threeDimension.createText('P('+parseFloat(Px1/40).toFixed(1)+','+parseFloat(Py/40).toFixed(1)+')',Px1+60,Py-10,0,'#000',24);
                vertices.push(Px1,Py,0);
                P=threeDimension.createCircle(vertices,10,'#7ED321');
                threeDimension.Obj.add(P,textP);
            }else if(getParameter.j>90&&getParameter.j!=180){
                textP=threeDimension.createText('P('+parseFloat(Px2/40).toFixed(1)+','+parseFloat(Py/40).toFixed(1)+')',Px2-60,Py-10,0,'#000',24);
            ...
            if(getParameter.j<90&&getParameter.j!=0){
                for(var i=0;i<getParameter.j;i++){
                    sx = 25*Math.cos(Math.PI/180*i)+Px2;
                    sy = 25*Math.sin(Math.PI/180*i)+Py;
                    vertices.push(threeDimension.vec3(sx,sy,3));
                }
                texts=threeDimension.createText('θ',40*Math.cos(Math.PI/180*getParameter.j/2)+Px2,40*Math.sin(Math.PI/180*getParameter.j/2)+10+Py,0,'#000',24);
                angs = threeDimension.createLineMesh(vertices,0x5B9BD5,3);
                threeDimension.Obj.add(angs,texts);
            }else if(getParameter.j>90&&getParameter.j!=180){
                for(var i=0;i<(180-getParameter.j);i++){
                    sx = -25*Math.cos(Math.PI/180*i)+Px1;
                    sy = 25*Math.sin(Math.PI/180*i)+Py;
                    vertices.push(threeDimension.vec3(sx,sy,3));
                }
                texts=threeDimension.createText('θ',-40*Math.cos(Math.PI/180*(180-getParameter.j)/2)+Px1,40*Math.sin(Math.PI/180*(180-getParameter.j)/2)+10+Py,0,'#000',24);
                angs = threeDimension.createLineMesh(vertices,0x5B9BD5,3);
                threeDimension.Obj.add(angs,texts);
            }
        }
        ...
        //点A
        vertices = [];
        vertices.push(getParameter.ax,getParameter.ay,0);
        var ax=parseFloat(getParameter.ax/40).toFixed(1);
        var ay=parseFloat(getParameter.ay/40).toFixed(1);
        var textA
        if(getParameter.j==0||getParameter.j==180){
            if(getParameter.ax>getParameter.bx){
                textA=threeDimension.createText('A('+(ax==0?'0.0':ax)+','+(ay==0?'0.0':ay)+')',getParameter.ax+70,getParameter.ay+30,0,'#000',24);
            }else{
                textA=threeDimension.createText('A('+(ax==0?'0.0':ax)+','+(ay==0?'0.0':ay)+')',getParameter.ax-70,getParameter.ay+30,0,'#000',24);
            }
        }else if(getParameter.j<=90){
            textA=threeDimension.createText('A('+(ax==0?'0.0':ax)+','+(ay==0?'0.0':ay)+')',getParameter.ax-70,getParameter.ay+30,0,'#000',24);
        }else{
            textA=threeDimension.createText('A('+(ax==0?'0.0':ax)+','+(ay==0?'0.0':ay)+')',getParameter.ax+70,getParameter.ay+30,0,'#000',24);
        }
        var A=threeDimension.createCircle(vertices,10,'#F49B02');
        var A1=threeDimension.createCircle(vertices,40,'#F49B02');
        A1.material.transparent = true;
        A1.material.opacity = 0;
        A1.name='a';
        
        //点B
        vertices = [];
        vertices.push(getParameter.bx,getParameter.by,0);
        var bx=parseFloat(getParameter.bx/40).toFixed(1);
        var by=parseFloat(getParameter.by/40).toFixed(1);
        var textB
        if(getParameter.j==0||getParameter.j==180){
            if(getParameter.bx>getParameter.ax){
                textB=threeDimension.createText('B('+(bx==0?'0.0':bx)+','+(by==0?'0.0':by)+')',getParameter.bx+70,getParameter.by+30,0,'#000',24);
            }else{
                textB=threeDimension.createText('B('+(bx==0?'0.0':bx)+','+(by==0?'0.0':by)+')',getParameter.bx-70,getParameter.by+30,0,'#000',24);
            }
        }else if(getParameter.j<=90){
        ...
        var B=threeDimension.createCircle(vertices,10,'#5DB5F0');
        var B1=threeDimension.createCircle(vertices,40,'#5DB5F0');
        B1.material.transparent = true;
        B1.material.opacity = 0;
        B1.name='b';
        selectobjs.push(A1,B1);
        threeDimension.Obj.add(line,A,textA,B,textB,A1,B1);
        ...
        if(getParameter.j!=0){
            for(var i=0;i<getParameter.j;i++){
                dx = 35*Math.cos(Math.PI/180*i);
                dy = 35*Math.sin(Math.PI/180*i);
                vertices.push(threeDimension.vec3(dx,dy,3));
            }
            var ang = threeDimension.createLineMesh(vertices,0x5B9BD5,3);
           
            var textj=threeDimension.createText('α',50*Math.cos(Math.PI/180*getParameter.j/2),50*Math.sin(Math.PI/180*getParameter.j/2)+15,0,'#000',24);
            threeDimension.Obj.add(ang,textj);
        }

        threeDimension.scene.add(threeDimension.Obj);
    },
    createLine_text:function(j,name){
         var vertices = [];
         var line_text=new THREE.Group();
         var y1=(10*Math.tan(j/180*Math.PI))
         var y2=(-10*Math.tan(j/180*Math.PI))
         var x1=(10)/Math.tan(j/180*Math.PI)
         var x2=(-10)/Math.tan(j/180*Math.PI)
         var x,y,text;
         if(j==90){
            vertices.push(threeDimension.vec3(0,400,0));
            vertices.push(threeDimension.vec3(0,-400,0));
            text=threeDimension.createText(name,15,415,0,'#000',24);
         }else{
            if(j<90){
                if(y1>-10&&y1<10){
                    vertices.push(threeDimension.vec3(400,y1*40,0));
                    x=415;
                    y=y1*40+20
                }else if(x1>-10&&x1<10){
                    vertices.push(threeDimension.vec3(x1*40,400,0)); 
                    x=x1*40+20;
                    y=415  
                }
                if(y2>-10&&y2<10){
                    vertices.push(threeDimension.vec3(-400,y2*40,0));
                }else if(x2>-10&&x2<10){
                    vertices.push(threeDimension.vec3(x2*40,-400,0));
                }
    ...
    onDocumentMouseMove:function(event){
        ...
        if(mousedownflag){
            if ( raycaster.ray.intersectPlane( plane, intersection ) ) {
                ...
                var x = Math.round(obj.x/40)*40
                var y = Math.round(obj.y/40)*40
                if((Math.pow(x/40,2)+Math.pow(y/40,2))<=100){
                    if(selectobj.name=='a'&&(Math.pow(x-getParameter.bx,2)+Math.pow(y-getParameter.by,2))>=1600){
                        if(getParameter.j>=135||getParameter.j<=45){
                            getParameter.ax = x;
                            getParameter.ay = x*Math.tan(getParameter.j/180*Math.PI);
                        }else{
                            getParameter.ax = y/Math.tan(getParameter.j/180*Math.PI)
                            getParameter.ay = y
                        }
                        
                    }else if(selectobj.name=='b'&&(Math.pow(x-getParameter.ax,2)+Math.pow(y-getParameter.ay,2))>=1600){
                        if(getParameter.j>=135||getParameter.j<=45){
                            getParameter.bx = x;
                            getParameter.by = x*Math.tan(getParameter.j/180*Math.PI);
                        }else{
                            getParameter.bx = y/Math.tan(getParameter.j/180*Math.PI)
                            getParameter.by = y
                        }
                    }
                }
...
$("#slider1").change(function(){
    ...
    getParameter.j = val;
    var lA=Math.sqrt(Math.pow(getParameter.ax,2)+Math.pow(getParameter.ay,2));
    var lB=Math.sqrt(Math.pow(getParameter.bx,2)+Math.pow(getParameter.by,2));
    var cosx=Math.cos(getParameter.j/180*Math.PI);
    var sinx=Math.sin(getParameter.j/180*Math.PI);
    var cosd=Math.cos((180-getParameter.j)/180*Math.PI);
    var sind=Math.sin((180-getParameter.j)/180*Math.PI);
    panduan('ax','ay',lA);
    panduan('bx','by',lB);
    threeDimension.createObj();

    //判断转动时点的象限变化
    function panduan(x,y,l){
        if(getParameter[y]>0){
            if(getParameter.j<90){
                getParameter[x]=l*cosx;
                getParameter[y]=l*sinx;
            }else if(getParameter.j==90){
                getParameter[x]=0;
                getParameter[y]=l;
            }else{
                getParameter[x]=-l*cosd;
                getParameter[y]=l*sind;
            }   
        }else if(getParameter[y]<0){
            if(getParameter.j<90){
                getParameter[x]=-l*cosx;
                getParameter[y]=-l*sinx;
            }else if(getParameter.j==90){
...
function renew(){
    ...
    key=false;
    $('.slider1').find('.sliderLeft').css({'width':'103px'});
    $('.slider1').find('.xdsoft_range2dslider_runner').css({'left':'103px'});
...
if(isMob){
	...
	//on/off事件
	$('#div1').on('touchstart',clickEve);
	$('#div2').on('touchstart',clickEve);
	/*全屏事件*/
	...
	//on/off事件
	$('#div1').on('click',clickEve);
	$('#div2').on('click',clickEve);
	/*全屏事件*/
