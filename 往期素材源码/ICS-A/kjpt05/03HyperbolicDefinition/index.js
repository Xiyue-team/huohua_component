...
var a=1,c=3;
var demo=0;
var line1=null,line2=null,dashline1=null,dashline2=null,dashline3=null,dashline4=null,dashline5=null,dashline6=null,dashline7=null,dashline8=null;
var sphere1=null,sphere2=null,sphereP1=null,sphereP2=null,sphereP3=null,sphereP4=null;
var text1=null,text2=null,text3=null,text4=null,text5=null,text6=null;
...
function ThreeDimensional() {
    ...
    this.createLine=function(){
        if(line1!=null){
            this.scene.remove(line1,line2,dashline1,dashline2,dashline3,dashline4,dashline5,dashline6,dashline7,dashline8);
        }
        var point1=[];

        line1 = new THREE.Object3D();
        line2 = new THREE.Object3D();
        var bpin=Math.pow(c,2)-Math.pow(a,2);
        var num=(c-a)/10;
        for(var i=a+(num*demo);i>=a;i-=0.01){
            var result1=((Math.pow(i,2)/Math.pow(a,2))-1)*bpin;
            var resulty1=Math.pow(result1,1/2);
            point1.push(new THREE.Vector3(i*50,resulty1*50,0));
        }
        for(var k=a;k<=a+(num*demo);k+=0.01){
            var result2=((Math.pow(k,2)/Math.pow(a,2))-1)*bpin;
            var resulty2=Math.pow(result2,1/2);
            point1.push(new THREE.Vector3(k*50,-resulty2*50,0));
        }
        ...
        //虚线
        var numy1=Math.pow((((Math.pow(((-a-(num*demo))),2)/Math.pow(a,2))-1)*bpin),1/2);
        var numy2=Math.pow((((Math.pow(((a+(num*demo))),2)/Math.pow(a,2))-1)*bpin),1/2);
        var vertices1=[];
        vertices1.push(vec3((-a-(num*demo))*50,numy1*50,-1));
        vertices1.push(vec3(-c*50,0,-1));
        ...
        vertices1=[];
        vertices1.push(vec3((a+(num*demo))*50,numy2*50,-1));
        vertices1.push(vec3(c*50,0,-1));
        geometryLine1 = new THREE.Geometry();
        geometryLine1.vertices = vertices1;
        geometryLine1.computeLineDistances();
        ...
        dashline5=dashline1.clone();
        this.scene.add(dashline5);
        dashline5.rotation.z=Math.PI;
        dashline6=dashline2.clone();
        this.scene.add(dashline6);
        dashline6.rotation.z=Math.PI;
        dashline7=dashline3.clone();
        this.scene.add(dashline7);
        dashline7.rotation.z=Math.PI;
        dashline8=dashline4.clone();
        this.scene.add(dashline8);
        dashline8.rotation.z=Math.PI;
    ...
    this.createsphereF=function(){
        if(group1!=null){
            this.scene.remove(group1,group2);
        ...
        group1.position.set(-c*50,0,0);




        var vertices2=[];
        vertices2.push(c*50,0,1);
        sphere2=createCircle([0, 0, 1],30,'#65B2EE',0);
        sphere2.material.depthTest = false;
        var sphere22=createCircle([0, 0, 0],10,'#65B2EE');
        sphere2.name="p2";
        group2.add(sphere2, sphere22);
        group2.position.set(c*50,0,1);
        selectobjs.push(sphere2);
        ...
        text1=createText("F1",-c*50,30,0,"#000000");
        text2=createText("F2",c*50,30,0,"#000000");

        this.scene.add(group1,group2,text1,text2);
    };
    this.createsphereP=function(){
        if(sphereP1!=null){
            this.scene.remove(sphereP1,sphereP2,sphereP3,sphereP4,text3,text4,text5,text6);
        }
        var vertices1=[];
        var num=(c-a)/10;
        var bpin=Math.pow(c,2)-Math.pow(a,2);
        var resultx=-a-(num*demo);
        var result1=((Math.pow(resultx,2)/Math.pow(a,2))-1)*bpin;
        var resulty=Math.pow(result1,1/2);
        vertices1.push(resultx*50,resulty*50,1);
        sphereP1=createCircle(vertices1,10,'#F55B23');
        var vertices2=[];
        vertices2.push(-resultx*50,resulty*50,1);
        sphereP2=createCircle(vertices2,10,'#F55B23');
        var vertices3=[];
        vertices3.push(resultx*50,-resulty*50,1);
        sphereP3=createCircle(vertices3,10,'#F55B23');
        var vertices4=[];
        vertices4.push(-resultx*50,-resulty*50,1);
        sphereP4=createCircle(vertices4,10,'#F55B23');

        text3=createText("P",resultx*50,resulty*50+30,0,"#000000");
        text4=createText("P",-resultx*50,resulty*50+30,0,"#000000");
        text5=createText("P",resultx*50,-resulty*50-10,0,"#000000");
        text6=createText("P",-resultx*50,-resulty*50-10,0,"#000000");

        if(demo==0){
          this.scene.add(sphereP1,sphereP2,text3,text4);
        }
        else{
          this.scene.add(sphereP1,sphereP2,sphereP3,sphereP4,text3,text4,text5,text6);
        }
    ...
    this.dynamic=function(){
        ...
        a1=setInterval(function(){
            ...
            $('.slider2').find('.sliderLeft').css({'width':(demo*41)+'px'});
            $('.slider2').find('.xdsoft_range2dslider_runner').css({'left':(demo*41)+'px'});
            $('.slider2').find('.xdsoft_slider_label').text(demo);
            $('.slider2').attr('value',''+demo+'|0');
            thiz.createLine();
            thiz.createsphereP();
            demo++;
        },200)
