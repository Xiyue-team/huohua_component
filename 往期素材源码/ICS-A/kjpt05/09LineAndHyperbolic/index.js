...
function ThreeDimensional() {
    ...
    this.createObj=function(){
        ...
        //双曲线
        vertices=[];
        var c=Math.pow(20,1/2);
        for(var i=-10;i<=-2;i+=0.01){
            vertices.push(vec3(i*50,Math.pow((4*(Math.pow(i,2))-16),1/2)*50,1))
        }
        for(var m=-2;m>=-10;m-=0.01){
            vertices.push(vec3(m*50,-Math.pow((4*(Math.pow(m,2))-16),1/2)*50,1))
        }
        ...
        //直线
        vertices=[];
        vertices.push(vec3(-10*50,k*(-10+1)*50,1));
        vertices.push(vec3(7*50,k*(7+1)*50,1));
        var linek=createLineMesh(vertices,'#1161c8',3);

        vertices=[];
        vertices.push(vec3(-1*50,0,1));
        var P=createCircle(vertices,10,'#60b6f1');

        vertices=[];
        vertices.push(vec3(-1*50,35,1));
        ...
        if(k<-2.31||k>2.31){
            ...
            $('#result3').text('相离')
        }else if(k>-2.31&&k<2.31){
            $('.formula-right').text(2);
            $('#result3').text('相交');
            var x1=(-2*k*k+Math.sqrt(4*Math.pow(k,4)-4*(k*k-4)*(k*k+16)))/(2*(k*k-4));
            var y1=k*(x1+1);
            var x2=(-2*k*k-Math.sqrt(4*Math.pow(k,4)-4*(k*k-4)*(k*k+16)))/(2*(k*k-4));
            var y2=k*(x2+1);
            ...
            var x=(-2*k*k+Math.sqrt(4*Math.pow(k,4)-4*(k*k-4)*(k*k+16)))/(2*(k*k-4));
            var y=k*(x+1);
...
$('#slider1').change(function(){
    ...
    if(k>=-2.34&&k<=-2.28){
        k=-2.31;
        $('.slider1').find('.sliderLeft').css({'width':'47px'});
        $('.slider1').find('.xdsoft_range2dslider_runner').css({'left':'47px'});
        // $('.slider1').find('.xdsoft_slider_label .xdsoft_slider_label_top').text('-0.67')
        $('.slider1').attr('value',''+-231+'|0');
    }else if(k>=2.28&&k<=2.34){
        k=2.31;
        $('.slider1').find('.sliderLeft').css({'width':'362px'});
        $('.slider1').find('.xdsoft_range2dslider_runner').css({'left':'362px'});
        // $('.slider1').find('.xdsoft_slider_label .xdsoft_slider_label_top').text('0.67');
        $('.slider1').attr('value',''+231+'|0');
    }
    three.createObj();
