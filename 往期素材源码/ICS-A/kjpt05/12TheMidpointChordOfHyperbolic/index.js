...
var xM=0.3,yM=4,selectobjs=[],selectobj=null,mousedownflag;
var threeDimension = {
    //初始化
    init:function(){
    ...
    createObject:function(){
        ...
        //双曲线
        vertices=[];
//        var c=Math.pow(25,1/2);
        for(var i=-10;i<=-Math.sqrt(2);i+=0.01){
            vertices.push(threeDimension.vec3(i*40,Math.pow(((2*Math.pow((i),2))-4),1/2)*40,1))
        }
        for(var m=-Math.sqrt(2);m>=-10;m-=0.01){
            vertices.push(threeDimension.vec3(m*40,-Math.pow(((2*Math.pow((m),2))-4),1/2)*40,1))
        }
...
//        //渐近线
//        var point1=[],point2=[];
//        point1.push(threeDimension.vec3(-10*40,(Math.sqrt(4)/Math.sqrt(2))*-10*40,1));
//        point1.push(threeDimension.vec3(10*40,(Math.sqrt(4)/Math.sqrt(2))*10*40,1));
//        point2.push(threeDimension.vec3(-10*40,-(Math.sqrt(4)/Math.sqrt(2))*-10*40,1));
//        point2.push(threeDimension.vec3(10*40,-(Math.sqrt(4)/Math.sqrt(2))*10*40,1));
//        var pline1=threeDimension.createLineMesh(point1,'#1161c8',3);
//        var pline2=threeDimension.createLineMesh(point2,'#1161c8',3);
//        threeDimension.obj.add(pline1,pline2);

        var a=4*xM*xM-2*yM*yM;
        var b=4*xM*yM*yM-8*Math.pow(xM,3);
        var c=4*Math.pow(xM,4)+Math.pow(yM,4)-4*xM*xM*yM*yM+4*yM*yM;

        var data=Math.pow(b,2)-4*a*c;
        var resultx=Math.sqrt(2);

        if(yM.toFixed(1)==0&&Math.abs(xM)>resultx){
            var vertices1=[];
            var x1=xM;
            var y1=Math.sqrt(Math.abs(2*x1*x1-4));
            vertices=[];
            ...
            vertices=[];
            var x2=xM;
            var y2=-Math.sqrt(Math.abs(2*x2*x2-4));
            ...
            var textA=threeDimension.createText('B('+x1.toFixed(1)+','+y1.toFixed(1)+')',x1*40+50,y1*40+20,1,'#000',18);
            threeDimension.obj.add(textA);

            var textB=threeDimension.createText('A('+x2.toFixed(1)+','+y2.toFixed(1)+')',x2*40-50,y2*40,1,'#000',18);
            threeDimension.obj.add(textB);

        }else if(xM.toFixed(1)==0){
           var vertices1=[];
           var y1=yM;
           var x1=Math.sqrt(1/2*(y1*y1)+2); 
           vertices=[];
           vertices.push(x1*40,y1*40,1);
           var A=threeDimension.createCircle(vertices,10,'#F55B23');
           threeDimension.obj.add(A);
           ...
           var y1=(2*xM*x1-2*xM*xM+yM*yM)/yM;
           vertices=[];
           vertices.push(x1*40,y1*40,1);
           var A=threeDimension.createCircle(vertices,10,'#F55B23');
           threeDimension.obj.add(A);
           ...
           var y2=(2*xM*x2-2*xM*xM+yM*yM)/yM;
           vertices=[];
           vertices.push(x2*40,y2*40,1);
           var B=threeDimension.createCircle(vertices,10,'#F55B23');
           threeDimension.obj.add(B);
        ...
        var k=4*xM/(2*yM);
        if(xM.toFixed(1)==0&&yM.toFixed(1)==0){
            $('.title_math2').html('<img src="images/2.png" style="height: 38px;width: auto;margin-top: 14px;"/> 不存在');
            $('.turnShade').html('');
        }else if(xM.toFixed(1)==0){
            $('.title_math2').html('<img src="images/1.png" style="height:auto;width:140px;margin-top=15px;"/>');
            $('.turnShade').html('<img src="images/3.png" style="height: auto;width: 100px;"/>');
        }else if(yM.toFixed(1)==0&&Math.abs(xM)>resultx){
            $('.title_math2').html('<img src="images/2.png" style="height: 38px;width: auto;margin-top: 14px;"/> 不存在');
            $('.turnShade').html('<img src="images/4.png" style="height: auto;width: 100px;"/>');
        }else if(data<=0){
            $('.title_math2').html('<img src="images/2.png" style="height: 38px;width: auto;margin-top: 14px;"/> 不存在');
            $('.turnShade').html('');
        }else{
            $('.title_math2').html('<img src="images/kab.png" alt="">');
            $('.turnShade').html('<img src="images/result.png" style="width:350px;"/>');
        }
...
//
//                }
                xM=Number(parseFloat(x).toFixed(2));
                yM=Number(parseFloat(y).toFixed(2));
                threeDimension.createObject();
