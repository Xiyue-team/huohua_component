      ...
      init(){
          ...
          var an2=function (O,i) {
              ...
              if(i==1){
                  ...
                  if(O.position.z>120||O.position.z<-130){
                      O.position.z*=0.5;
                  }
                  ...
                  if(!thiz.radio_checked) {
                      if (O.position.x > 190 || O.position.x < -190) {
                          O.position.x *= 0.5;
                      }
                  }else{
                      if(O.position.x*Math.tan(Math.PI/3)-350>O.position.y||O.position.x*Math.tan(Math.PI/3)+310<O.position.y){
                          O.position.x*=0.5;
                          O.position.y*=0.5;
                      }
                  }
          ...
          function YB() {
              ...
              for(var i=0;i<5;i++){
                  for(var j=0;j<5;j++){
                      for(var k=0;k<8;k++){
                          shapeY['shape'+i][j*8+k]=shapeB.clone();
                          shapeY['shape'+i][j*8+k].mass=0;
                          shapeY['shape'+i][j*8+k].position.x=-67.5-90+k*45;
                          shapeY['shape'+i][j*8+k].position.z=-67.5-22.5+j*45;
                          shapeY['shape'+i][j*8+k].position.y=-90+i*45;
                          if(i==1||i==3){
                              shapeY['shape'+i][j*8+k].position.x+=22.5;
                              shapeY['shape'+i][j*8+k].position.z-=22.5;
                          }
                          var y=shapeY['shape'+i][j*8+k].position.y;
                          var z=shapeY['shape'+i][j*8+k].position.z;
                          an(shapeY['shape'+i][j*8+k],y,z)
                          scene.add(shapeY['shape'+i][j*8+k]);
                      }
                  }
              }
          }
...
//                          }
          ...
          var YZ=function (f,f2) {
              ...
              for(var i=DZ.length/5*4-1;i<DZ.length;i++){
                  DZ[i].position.x+=step*3;
                  DZ[i].__dirtyPosition=true;
              }
              for(var i=DZ.length/5*2-1;i<DZ.length/5*4;i++){
                  DZ[i].position.x+=step;
                  DZ[i].__dirtyPosition=true;
              }
              for(var i=0;i<DZ.length/5*2;i++){
                  DZ[i].position.x-=step;
                  DZ[i].__dirtyPosition=true;
              }
          ...
          (function () {
              var cube1 = new THREE.Mesh(new THREE.CubeGeometry(10, 300, 250),
              ...
              cube1.position.z=-13;
              var F=createText('-',-200,200,-6,'#000',28);
              var F1=createText('-',-200,-150,-6,'#000',28);

              var cube2 = new THREE.Mesh(new THREE.CubeGeometry(10, 300, 250),
                  new THREE.MeshPhongMaterial({
                      color: 0xcccccc,
                      transparent:true,
                      opacity:0.5
              ...
              cube2.position.x=225;
              cube2.position.z=-13;
              var Z=createText('+',225,200,-6,'#000',28);
              var Z1=createText('+',225,-150,-6,'#000',28);
          ...
          var render=function() {
              if(thiz.switch_checked){
                  for(var i in scene.children){
                      if((scene.children[i].position.x>220
                              || scene.children[i].position.x<-210
                              || scene.children[i].position.y>125
                              || scene.children[i].position.y<-125
                              || scene.children[i].position.z>150
                              || scene.children[i].position.z<-150
