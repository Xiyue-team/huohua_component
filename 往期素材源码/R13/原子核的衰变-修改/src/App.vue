  ...
  <div id="app" class="noselect">
    <div class="container">
      ...
      <div id="renderCanvas">
        ...
        <div v-if='src&&over'
             style="width:300px;height:145px;position:absolute;bottom:10px;left:0;right:0;margin:auto;z-index: 999;">
          <img v-if="src=='mark1'" style="margin-top:34px;" src="static/UI/1.png">
          <img v-if="src=='mark2'" src="static/UI/2.png">
          <img v-if="src=='mark2'" src="static/UI/3.png">
        </div>
      </div>
    </div>
    <!--侧边按钮区-->
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        var anImg = [];
        for (let i = 0; i <= 10; i++) {
          i <= 9 ? anImg[i] = loader.load('static/collide/collide_' + i + '.png') : anImg[i] = anImg[0];
        }

        //U原子
        var U = createP(220 * 1.8, 216 * 1.8, 5, UPic);
        //Th原子
        var Th = createP(220 * 1.8, 216 * 1.8, 5, ThPic);
        //Pa原子
        var Pa = createP(220 * 1.8, 216 * 1.8, 5, PaPic);
        //Alpha原子
        var Alpha = createP(128 * 0.7 * 1.8, 122 * 0.7 * 1.8, 1, AlphaPic);
        //电子
        var DZ = createP(60 * 0.5 * 1.8, 60 * 0.5 * 1.8, 1, DZPic);
        //反电子中微子
        var ZW = createP(60 * 0.5 * 1.8, 60 * 0.5 * 1.8, 1, ZWPic);

        //能量波
        var Power = createP(820 * 1.8, 820 * 1.8, 0, 'static/collide/collide_0.png');

        //匀强磁场
        // var Arrow=createP(691*1.4,428*1.4,0,arrowPic);
        scene.add(U, Th, Pa, Alpha, DZ, ZW, Power/*,Arrow*/);
        ...
        var Play = (type) => {
          ...
          if (type == 1) {
            ...
            // x0=r1*Math.cos(ang12*PI)+Alpha.position.x;
            // y0=r1*Math.sin(ang12*PI)+Alpha.position.y;
            stepx = Math.cos(ang10 * PI) * 3;
            stepy = Math.sin(ang10 * PI) * 3;
            an = () => {
              anImgNum++;
              ...
              //   }
              // }
              if (anImgNum >= 30 && anImgNum % 10 == 0) {
                Power.material.map = anImg[anImgNum / 10];
                if(!Th.visible){
                  U.visible = false;
                  Th.visible = true;
                  this.Obj = Th;
            ...
            // x1=r2*Math.cos(ang22*PI)+DZ.position.x;
            // y1=r2*Math.sin(ang22*PI)+DZ.position.y;
            step1x = Math.cos(ang20 * PI) * 3;
            step1y = Math.sin(ang20 * PI) * 3;
            if (step1x0 == null) {
              step1x0 = step1x;
              step1y0 = step1y;
            }
