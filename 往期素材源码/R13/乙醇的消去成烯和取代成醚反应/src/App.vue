<template>
    <div id="app" class="noselect">
        <div class="container">
            ...
            <!--视图区-->
            <div class="ViewSpace" id="scale">
                <div :style="[zoom]" class="viewBox">
                    <div class="imgWrap" v-if='!R' ref="canvas">
                        <!-- </div> -->
                        <img :src="'static/image/Elimination into ene/'+num+'.jpg'">
                    </div>
                    <div class="imgWrap" v-if='R' ref="canvas">
                        <!-- </div> -->
                        <img :src="'static/image/Replace into ether/'+num+'.jpg'">
                    </div>
                </div>
            </div>
            <div class="footerTrue">
                ...
                <div class="footer">
                    <ui-slider id='slider' class="footerSlider" :min="0" :max="130" :title="false" :box="false"
                ...
                <div class="cz">
                    <ui-btn :type="blue1" size="big" class="rightBtn" id="elimination" @click.native="elimination()">
                        消去成烯
                    </ui-btn>
                    <ui-btn :type="blue2" size="big" class="rightBtn" id="replace" @click.native="Replace()">取代成醚
                    </ui-btn>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        created() {
            ...
            let promises = this.arr.map((value, index) => {
                return this.preloadImage(`static/image/Elimination into ene/${value}.jpg`).then((image) => {
                    this.store1[index] = image;
                })
            });
            Promise.all(promises).then(() => {
                this.store = this.store1;
                let promises = this.arr2.map((value, index) => {
                    return this.preloadImage(`static/image/Replace into ether/${value}.jpg`).then((image) => {
        ...
        mounted() {
            ...
            this.value = 0;
            $('#elimination').css('color', '#FFF');
            $('#replace').css('color', '#666');
        },
        computed: {},
        watch: {
            value(v) {
        ...
        methods: {
            ...
            setSliderPonint() {
                ...
                $('#sliderP span').each(function () {
                    $(this).index();
                    $(this).css("left", sliderW * vm.sliderPoint[$(this).index()] / 130 - 7)
                });
                $('.ui-label li').each(function () {
                    $(this).index();
                    $(this).css("left", sliderW * vm.sliderPoint[$(this).index()] / 130 + 22)
                })
