...
<template v-else>
  ...
  >
  <span v-text="dotTxt" class="dotTxt" id="xliz"></span>
  <!--tip-->
...
<span v-if="piecewiseLabel"
class="ui-piecewise-label ui-piecewise-label-top"
:style="{fontSize:title&&!obj.inRange?'12px':'',lineHeight:title&&!obj.inRange?'18px':''}"
:class="{labelActiveStyle:obj.inRange}">
{{ obj.label[1] }}
</span>
...
<script>
  export default {
             ...
             * */
             wrapStyles () {
              return {
                ...
                padding: this.direction === 'vertical'?`${this.dotSize / 2}px ${this.dotSize / 2+5}px`:(this.piecewiseLabel?`0 ${this.dotSize / 2+20}px`:`0 ${this.dotSize / 2}px`)
              }
            },
    ...
    .uiSlider .ui-label li div:first-child {
      font-size: 18px;
    }
