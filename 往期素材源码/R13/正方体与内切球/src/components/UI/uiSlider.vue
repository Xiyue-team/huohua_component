<template>
  <div ref="wrap" :class="['uiSlider',isBox, flowDirection,  { 'ui-has-piecewise': piecewiseLabel,timeLine:timeLine,'noBlueProcess':noBlueProcess?noBlueProcess:noBlue }]" :style="wrapStyles" @click="wrapClick">
    <!--标题-->
    <h3 v-text="title" v-if="title" class="ui-title"></h3>
    <!--副标题-->
    <p v-text="subTitle" v-if="subTitle" class="ui-subTitle"></p>
    <!--滑条上的文字-->
    <ul v-if="timeLine && label" ref="label" :style="{width:infinity?'53%':''}" :class="'label'+labelRows.length" class="ui-label">
      <li v-for="(la,index) in labelRows" v-text="la.txt" :class="{activeLabel:la.active}" :style="{width:100/labelRows.length+'%'}"></li>
    </ul>
    <!--滑条上的文字-->
    <ul v-if="!timeLine && label" ref="label" :style="{width:infinity?'53%':''}" class="ui-label">
      <li v-for="(la,index) in label">
        <div>{{la[1]}}</div>
        <div>{{la[0]}}</div>
      </li>
    </ul>
    <!--滑块-->
    <div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles,infinityStyle]">
      <!--双向-->
      <template v-if="valueIsArray">
        <div ref="dot0" :class="[tooltipStatus, 'ui-dot']" :style="[dotStyles]" @mousedown="moveStart(0)" @touchstart="moveStart(0)"> <span v-text="dotTxt && dotTxt[0]" class="dotTxt"></span>
          <!--tip--><span :class="['ui-toolTip-' + tooltipDirection[0], 'ui-toolTip-wrap']">
            {{ formatter ? formatting(val[0]) : val[0] }}
          </span> </div>
        <div ref="dot1" :class="[tooltipStatus, 'ui-dot']" :style="[dotStyles]" @mousedown="moveStart(1)" @touchstart="moveStart(1)"> <span v-text="dotTxt && dotTxt[1]" class="dotTxt"></span>
          <!--tip--><span :class="['ui-toolTip-' + tooltipDirection[1], 'ui-toolTip-wrap']">
            {{ formatter ? formatting(val[1]) : val[1] }}
                    </span> </div>
      </template>
      <!--单向-->
      <template v-else>
        <div ref="dot" :class="[tooltipStatus, 'ui-dot']" :style="[dotStyles]" @mousedown="moveStart" @touchstart="moveStart"> <span v-text="dotTxt" class="dotTxt"></span>
          <!--tip--><span :class="['ui-toolTip-' + tooltipDirection, 'ui-toolTip-wrap']">
            {{ formatter ? formatting(val) : val }}
                    </span> </div>
      </template>
      <!--滑条及滑条上的圆点、文字-->
      <ul class="ui-piecewise" ref="sliderWidth">
        <li v-for="(obj, index) in piecewiseDotWrap" class="ui-piecewise-item" :style="[piecewiseDotStyle, obj.style]" :key="index">
          <!--滑条上的圆--><span v-if="piecewise" class="ui-piecewise-dot" :class="{piecewiseActiveStyle:obj.inRange}"></span>
          <!--滑条上方的文字--><span v-if="piecewiseLabel" class="ui-piecewise-label" :style="{fontSize:title&&!obj.inRange?'12px':'',lineHeight:title&&!obj.inRange?'18px':''}" :class="{labelActiveStyle:obj.inRange}">
                            {{ obj.label[0] }}
                    </span> <span v-if="piecewiseLabel" class="ui-piecewise-label ui-piecewise-label-top" :style="{fontSize:title&&!obj.inRange?'12px':'',lineHeight:title&&!obj.inRange?'18px':''}" :class="{labelActiveStyle:obj.inRange}">
                            {{ obj.label[1] }}
                    </span> </li>
      </ul>
      <!--蓝色滑条-->
    ...
    <!--无穷大样式-->
    <div v-if="infinity" class="ui-infinity" :style="{height:processHeight+'px'}"> <span></span><span></span> <span></span> <span></span><span></span> <small>+∞</small> </div>
    <input v-if="!valueIsArray && !data" class="vue-slider-sr-only" type="range" v-model="val" :min="min" :max="max" /> </div>
</template>
<script>
export default {
  data() {
  ...
  computed: {
    ...
    currentIndex() {
      if (this.valueIsArray) {
        return this.data ? this.currentValue : [
          (this.currentValue[0] - this.minimum) / this.spacing, (this.currentValue[1] - this.minimum) / this.spacing
        ]
      } else {
    ...
    wrapStyles() {
      return {
        ...
        padding: this.direction === 'vertical' ? `${this.dotSize / 2}px ${this.dotSize / 2+5}px` : (this.piecewiseLabel ? `0 ${this.dotSize / 2+20}px` : `0 ${this.dotSize / 2+25}px`)
      }
    },
    elemStyles() {
    ...
    piecewiseDotWrap() {
      ...
      for (let i = 0; i <= this.total; i++) {
        let style = this.direction === 'vertical' ? {
          bottom: `${this.gap * i - this.processWidth}px`,
          left: '-3px'
        } : {
  ...
  methods: {
    calcDotArr() {
      let w = this.$refs.sliderWidth.clientWidth;
      this.dotArr = [];
      for (let i = 1; i <=this.dotsNum; i++) {
        this.dotArr.push(i / this.dotsNum * w);
      }
      return this.dotArr;
    },
    setDotColor(arr, pos) {
      ...
      $(`.ui-piecewise>li:lt(${num+1}) .ui-piecewise-dot`).css('background', '#5caefd');
      $(`.ui-piecewise>li:gt(${num}) .ui-piecewise-dot`).css('background', '#f0f0f0');
    },
    /*
     * 绑定事件
     * */
    ...
    getPos(e) {
      ...
      if (this.direction === 'vertical') {
        if (this.reverse) {
          return (e.pageY / this.zoom - this.offset);
        } else {
          return this.size * this.zoom - (e.pageY / this.zoom - this.offset);
        }
      } else {
        if (this.reverse) {
          return this.size * this.zoom - (e.clientX / this.zoom - this.offset);
        } else {
          return (e.clientX / this.zoom - this.offset);
        }
      }
    },
    /*
    ...
    moving(e) {
      if (!this.flag) {
        ...
        let leftW = this.getPos(e);
        this.setValueOnPos(leftW, true);
        this.setDotColor(this.dotArr,leftW );
    ...
    setValueOnPos(pos, isDrag) {
      ...
      if (this.valueIsArray && this.beyondCircle) {
        if (pos >= 0 && pos <= this.$refs.elem.offsetWidth) {
          ...
          this.setTransform(0);
          this.setCurrentValue(this.min, isDrag);
        } else if (pos > this.$refs.elem.offsetWidth) {
          this.setTransform(this.$refs.elem.offsetWidth);
          this.setCurrentValue(this.max, isDrag);
        }
      } else {
    ...
    getActiveLabel(value) {
      ...
      for (let i = 0; i < this.label.length; i++) {
        this.labelRows[i] = {}
        this.$set(this.labelRows[i], 'txt', this.label[i]);
        if (value) {
          this.$set(this.labelRows[i], 'active', value >= (2 * (i + 1) - 1) * this.labelItemValue)
        }
      }
    },
    /*
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      if (this.direction === 'vertical') {
        ...
        this.labelItemValue = (this.$refs.label.clientWidth / this.label.length) / 2;
        this.getActiveLabel()
      }
      this.getStaticData()
...
.uiSlider.vue-slider-vertical.ui-has-piecewise .vue-slider {
  width: 6px;
}

.uiSlider.vue-slider-vertical.ui-has-piecewise .ui-dot {
  left: -19px;
}
...
.uiSlider.vue-slider-vertical.ui-has-piecewise .vue-slider::after {
  display: none;
}

.uiSlider.vue-slider-vertical.ui-has-piecewise .vue-slider::before {
  display: none;
}
...
.uiSlider.vue-slider-horizontal .ui-piecewise-label-top {
  top: -40px;
}
...
.timeLine .ui-label li.activeLabel::after {
  background-color: #5caefd;
}

.timeLine .ui-label.label3 {
  left: 13%;
}

.timeLine .ui-label.label3::after {
  left: 83%;
}

.timeLine .ui-label.label4 {
  left: 10%;
}

.timeLine .ui-label.label4::after {}

.timeLine .ui-label.label5 {
  left: 6%;
}

.timeLine .ui-label.label5::after {
  left: 89%;
}
