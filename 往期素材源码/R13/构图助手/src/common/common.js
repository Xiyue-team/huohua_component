...
let contentW = window.innerWidth * 0.25;
window.resolution = new THREE.Vector2(window.innerWidth - contentW, window.innerHeight - 144);
var createLineMesh = (vertices, color = '#000', style = 3, width = 1, opacity = 1) => {
...
var createStraightLine = (arr, { style = 3, width = 2, color = '#000', opacity = 1 } = {}) => {
  let vertices = [];
...
var createText = (texts, x, y, z, { color = '#000', size = 48, sub = false, sqrt = false, isItalic = true } = {}) => {
...
let axis = null;
var createCircle = (radius, { color = '#000', start = 0, end = Math.PI * 2, opacity = 1, segments = 36, isLay = false,
  position = [0, 0, 0], depthTest = false } = {}) => {
...
var createSphere = (radius, { phiStart = 0, phiLength = Math.PI * 2, thetaStart = 0, thetaLength = Math.PI, color = '#000',
  opacity = 1, segments = 36, depthTest = true, transparent = false } = {}) => {
  var geometry = new THREE.SphereBufferGeometry(radius, segments, segments, phiStart, phiLength, thetaStart,
    thetaLength);
  var material = new THREE.MeshBasicMaterial({ color, transparent, opacity, depthTest, });
  var sphere = new THREE.Mesh(geometry, material);
  return sphere;
}
var createBox = (width, height, depth, { color = '#000', opacity = 1, segments = 1, depthTest = false } = {}) => {
...
var getLong = (startPos, endPos, group, obj, { style = 3, line_width = 3, color = '#000', step = 0.01, isShrink = false,
  isReset = false } = {}) => {
  return new Promise((resolve, reject) => {
    if (isReset) {
      // reject();
