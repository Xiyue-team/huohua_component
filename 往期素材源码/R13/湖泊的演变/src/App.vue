        ...
        <div class="container">
            ...
            <div class="ViewSpace">
                <div :style="[zoom]" class="viewBox">
                    <div class="imgWrap" id="canvas" ref='canvas'>
                        <!-- </div> -->
                        <img :src="'static/image/'+num+'.jpg'" draggable="false">
                        <img :src="'static/image/'+num+'.jpg'" draggable="false">
                    </div>
                </div>
            </div>
            <div class="footerTrue">
                ...
                <div class="footer">
                    <ui-slider id='slider' class="footerSlider" :min="0" :max="165" :title="false" :box="false"
    ...
    export default {
        ...
        data() {
            return {
                ...
                num: 0,
                timer: null,
                changePlayImg: "static/UI/play@2x.png",
                sliderPoint: [0, 35, 66, 99, 132, 165],
                index: 0,
                f: "",
                store: "",
                mark: 0,
        ...
        mounted() {
            ...
            this.resize();
            this.getViewSize();
            $('#slider').append('<div id="sliderP"><span id="s1"></span><span id="s2"></span><span id="s3"></span><span id="s4"></span><span id="s5"></span><span id="s6"></span></div>');
        ...
        methods: {
            control() {
                this.ele = document.querySelector(".ViewSpace");
                Transform(this.ele);
            ...
            setSliderPonint() {
                ...
                $('#sliderP span').each(function () {
                    $(this).index();
                    $(this).css("left", sliderW * vm.sliderPoint[$(this).index()] / 165 - 7)
                });
                $('.ui-label li').each(function () {
                    $(this).index();
                    $(this).css("left", sliderW * vm.sliderPoint[$(this).index()] / 165 + 22)
                })
            ...
            played() {
                let thiz = this;
                if (!this.isPlayed) {
                    this.changePlayImg = 'static/UI/pause@2x.png';
                    if (thiz.value === 165) {
                        thiz.value = 1;
                        thiz.index = -1;
                    }
                    let an = () => {
                        if (thiz.value === 165) {
                            thiz.isPlayed = false;
                            thiz.index = -1;
                            thiz.changePlayImg = 'static/UI/play@2x.png';
                            clearTimeout(thiz.timer);
                            return;
                        }
                        thiz.value++;
                        thiz.timer = setTimeout(an, 100);
                    };
                    an();

                } else {
                    clearTimeout(thiz.timer);
                    this.changePlayImg = 'static/UI/play@2x.png'
                }
