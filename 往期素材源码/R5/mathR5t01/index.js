...
// -----------1.标题：直线的倾斜角；
// 2.视图：
//---------- ①平面直角坐标系：x轴范围[-10.10]，y轴范围[-10.10]，刻度为1个单位；
// ②直线𝑦=𝑥，标记倾斜角𝛼（夹角𝛼是直线在𝑥轴上方部分和𝑥轴正方向的夹角，标记的是𝛼的数值）；
// 3.控件：
// -------①滑动条（名称是倾斜角𝛼）；
//-------- ②重置按钮。
// 二、功能描述
// 1.滑动条：滑动的最小单位为1°，滑动条每向左（右）拖动1°，视图变化：
// ①设移动后的倾斜角为𝛼，直线方程变为当前倾斜角所表示的直线，即y=kx(k=tanα)；
// ②当α=180°，直线方程为y=0。
// 2.重置按钮：可用重置按钮将视图及控件恢复到初始状态。
...
var threeDimension = {
    ...
    createAxis:function(){
        ...
        threeDimension.scene.add( threeDimension.axis,lineL1);
    },
    //坐标轴分度线
    ...
    createObj:function(){
        if(threeDimension.Obj!=null){
            threeDimension.scene.remove(threeDimension.Obj,l1);
        }
        ...
        var value=Math.tan(j1/180*Math.PI);

        // var sb="y="+value.toFixed(2)+"x";
        // if(j1==45){
        //     sb="y=x";
        // }else if(j1==135){
        //     sb="y=-x";
        ...
        // l1=new THREE.Group();
        // if(j1<90){
        //     text1=threeDimension.createText(sb,x1+20,y1+30,0,'#6D6EFF',24);
        // }else{
        //     text1=threeDimension.createText(sb,x1-25,y1+30,0,'#6D6EFF',24);
        // }
        // l1.add(text1);
        ...
        if(j1!=0){
            ...
            threeDimension.Obj.add(ang1,ang1P);
         }    
        textj1=threeDimension.createText(j1+"°",40*Math.cos(Math.PI/180*j1/2)-2+10,40*Math.sin(Math.PI/180*j1/2)+13,4,'#6D6EFF',24);
        threeDimension.Obj.add(textj1);

        threeDimension.Obj.add(P1,lineC1);
        threeDimension.scene.add(threeDimension.Obj,l1);
    },
