...
var createStraightLine = (arr, { style = 3, width = 3, color = '#000', opacity = 1 } = {}) => {
  let vertices = [];
...
var createSphere = (radius, { phiStart = 0, phiLength = Math.PI * 2, thetaStart = 0, thetaLength = Math.PI, color = '#000', opacity = 1, segments = 36, depthTest = false, transparent = false } = {}) => {
  var geometry = new THREE.SphereBufferGeometry(radius, segments, segments, phiStart, phiLength, thetaStart, thetaLength);
  var material = new THREE.MeshPhongMaterial({ color, transparent, opacity, depthTest, });
...
var getLong = (startPos, endPos, group, obj, { style = 3, line_width = 3, color = '#000', step = 0.01, isShrink = false, isReset = false } = {}) => {
  return new Promise((resolve, reject) => {
    if (isReset) {
      // reject();
      return;
