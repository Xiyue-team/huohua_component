<template>
    <div id="app">
        ...
        <div id="main_right" style="">
            ...
            <ui-slider ref="test" class="footerSlider"  :zoom="zoomF" :value="3" :speed="0" :boxWidth="300" :label="['3','50']" :boxHeight="0" :title="'分割线n'" :min="3" :max="50" :dotTxt="'N'" :infinity="true" v-model='value'>
            </ui-slider>
            <div id='check'>
                <div>组合</div>
                <span>
...
<script>
    import uiSlider from './UI/uiSlider';//滑条
export default {
    components: {
        uiSlider
    },
    data() {
        return {
            isMob: /(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/g.test(navigator.userAgent),
            canvas: null,
            checked: false,
            sceneChecked: false,
            zoomF: 1,
    ...
    methods: {
        ...
        createScene(engine) {
            ...
            // if (thiz.isMob && window.innerWidth < 800) {
            //     var circle = createcircle(r, 360, 0, new BABYLON.Vector3(-100, 70, 0), linecolor);
            // }else{
            //     var circle = createcircle(r, 360, 0, new BABYLON.Vector3(-dishalf, 0, 0), linecolor);
            // }
            // onValueChange(slidervalue);

            thiz.onValueChange = function(value) {

                ground.dispose();
                slidervalue = Math.floor(value);
                var line = createline(r, 360 / slidervalue, 0, new BABYLON.Vector3(-dishalf, 0, 0), linecolor);
                // if (thiz.isMob && window.innerWidth < 800) {
                //     var line = createline(r, 360 / slidervalue, 0, new BABYLON.Vector3(-100, 70, 0), linecolor);
                // }else{
                //     var line = createline(r, 360 / slidervalue, 0, new BABYLON.Vector3(-dishalf, 0, 0), linecolor);
                // }
                var i;
            ...
            thiz.onValueChange(slidervalue);
            var firsttouch = true;
            function combination() {
                ...
                posx = 0;
                // if (thiz.isMob && window.innerWidth < 800) {
                //     groundright.position = new BABYLON.Vector3(0, 60, 0);
                // }else{
                //     groundright.position = new BABYLON.Vector3(0, 0, 0);
                // }
