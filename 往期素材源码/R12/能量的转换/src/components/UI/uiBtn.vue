<template>
  <div class="UI-btn noselect" :class="classObj" :style="{width:width+'px',height:height+'px'}" :value="value" @click="clickEvent">
    <!--重置按钮-->
    ...
    <!--zoomIn--> <!--zoomOut-->
    <img :src="zoomSrc" v-if="type==='zoom'" alt="">
    <!--单选框-->
...
<script>
  export default{
    props: {
      ...
      vertical: { type: Boolean,default:false}//用v-model设置是否选中
    },
    components: {},
    created(){},
    watch:{
      ...
      value(val){
        //如果是播放按钮，则点击改变图片
        if(this.type==='play'){
          if(val){this.playSrc = 'static/UI/Combined Shape@2x.png';
          }else{this.playSrc = 'static/UI/play icon@2x.png'}
        }else if(this.type==='zoom'){
          if(val){this.zoomSrc = 'static/UI/Group 8.png';
          }else{this.zoomSrc = 'static/UI/Group 81.png'}
        }
      },
    },
    computed:{
      ...
      classObj(){
        return {
          ...
          'UI-circle': this.type==='play' || this.type==='zoom',
    ...
    data(){
      return {
        playSrc:'static/UI/Combined Shape@2x.png',
        zoomSrc:'static/UI/Group 8.png',
      }
    },
    methods: {
      //点击事件
...
<style>
  ...
  .UI-btn.btn-vertical p{
    float: none;
    line-height: 1;
  }
  .UI-btn.btn-vertical span{
    float: none;
    display: inline-block;
    margin-top: 18px;
  }
