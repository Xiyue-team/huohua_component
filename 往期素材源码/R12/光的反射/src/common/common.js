...
var createStraightLine = (arr, { style = 3, width = 1, color = '#000', opacity = 1,depthTest = true } = {}) => {
  let vertices = [];
...
var createText = (texts, x, y, z, color = '#000', size = 100) => {
...
var createCylinder = (top, bottom, height, color) => {
  var geometry = new THREE.CylinderBufferGeometry(top, bottom, height, 16);
...
var getLong = (startPos, endPos, group, angle, linestep, { style = 3, line_width = 3, color = '#000', step = 0.01 } = {}) => {
  return new Promise((resolve, reject) => {
    ...
    function move() {
      ...
      group.timer = setTimeout(move,40);
    }
    move();
  })
}
