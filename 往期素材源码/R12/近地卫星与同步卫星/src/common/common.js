...
var createLineMesh = (vertices, color = '#000', style = 3, width = 2, opacity = 1) => {
...
//画圆线
var drawCircleLine = (radius, { style = 3, color = '#000', line_width = 1, isLay = false, position = [0, 0, 0] } = {}) => {
  let x, y, vertices = [];
  ...
  let line = createLineMesh(vertices, color, style, line_width);
...
let axis = null;
var createPlane = (radius) => {
  var geometry = new THREE.PlaneGeometry(radius, radius, 6);
...
var createSphere = (radius, { end = Math.PI * 2, color = '#000', opacity = 1, segments = 32, depthTest = false } = {}) => {
...
var createBox = (w, h, d, {color="#0f0",opacity=1,depthTest=false} = {}) => {
