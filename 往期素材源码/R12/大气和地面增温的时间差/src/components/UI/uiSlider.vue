<template>
    <div ref="wrap" :class="['uiSlider',isBox, flowDirection,  { 'ui-has-piecewise': piecewiseLabel,timeLine:timeLine,'noBlueProcess':noBlueProcess?noBlueProcess:noBlue }]" :style="wrapStyles" @click="wrapClick">
        ...
        <!--滑条上的文字-->
        <ul v-if="!timeLine && label" ref="label" :style="{width:infinity?'53%':'',marginLeft:dotSize/2+'px'}" class="ui-label">
            <li v-for="(la,index) in label" v-text="la" :style="{width:100/label.length+'%'}"></li>
        </ul>
        <!--滑块-->
...
export default {
    ...
    props: {
        ...
        //是否有标题,且标题内容是
        title: { type: [Boolean, String], default: 't/h' },
        //副标题
...
.uiSlider .ui-label {
    ...
    margin: 0 auto;
    display: table;
    margin-left: 14.957px !important;
}
