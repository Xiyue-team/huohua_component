<template>
  <div class="UI-btn" :class="classObj" :value="value" @click="clickEvent">
    ...
    <div :class="{checked:value}" class="circle_span" v-if="this.type==='switch'">
      <div class="circle_em" :class="{circle_em_right:value}"></div>
    </div>
  </div>
</template>
...
<style>
  ...
  .btn-switch .circle_span .circle_em {
    width: 20px;
    height: 20px;
    background-color: #ffffff;
  ...
  .btn-switch .circle_span.checked {
    background-color: #5caefd;
  }

  .btn-switch .circle_span.checked .circle_em_right {
    float: right;
    margin-top: 3px;
    margin-left: 0;
    margin-right: 3px;
