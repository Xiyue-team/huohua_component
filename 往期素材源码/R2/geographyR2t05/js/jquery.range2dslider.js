...
!function($){	
	...
	!function($){
		 var drag = false,
			...
			pointerEventToXY = function( e ) {
				...
				if( e.type == 'touchstart' || e.type == 'touchmove' || e.type == 'touchend' || e.type == 'touchcancel' ) {
					...
					out.x = touch.clientX/zoom;
					out.y = touch.clientY/zoom;
				}else if (e.type == 'mousedown' || e.type == 'mouseup' || e.type == 'mousemove' || e.type == 'mouseover'|| e.type=='mouseout' || e.type=='mouseenter' || e.type=='mouseleave') {
					out.x = e.clientX/zoom;
					out.y = e.clientY/zoom;
				}
				return out;
			},
		...
		$.fn.xdSoftDraggable = function( _options ){
			...
			if( !setGlobalHandler ){
				document.body.addEventListener('touchmove', xdSoftDraggableDrag);
				document.body.addEventListener('touchend', xdSoftDraggableEnd);
				window.addEventListener('touchend', xdSoftDraggableEnd);
				document.body.addEventListener('mousemove', xdSoftDraggableDrag);
				document.body.addEventListener('mouseup', xdSoftDraggableEnd);
				window.addEventListener('mouseup', xdSoftDraggableEnd);
			...
			return this.each(function(){
				...
				if( this.className.search('xdsoft_draggable')==-1 ){
					this.addEventListener('touchstart', xdSoftDraggableStart);
					this.addEventListener('mousedown', xdSoftDraggableStart);
				}
			});
		...
		if( typeof(_options)=='string' ){
		 ...
		 return this.each(function(){
			...
			for(i=0;i<_this.values.length;i++){
				if( !_this.$runners[i] ){
                    _this.$runners.push($runner = $('<div class="xdsoft_range2dslider_runner xdsoft_range2dslider_runner'+i+'"><div style="position:absolute;top:-24px;left:-24px;width:80px;height:80px;background:transparent;opacity:0.5;cursor:pointer;"></div></div>'));
			...
			_this.$sliderBox
				...
				.append(_this.$runners).append(_this.$sliderLeft);
