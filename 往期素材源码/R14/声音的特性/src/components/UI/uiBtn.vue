<template>
  <div class="UI-btn" :class="classObj" :value="value" @click="clickEvent">
    ...
    <!--单选框-->
    <span :class="{checked:value}" v-if="this.type==='radio'">
      <em></em>
    </span>
    <p v-if="this.type==='radio'">
      <slot></slot>
    </p>
    <!--多选框-->
    <span :class="{checked:value}" v-if="this.type==='checkbox'">
      <img src="static/UI/nike@2x.png" /> </span>
    <p v-if="this.type==='checkbox'">
      <slot></slot>
    </p>
    <!--switch-->
    <p v-if="this.type==='switch'">
      <slot></slot>
    </p>
    <span :class="{checked:value}" v-if="this.type==='switch'">
      <em></em>
    </span>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    value(val) {
      ...
      if (this.type === 'play') {
        if (val) {
          this.playSrc = 'static/UI/play icon@2x.png';
        } else {
          this.playSrc = 'static/UI/Combined Shape@2x.png'
        }
      }
    }
  },
