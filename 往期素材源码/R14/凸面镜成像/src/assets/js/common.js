...
let resolution = () => {
    ...
    if (w < 500 || h < 500) {
        ...
        return new THREE.Vector2(window.innerWidth - 280, window.innerHeight);
    }
}
...
let axis = null;
let createDiyShape = (vector, { color = '#F7881E' } = {}) => {
    let shape = new THREE.Shape();
    shape.moveTo(vector[0].x, vector[0].y)
    for (let i = 1; i < vector.length; i += 1) {
        shape.lineTo(vector[i].x, vector[i].y)
    }
    shape.lineTo(vector[0].x, vector[0].y)
...
let unitLine = ({ width = 1, color = '#F7881E', style = 3 } = {}) => {
    let line = createStraightLine([
        [1, 0, 0],
        [-1, 0, 0]
    ], {style, width, color});
...
let scaleLine = (startPos, endPos, line, { len = 0 } = {}) => {
    let [x, y, z] = startPos;
    let [x1, y1, z1] = endPos;
    // 获取线段长度
    if (!len) {
