<template>
	<div id="app" class="noselect">
		...
		<div class="app_aside">
			...
			<div class="center">
				<div class="sidle11">
					<ui-slider :class="{noActive:f6}"  @drag-end="dragEnd(4)" :label="['-3','3']" :min="-3" :max="3" :title="'振幅'"
					 :formatter="phi3" class="slider4" :speed="0" :noBlueProcess="true" v-model="zhenfu1"></ui-slider>
					<ui-slider :class="{noActive:f1}" @drag-end="dragEnd(1)" :label="['-π','π']" :min="-12" :max="12" :title="'平移'"
					 :formatter="phi" class="slider" :speed="0" :noBlueProcess="true" v-model="value"></ui-slider>
					...
					<div class="updown" :class="{noActive:f2}">
						...
						<div style="font-size: 14px;font-weight: 500;line-height: 1.0;color: #999999;padding: 0 22px;height: 14px">
							<span style="float: left;display: block">缩</span>
							<span style="float: right;display: block">伸</span>
						</div>
						<div style="padding: 0 12px;height:50px;margin-top: 2px">
							<div class="pointDiv"><span class="pointS">
									<p style="border-bottom: 1px solid #999999;">1</p>
									<p>4</p>
								</span></div>
							<div class="pointDiv"><span class="pointS">
									<p style="border-bottom: 1px solid #999999;">1</p>
							...
							<div class="pointDiv"><span class="pointS">
									<p style="margin-top: 25px">1</p>
								</span></div>
							<div class="pointDiv"><span class="pointS">
									<p style="margin-top: 25px">2</p>
								</span></div>
							<div class="pointDiv"><span class="pointS">
									<p style="margin-top: 25px">3</p>
								</span></div>
							<div class="pointDiv"><span class="pointS">
									<p style="margin-top: 25px">4</p>
								</span></div>
						</div>
					</div>
				</div>
				...
				<div class="sidle22">
					<ui-slider :class="{noActive:f5}"  @drag-end="dragEnd(3)" :label="['-3','3']" :min="-3" :max="3" :title="'振幅'"
					 :formatter="phi2" class="slider3" :speed="0" :noBlueProcess="true" v-model="zhenfu"></ui-slider>
					...
					<div class="updown" :class="{noActive:f3}" style="margin-top: 0;">
						...
						<div style="padding: 0 12px;height:50px;margin-top: 2px">
							<div class="pointDiv1"><span class="pointS1">
									<p style="border-bottom: 1px solid #999999;">1</p>
									<p>4</p>
								</span></div>
							<div class="pointDiv1"><span class="pointS1">
					...
					<div :style="'width:100%;height:1px;background: rgba(0,0,0,0.06)'"></div>
					<ui-slider :class="{noActive:f4}" @drag-end="dragEnd(2)" :label="['-π','π']" :min="-12" :max="12" :title="'平移'"
					 :formatter="phi1" class="slider2" :speed="0" :noBlueProcess="true" v-model="value2"></ui-slider>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.TO = this.init();
			$('.slider').append(
				'<div class="sliderP"><span class="s1"></span><span class="s2"><p>0</p></span><span class="s3"></span><div class="blueLine"></div></div>'
			);

			$('.slider2').append(
				'<div class="sliderP"><span class="s1"></span><span class="s2"><p>0</p></span><span class="s3"></span><div class="blueLine1"></div></div>'
			);

			$('.slider3').append(
				'<div class="sliderP"><span class="s1"></span><span class="s2"><p>0</p></span><span class="s3"></span><div class="blueLine2"></div></div>'
			);

			$('.slider4').append(
				'<div class="sliderP"><span class="s1"></span><span class="s2"><p>0</p></span><span class="s3"></span><div class="blueLine3"></div></div>'
			);
		...
		computed: {
			...
			phi2() {
				return this.zhenfu >= 0 ? this.phi2Arr[Math.abs(this.zhenfu)] : ("-" + this.phi2Arr[Math.abs(this.zhenfu)]);
			},
			phi3() {
				return this.zhenfu1 >= 0 ? this.phi2Arr[Math.abs(this.zhenfu1)] : ("-" + this.phi2Arr[Math.abs(this.zhenfu1)]);
			},
		},
		watch: {
			value(newvalue) {
			...
			zhenfu(newvalue) {
				...
				if (newvalue == 0 && this.zhenfu == 1) {
					this.TO.dashLine1Hide();
				} else {
					this.TO.dashLine1Show();
				}
				this.countWidth2(newvalue);
				this.TO.createObj4();
			},
			zhenfu1(newvalue) {
				console.log(newvalue);
				if (newvalue == 0 && this.zhenfu1 == 1) {
					this.TO.dashLine1Hide();
				} else {
					this.TO.dashLine1Show();
				}
				this.countWidth3(newvalue);
				this.TO.createObj3();
			}
		},
		methods: {
          ...
          Linedash11 = null,
				...
				var pointClick = function() {
					...
					if (index == 3 && thiz.value == 0) {
						...
						dashLine.visible = false;
						scene.remove(Linedash0);
						Linedash = createLineMesh(Line.geometry.vertices, '#000', 2, 2);
						Linedash.position.set(50,220,0);
						scene.add(Linedash);
					}
				...
				var lineClick = function() {
					Linedash0 = createLineMesh(Line.geometry.vertices, '#000', 2, 2);
						Linedash0.position.set(50,220,0);
						scene.add(Linedash0);
				}
				...
				var pointClick1 = function() {
					...
					if (index == 3 && thiz.value2 == 0) {
						...
						Linedash11 = createLineMesh(Line1.geometry.vertices, '#000', 2, 2);
						Linedash11.position.set(50,-220,0);
						scene.add(Linedash11);
					}
				...
				var lineClick1 = function() {
					scene.remove(dashLine1);
					scene.remove(Linedash11);
					Linedash1 = createLineMesh(Line1.geometry.vertices, '#000', 2, 2);
						Linedash1.position.set(50,-220,0);
						scene.add(Linedash1);
				}
				...
				if (isMob) {
					...
					$('.slider2').on('touchstart', lineClick1);
					$('.slider').on('touchstart', lineClick);
					stepNumber = 1;
				} else {
					$('.pointDiv').on('mousedown', pointClick);
					$('.pointDiv1').on('mousedown', pointClick1);
					$('.slider2').on('mousedown', lineClick1);
					$('.slider').on('mousedown', lineClick);
				...
				var createObj = () => {
					let vertices = [],
						dy;
					for (let i = -400; i < 401; i += 5) {
						dy = this.zf * 100 * sin(i / 160 * PI);
				...
				var createObj1 = (line, x1, x2, num) => {
					...
					if (num == 2) {
						for (let i = -400; i < 401; i += 3) {
							dy = this.zf * 100 * sin(x1 * (i + x2) / 160 * PI);
							dx = i;
          ...
          //虚线
            ...
            //实线
				...
				var resetWidget = () => {
					...
					dashLine.visible = false;
					dashLine1.visible = false;	
				  scene.remove(Linedash);
				  scene.remove(Linedash1);
				  scene.remove(Linedash11);
...
<style>
	...
	*:after {
	...
	.slider2>div.sliderP>span,
	.slider3>div.sliderP>span,
	.slider4>div.sliderP>span {
