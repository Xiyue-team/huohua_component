    ...
    methods: {
      init(){
        ...
        var createObj = () => {
          ...
          var textM = createText("M",this.point['m'][0]>0?20:-20,this.point['m'][1]+16,0,"#6D6EFF",24)
          ...
          vertices.push(vec3(this.point["a"][0]*40,this.point["a"][1]*40,1));
          vertices.push(vec3(this.point["m1"][0]*40,this.point["m1"][1]*40,1));
          var lineAM1 = createLineMesh(vertices,"#000",3,2);

          //画BM1
          vertices = [];
          vertices.push(vec3(this.point["m1"][0]*40,this.point["m1"][1]*40,1));
          vertices.push(vec3(this.point["b"][0]*40,this.point["b"][1]*40,1));
          var lineBM1 = createLineMesh(vertices,"#000",3,2);
          //P,R,S
          var textP = createText("P",this.p>0?-this.p/2*40-20:-this.p/2*40+20,this.point['p'][1]*40+20,0,"#6D6EFF",24);
          // var textR = createText("R",this.p>0?-this.p/2*40-20:-this.p/2*40+20,this.point['r'][1]*40+14,0,"#6D6EFF",24);
          var textS = createText("Q",this.p>0?-this.p/2*40-20:-this.p/2*40+20,this.point['s'][1]*40+20,0,"#6D6EFF",24);
          ...
          //画AM1和BM1之间的垂角
          vertices = [];
          vertices.push(vec3(0,-15,1));
          vertices.push(vec3(15,-15,1));
          vertices.push(vec3(15,0,1));
          var angle = createLineMesh(vertices,"#000",3,2);
          var k = (this.point['m1'][1]-this.point['b'][1])/(this.point['m1'][0]-this.point['b'][0]);
          angle.position.set(this.point['m1'][0]*40,this.point['m1'][1]*40,1);
          
          if(this.point['m1'][0]<0){
            angle.rotation.z = (this.point['a'][1]>0?90/180*Math.PI:0) +  Math.atan(k);
          } else {
            angle.rotation.z = (this.point['a'][1]>0?180/180*Math.PI:-90/180*Math.PI) +  Math.atan(k);
          }
           
          if(this.point['a'][0] != 0) {
            obj1.add(groupM,groupM1,lineMM1,angle,lineBS,lineBM1,lineAM1,textS,textP,angle1,angle2);
          }
          obj1.add(lineAP)
          if(this.checked) {
            scene.add(obj1);
