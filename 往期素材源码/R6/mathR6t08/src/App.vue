  ...
  export default {
    ...
    methods: {
      init(){
        ...
        var textA = createText("A",this.point['a'][0]>0?35:-35,this.point['a'][1]>0?10:20,0,"#6D6EFF",24);
        if(!touchA){
        ...
        var textB = createText("B",this.point['b'][0]>0?20:-20,this.point['b'][1]>0?20:20,0,"#6D6EFF",24);
        ...
        var textQ = createText("Q",this.p>0?-this.p/2*40-20:-this.p/2*40+20,this.point['q'][1]*40+14,0,"#6D6EFF",24);
        ...
        vertices = [];
        vertices.push(vec3(this.point["q"][0]*40,this.point["q"][1]*40>0?this.point["q"][1]*40-10:this.point["q"][1]*40+10,1));
        vertices.push(vec3(this.point["q"][0]*40>0?this.point["q"][0]*40-10:this.point["q"][0]*40+10,this.point["q"][1]*40>0?this.point["q"][1]*40-10:this.point["q"][1]*40+10,1));
        vertices.push(vec3(this.point["q"][0]*40>0?this.point["q"][0]*40-10:this.point["q"][0]*40+10,this.point["q"][1]*40,1));
        var angle2 = createLineMesh(vertices,"#D32121",3);
       

        obj2.add(lineAF,groupA,groupF,groupB,lineBF,lineAP,textP,textQ,lineBQ,angle2,angle1);
        scene.add(obj2);
        //显示三点共线
        ...
        //计算AQ斜率，做延伸线
        var k1,x1,y1,x2,y2;
        k1 = (this.point['a'][1]-this.point['q'][1])/(this.point['a'][0]-this.point['q'][0]);
        x1 = this.point['a'][0]>0?this.point['a'][0]+1:this.point['a'][0]-1;
        y1 = k1*(x1-this.point['a'][0])+this.point['a'][1];
        x2 = this.point['q'][0]>0?this.point['q'][0]+1:this.point['q'][0]-1;
        y2 = k1*(x2-this.point['q'][0])+this.point['q'][1];
        vertices = [];
        vertices.push(vec3(x1*40,y1*40,1));
        vertices.push(vec3(x2*40,y2*40,1));
        var lineAQ = createLineMesh(vertices,"#000",3,2);
        var textN = createText("n",x1>0?x1*40+20:x1*40-20,y1>0?y1*40+20:y1*40-20,0,"#6D6EFF",24);
        //计算BP斜率，做延伸线
        var k2,x3,y3,x4,y4;
        k2 = (this.point['b'][1]-this.point['p'][1])/(this.point['b'][0]-this.point['p'][0]);
        x3 = this.point['b'][0]>0?this.point['b'][0]+1:this.point['b'][0]-1;
        y3 = k2*(x3-this.point['b'][0])+this.point['b'][1];
        x4 = this.point['p'][0]>0?this.point['p'][0]+1:this.point['p'][0]-1;
        y4 = k2*(x4-this.point['p'][0])+this.point['p'][1];
        vertices = [];
        vertices.push(vec3(x3*40,y3*40,1));
        vertices.push(vec3(x4*40,y4*40,1));
        var lineBP = createLineMesh(vertices,"#000",3,2);
        var textM = createText("m",x4>0?x4*40+20:x4*40-20,y4>0?y4*40+20:y4*40-20,0,"#6D6EFF",24);
        if(this.point['a'][1]!=0){
          obj1.add(lineAQ,lineBP,textM,textN);          
        }
