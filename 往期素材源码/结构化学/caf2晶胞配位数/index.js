...
function modelPut(obj,mtl,O,scale,callback) {
    ...
    mtlLoader.load( mtl, function( materials ) {
        ...
        for(var i in materials.materials){
            if(materials.materials[i].name=='13___Default'){
                materials.materials[i]=new THREE.MeshPhongMaterial({color:'#A3BBF3'});
            }else if(materials.materials[i].name=='14___Default'){
                materials.materials[i]=new THREE.MeshPhongMaterial({color:'#FFC7AD'});
            }else{
...
modelPut('caf2.obj','caf2.mtl',model1,1,function (O) {
    scene.add(O);
    O.position.y = 0;
});
...
function addC() {
    ...
    if(flag == 0){
        modelPut('caf2.obj','caf2.mtl',model2,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500){
            model1.position.set(-37,-10,0);
            model2.position.set(37,-10,0);
        }
        else {
            model1.position.set(-37,50,0);
            model2.position.set(37,50,0);
        }

        flag++;
    }else if(flag == 1){
        modelPut('caf2.obj','caf2.mtl',model3,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500){
            model1.position.set(-37,-10,0);
            model2.position.set(37,-10,0);
            model3.position.set(-37,64,0);
        }
        else{
            model1.position.set(-37,12,0);
            model2.position.set(37,12,0);
            model3.position.set(-37,86,0);
        }

        flag++;
    }else {
        modelPut('caf2.obj','caf2.mtl',model4,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500){
            model4.position.set(37,64,0);
        }
        else{
            model4.position.set(37,86,0);
        }
