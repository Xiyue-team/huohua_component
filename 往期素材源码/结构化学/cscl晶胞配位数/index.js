...
const ball=C('0xff0000',10,0,0,0);
scene.add(ball);
...
function modelPut(obj,mtl,O,scale,callback) {
    ...
    mtlLoader.load( mtl, function( materials ) {
        ...
        for(var i in materials.materials){
            if(materials.materials[i].name=='19___Default'){
                materials.materials[i]=new THREE.MeshPhongMaterial({color:'#1F741C'});
            }else if(materials.materials[i].name=='20___Default'){
                materials.materials[i]=new THREE.MeshPhongMaterial({color:'#96DED8'});
            }else{
...
modelPut('cscl.obj','cscl.mtl',model1,1,function (O) {
    scene.add(O);
    if(window.innerWidth<500||window.innerHeight<500) {
        O.position.y = 0;
    }
...
function addC() {
    ...
    if(flag == 0){
        modelPut('cscl.obj','cscl.mtl',model2,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500) {
            model1.position.set(-10, -10, 0);
            model2.position.set(40, -10, 0);
        }
        else{
            model1.position.set(-25, 10, 0);
            model2.position.set(25, 10, 0);
        }
        flag++;
    }else if(flag == 1){
        modelPut('cscl.obj','cscl.mtl',model3,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500) {
            model1.position.set(-10, -10, 0);
            model2.position.set(40, -10, 0);
            model3.position.set(-10, 40, 0);
        }
        else{
            model1.position.set(-25, 10, 0);
            model2.position.set(25, 10, 0);
            model3.position.set(-25, 60, 0);
        }
        flag++;
    }else {
        modelPut('cscl.obj','cscl.mtl',model4,1,function (O) {
            scene.add(O)
        });
        if(window.innerWidth<500||window.innerHeight<500) {
            model4.position.set(40, 40, 0);
        }
        else {
            model4.position.set(25, 60, 0);
        }
    }
}
