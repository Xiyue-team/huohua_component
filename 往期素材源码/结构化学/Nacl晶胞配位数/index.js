...
function init() {
    ...
    camera.position.set(160, -100, 460);
    // camera.zoom = 1;
...
function modelPut(obj, mtl, O, scale, callback) {
    ...
    mtlLoader.load(mtl, function (materials) {
        ...
        for (var i in materials.materials) {
            if (materials.materials[i].name == '02___Default') {
                materials.materials[i] = new THREE.MeshPhongMaterial({color: '#ECE02A'});
            } else if (materials.materials[i].name == '01___Default') {
                materials.materials[i] = new THREE.MeshPhongMaterial({color: '#1F741C'});
            } else {
...
modelPut('nacl_jingti.obj', 'nacl_jingti.mtl', model1, 1, function (O) {
    scene.add(O)
});
...
function step() {
    ...
    if (index == 0) {
        if (checked1) {
            ...
            model1.children[0].children[0].visible = false;
            checked1 = false;
        } else {
            $(this).children('span').css('background', '#5CAEFD').children().css({'right': '2px', 'left': 'auto'});
            model1.children[0].children[0].visible = true;
            checked1 = true;
        }
    } else {
        if (checked2) {
...
function reset() {
    ...
    checked1 = false;
    checked2 = false;
    camera.position.set(160, 120, 460);
    camera.zoom = 1;
    camera.updateProjectionMatrix();
}
...
function addC() {
    ...
    if (flag == 0) {
        modelPut('nacl_jingti.obj', 'nacl_jingti.mtl', model2, 1, function (O) {
            scene.add(O);

            if (window.innerWidth < 500 || window.innerHeight < 500) {
                O.position.set(37, 6, 0);
            }
            else {
                O.position.set(37, 50, 0);
            }

        });
        if (window.innerWidth < 500 || window.innerHeight < 500) {
            model1.position.set(-37, 6, 0);
            model2.position.set(37, 6, 0);
        }
        else {
            model1.position.set(-37, 50, 0);
        }

        flag++;
    } else if (flag == 1) {
        modelPut('nacl_jingti.obj', 'nacl_jingti.mtl', model3, 1, function (O) {
            scene.add(O)
        });

        if (window.innerWidth < 500 || window.innerHeight < 500) {
            model1.position.set(-37,14, 0);
            model2.position.set(37, 14, 0);
            model3.position.set(-37, 104, 0);
            console.log(model1);
        }
        else {
            model1.position.set(-37, -24, 0);
            model2.position.set(37, -24, 0);
            model3.position.set(-37, 104, 0);
        }
        flag++;
    } else {
        modelPut('nacl_jingti.obj', 'nacl_jingti.mtl', model4, 1, function (O) {
            scene.add(O)
        });
        if (window.innerWidth < 500 || window.innerHeight < 500) {
            model4.position.set(37, 104, 0);
        }
        else {
            model4.position.set(37, 104, 0);
        }
