<template>
  <div id="app" class="noselect">
    ...
    <div class="right_center" :style="'zoom:'+zoomF">
      <ui-slider direction="vertical" :boxWidth="110" :boxHeight="450" :title="'角度α'" :noBlueProcess="true" :realTime="true" :label="['360°','-360°']" :min="-360" :formatter="formatter" :max="360" v-model="value" id='slider' :zoom="zoomF" :speed="0" :processWidth="6"></ui-slider>
  ...
  methods: {
    ...
    init() {
      ...
      var createObj = () => {
        ...
        //圆弧
        Arc = new THREE.Group();
        let arc = createCircle(40, 0, Math.PI / 3, '#fce3d5');
        arc.position.z = -2;
        let circleP2 = createArc(40, 0, 60, '#f17c3a', 3);
        circleP2.position.z = -1;
        Arc.add(arc, circleP2);
        Arc.position.z = 2;
        scene.add(circle, pointP, textP, Arc, textα);
      ...
      let drawLine = (val) => {
        ...
        circleP2.position.z = -1;
        Arc.add(arc, circleP2);
        textα.position.set(60 * Math.cos(val / 180 * Math.PI / 2), 60 * Math.sin(val / 180 * Math.PI / 2) + 13, 0);
        scene.add(Arc);
      };
