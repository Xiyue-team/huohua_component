  ...
  methods: {
    ...
    init() {
      ...
      var initLine = () => {
        ...
        lineObj.bt = common.unitLine({ color: "#0DB2FF", width: 4 });
        lineObj.texta = common.createText('α', 0, 0, 2, '#BD00D4', 24);
        lineObj.textpi = common.createText('π', 0, 0, 2, '#BD00D4', 24);
      ...
      var createObj1 = () => {
        ...
        touchAImg = common.createImg([common.vec3(0, 0, 2)], 128 * 0.9, 64 * 0.9, "static/UI/A@2x.png");
        touchAImg.name = 'A';
      ...
      let dx, dy,pointArr=[]; 
      for (var i = 0; i < 361; i ++) {
          dx = ANGLE_SIZE * Math.cos(Math.PI / 180 * i);
          dy = ANGLE_SIZE * Math.sin(Math.PI / 180 * i);
          pointArr.push(common.vec3(dx, dy,0));
        }
      var drawAngle = (startangle, endangle, size, color, circleColor, index, textObj) => {
       let  vertices = [];
        let obj = new THREE.Object3D();
        vertices = pointArr.slice(startangle,endangle);
      ...
      var drawSubline = (P) => {
        ...
        lineObj.bt = common.scaleLine([RADIUS, y, 1], [RADIUS, 0, 1], lineObj.bt)
        group.add(lineObj.circlet, lineObj.bt);
        lineObj.textm.position.set(P.x - 20, 0, 0);
        lineObj.textm1.position.set(-P.x - 20, 0, 0);
        lineObj.textt.position.set(RADIUS + 20, y, 0);
        group.add(lineObj.textm,lineObj.textm1,lineObj.textt)
        return group;
      }
      //是否显示辅助线
