<template>
  <div id="app" class="noselect">
    <div id="loading" v-if="loadingF"><div>loading...</div></div>
    <div class="container">
      <!--头部-->
  ...
  methods: {
    ...
    init() {
      ...
      let createP=(W,H,color)=>{
        ...
        let material= new THREE.MeshBasicMaterial({color:color,side:THREE.DoubleSide,transparent:true});
      ...
      let Text=createP1(364*0.6,120*0.6,text);
      Text.position.set(-300,80,5);
      scene.add(Text);

      let loader = new THREE.GLTFLoader();
      loader.load( './static/obj/qiang.gltf', (data)=> {
        let object = data.scene.children[0];
        object.rotation.z=Math.PI/2;
        object.position.set(-300,-15,0);
        selectobjs.push(object);
        scene.add( object );
        thiz.loadingF=false;
      ...
      //铝箔
      let Lv=new THREE.Mesh(new THREE.CubeGeometry(1, 200, 200),
        new THREE.MeshPhysicalMaterial({
          map:THREE.ImageUtils.loadTexture('./static/UI/lvban.jpg'),
          transparent:true,
        })
      );
      scene.add(Lv);
      ...
      let FS=(x)=>{
        ...
        DZ[len2].position.set(-300-x,8-Math.random()*16,8-Math.random()*16);
        OBJ.add(DZ[len2]);
