<template>
  <div class="container">
    ...
    <div v-show="loadModelF" id="onClickEvent">
      <div style="background: #fff;box-shadow: 0 0 10px rgba(0,0,0,0.3);color: #333;" v-text="button_text"></div>
    </div>
    <div id="imageShow" :style="'display:'+block+';zoom:'+scale+';'">
      <img :src="img_load">
      <div id="close"><img src="static/image/close.png"></div>
    </div>
    <div>
    </div>
  </div>
  ...
  export default {
    data() {
      return {
        ...
        block: 'none',
        img_load: "static/image/diqiubankuai.jpg",
        scale: 1
      };
    },
    methods: {
      init() {
        ...
        var thiz = this;
        this.scale = window.innerWidth / 2500;
      ...
      createScene(engine) {
        ...
        document.getElementById("renderCanvas").addEventListener("pointerdown", onPointerDown, false);
        scene.onDispose = function () {
          document.getElementById("renderCanvas").removeEventListener("pointerdown", onPointerDown);
        };
        var num = 0;
        ...
        function clickButtonEvent() {
          ...
          if (num == 1) {
            ...
            thiz.img_load = "static/image/diqiuhuoshan.jpg";
            mesh_huoshan.isVisible = true;
          } else if (num == 2) {
            $('#onClickEvent>div').css({backgroundColor: '#fff', color: '#333'});
            thiz.button_text = "世界火山地震带分布";
            thiz.img_load = "static/image/diqiubankuai.jpg";
            materialPlane.diffuseTexture = new BABYLON.Texture("static/image/earth_bankuai.png", scene);
            mesh_huoshan.isVisible = false;
            num = 0;
          }
        }
        ...
        if (isMob) {
          ...
          $('#close').on('touchstart', onCloseClick);
        } else {
