<template>
  <div id="app" class="noselect">
    <div class="container">
      <div class="cl" v-show="isFirst"><h3>loading...</h3></div>
      <!--头部-->
    ...
    <ui-btn type="switch" v-model="switch_checked1" class="aa">
    表达式
    </ui-btn>
    <ui-btn type="switch" v-model="switch_checked">
      三角函数线（基础三种）
    </ui-btn>
    <div class="switch_pic" v-if="switch_checked1">
      <img src="../static/UI/switch.png" alt="">
    </div>
  </div>
</template>
<script>
    ...
    methods: {
      ...
      init() {
        ...
        let createLinePM = (x, y) => {
          ...
          textM = createText('M', x, y >= 0 ? -10 : 40, 0, '#000000', 25);

          textα.position.set(x + 30, y / 2, 0);
        ...
        let createLineOM = (x, y) => {
          ...
          lineOM.position.set(x / 2, 0, 4);

          cosα = createText('cosα', x / 2, y >= 0 ? '-10' : '40', 1, '#000000', 25);
        ...
        lineTA1 = createLineMesh(startVertices, '#0db2ff', 3, 4);
        scene.add(lineTA1);
        let createLineTA = (x, y) => {
          scene.remove(lineTA);
          lineTA = new THREE.Group();

          let ratio = Math.sqrt(Math.pow(300 * y / x, 2));
          lineTA1.scale.set(ratio, ratio, ratio);
          lineTA1.position.set(300, 300 * y / x / 2, 4);
          lineTA1.rotation.z = Math.PI / 2;

          //虚线
          let vertices = [];
          vertices.push(0, 0, 0, 300, 300 * y / x, 0);
          dashLine = createLineMesh(vertices, '#000000', 2, 3);
          if (y < 1 && y > -1) {
          ...
          //T点
          pointT.position.set(300, 300 * y / x, 5);

          let tan = createText('tanα', 335, 300 * y / x / 2, 0, '#000000', 25);

          lineTA.add(text, dashLine, tan);
          if (this.switch_checked) {
            lineTA.visible = true;
            pointT.visible = true;
            lineTA1.visible = true;
          } else {
            lineTA.visible = false;
            pointT.visible = false;
            lineTA1.visible = false;
          }
          scene.add(lineTA);
        };
        ...
        var createObj = () => {
          ...
          let sinr = r * Math.sin(Math.PI / 3), cosr = r * Math.cos(Math.PI / 3), sinR = R * Math.sin(Math.PI / 3),
            cosR = R * Math.cos(Math.PI / 3);
          //可拖动的圆点
          thiz.cover = new THREE.Mesh(new THREE.PlaneBufferGeometry(200,200),new THREE.MeshBasicMaterial({color:'#1FF',transparent:true,opacity:0}));
          thiz.cover.position.set(cosR,sinR,-1)
          touchP = createImg([vec3(cosR, sinR, 6)], 128 * 0.9, 64 * 0.9, 'static/UI/A@2x.png');
          touchP.rotation.z = -30 * Math.PI / 180;
          var loader = new THREE.TextureLoader();
          loader.load("static/UI/A@2x.png",(image)=>{
            console.log(touchP)
            touchP.children[0].material.map = image;
            thiz.isFirst = false;
          });

          selectobjs.push(touchP.children[0],thiz.cover);
          scene.add(thiz.cover);

          textP = createText('P(x,y)', cosr + 50, sinr + 30, 4, '#000', 28);

          textA = createText('A(1,0)', 340, 30, 0, '#000', 21);

          texta = createText('α的终边', cosR, sinR, 0, '#000', 28);

          //lineOP
          createLineOP(cosR, sinR);

          textα = createText('sinα', cosr + 30, sinr / 2, 0, '#000000', 25);
          //linePM
          createLinePM(cosr, sinr);
          //lineOM
          createLineOM(cosr, sinr);

          //创建T点
          pointT = new THREE.Group;
          let geometry1 = new THREE.CircleGeometry(6, 32);
          let material1 = new THREE.MeshBasicMaterial({color: '#FF5D00', transparent: true, opacity: 1});
          let circleT = new THREE.Mesh(geometry1, material1);
          vertices = [];
          for (let i = 0; i <= 360; i += 2) {
          ...
          circleP1 = createLineMesh(vertices, '#000000', 3, 1);

          pointT.add(circleT, circleP1);

          createLineTA(cosr, sinr);
          ...
          pointP.position.set(cosr, sinr, 4);

          angleA = new THREE.Group();
          angleA = drawAngle(0, 60, 25, '#258269', '#5EAA97', 'α', -2);

          scene.add(circle, touchP, pointP, textP, textA, pointT, angleA, texta, textα);
        };
        createObj();
        ...
        let drawLine = () => {
          scene.remove(angleA);
          let obj = intersection.sub(offset), x = obj.x, y = obj.y, x1, y1, x11 = x;
        ...
        var drawAxisArrow = (origin, dir, color, style) => {
          ...
          if (style == 1) {
            ...
            axis.add(triangle2);
            text = createText('x', dir.x, -5, 0, '#838383', 24);
            axis.add(text);
            text = createText('0', -14, -2, 0, '#838383', 24);
            ...
            var triangle2 = createTriangleFace(vertices, "#000");
            axis.add(triangle2);
            text = createText('y', 20, dir.y + 10, 0, '#838383', 24);
            axis.add(text);
          }
        };
        var createAxis = () => {
        ...
        var resetWidget = () => {
          touchP.position.set(R * Math.cos(Math.PI / 3), R * Math.sin(Math.PI / 3), 6);
          touchP.rotation.z = -30 * Math.PI / 180;
          ...
          createLineOM(r * Math.cos(Math.PI / 3), r * Math.sin(Math.PI / 3));
          lineTA.visible = false;
          linePM.visible = false;
          lineOM.visible = false;
          this.switch_checked = false;
          createLineTA(r * Math.cos(Math.PI / 3), r * Math.sin(Math.PI / 3));
          textP.position.set(r * Math.cos(Math.PI / 3) + 50, r * Math.sin(Math.PI / 3) + 30, 4);
          scene.remove(angleA);
          angleA = new THREE.Group();
          angleA = drawAngle(0, 60, 25, '#258269', '#5EAA97', 'α', -2);
          scene.add(angleA)
          pointT.position.set(300, 519.5, 5);
        };

        var TO = function () {
          return {
            reset: resetWidget,
            lineTAShow: lineTAShow,
            lineTAHide: lineTAHide
          }
        };
        return TO();
      },
