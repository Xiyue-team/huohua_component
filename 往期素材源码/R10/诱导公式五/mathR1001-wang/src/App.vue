  ...
  methods: {
    ...
    init() {
      ...
      var initLine = () => {
        ...
        lineObj.om = common.unitLine({ color: "#FF1F3A" });
      ...
      var createObj1 = () => {
        ...
        //创建贴图π/2;
        textureA = common.createImg([common.vec3(0, 0, 1)], 37, 26, "static/UI/222.png");
        textureA.scale.set(1.1, 1.1, 1.1);
        if (!touchAImg) {
      ...
      var createObj = () => {
        ...
        let angleA;
        if (this.alpha != 0) {
          angleA = drawAngle(0, this.alpha, ANGLE_SIZE, '#000', '#258269', 'α', -2);
          obj.add(angleA);
        }
        //创建原点到两个拖动点的连线
      ...
      var drawAngle = (startangle, endangle, size, color, circleColor, font, index, booltext = true) => {
        let dx, dy, vertices = [];
        ...
        if (booltext) {
          text = common.createText(font, (size + 15) * cos(common.radian(angle)) - 5, (size + 15) * sin(common.radian(angle)) + 18, 2, '#BD00D4', 24);
          obj.add(text);
        } else {
          textureA.position.set((size + 15) * cos(common.radian(angle)) - 5, (size + 15) * sin(common.radian(angle)) + 10, 2);
          obj.add(textureA);
        }
        obj.add(line, circle);
        return obj;
      ...
      //画跟P'相关的线段
      var secLine = (startangle, endangle, booltext) => {
        ...
        if (startangle != endangle) {
          let angle = drawAngle(startangle, endangle, ANGLE_SIZE * 2, '#F5A623', '#F8E71C', ' ', -1, booltext);
          copyPGroup.add(angle);
        }
        //画y=x线段
        let yx = common.countPos(TOUCH_RADIUS, 45);
        lineObj.yx = common.scaleLine([-yx.x, -yx.y, 0], [yx.x, yx.y, 0], lineObj.yx);
        // let textyx = common.createText('y=x', yx.x - 10, yx.y - 20, 0, '#1500FF', 24);
        lineObj.textyx.position.set( yx.x - 10, yx.y - 20, 0);
        copyPGroup.add(lineObj.op1, textGroup, lineObj.yx, lineObj.textyx);
        return copyPGroup;
      }
      //拖动P点创建P'的线段
      var drawP1Line = (startangle, endangle) => {
        ...
        copyPGroup = secLine(startangle, endangle, false);
        ...
        function rotateLine() {
          ...
          if (max_limit_radius >= 0) {
            if (that.beta >= max_limit_radius) {
              ...
              copyPGroup = secLine(0, that.beta, false);
              scene.add(copyPGroup);
              return;
            }
            copyPGroup = secLine(0, that.beta, false);
            ...
            if (that.beta <= max_limit_radius) {
              ...
              cancelAnimationFrame(timer);
              that.beta = max_limit_radius;
              copyPGroup = secLine(that.beta, 0, false);
              scene.add(copyPGroup);
              return;
            }
            copyPGroup = secLine(that.beta, 0, false);
      ...
      var drawSubline = (P) => {
        ...
        lineObj.om = common.scaleLine([0, 0, 1], [P.x, 0, 1], lineObj.om)
        group.add(lineObj.om);
        lineObj.om1 = common.scaleLine([0, 0, 1], [P.y, 0, 1], lineObj.om1)
        group.add(lineObj.om1);
        ...
        //创建交点圆点
        lineObj.circlem.position.set(P.x, 0, 1);
        group.add(lineObj.circlem);
        lineObj.circlem1.position.set(P.y, 0, 1);
        ...
        if (mousedownflag) {
          if (raycaster.ray.intersectPlane(plane, intersection)) {
            ...
            if (selectobj.name === 'A') {
              ...
              touchA.y = y
              touchA.x = x
              this.alpha = Math.floor(countAngle(touchA));
            }
            createObj();
          }
        }
