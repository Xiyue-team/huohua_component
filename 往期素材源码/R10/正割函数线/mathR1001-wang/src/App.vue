<template>
  <div id="app" class="noselect">
    ...
    <div class="ctrl">
      <ui-btn type="switch" v-model="checked">正割函数线</ui-btn>
    </div>
    <!--侧边按钮区-->
  </div>
</template>
  ...
  methods: {
    ...
    init() {
      ...
      var initLine = () => {
        ...
        lineObj.pn = common.unitLine({ color: "#000", width: 3 });
        lineObj.on = common.unitLine({ color: "#FF1DF8", width: 4 });
        lineObj.texta1 = common.createText('α', 0, 0, 2, '#BD00D4', 24);
        lineObj.textn = common.createText('N', 0, 0, 0, '#000', 24);
        lineObj.textseca = common.createText('secα', 0, 0, 0, '#000', 28);
        lineObj.texta = common.createText('α的终边', 0, 0, 0, '#000', 28);
        lineObj.circlen = common.createStrokeCircle(6, '#D0021B');
      ...
      var drawSecantLine = (alpha) => {
        ...
        let x = RADIUS / cos(common.radian(alpha));
        lineObj.on = common.scaleLine([0, 0, 1], [x, 0, 1], lineObj.on);
        lineObj.textseca.position.set(x > 400 ? 200 : x / 2, 0, 5);
        secGroup.add(lineObj.on, lineObj.textseca);
        lineObj.pn = common.scaleLine([P.x, P.y, 1], [x, 0, 1], lineObj.pn);
        secGroup.add(lineObj.pn);
        lineObj.circlen.position.set(x, 0, 2);
        secGroup.add(lineObj.circlen);
        lineObj.textn.position.set(x, 0, 0);
        secGroup.add(lineObj.textn);
        scene.add(secGroup);
      }
      var countPPos = (obj) => {
        let point = {};
