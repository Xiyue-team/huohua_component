    ...
    methods: {
      ...
      init() {
        var scene = null,
          ...
          R = 390,
          r = 300,
          STARTX = r * Math.cos(Math.PI / 3),
          STARTY = r * Math.sin(Math.PI / 3),
          touchP = null,
          selectobjs = [],
          selectobj = null,
        ...
        var createLineOP = (x, y) => {
          ...
          texta = createText('α的终边', x1, y1, 0, '#000', 28);

          scene.add(lineOP, texta);
        };
        ...
        lineTAstart = createLineMesh(startvertices, '#0db2ff', 3, 4);
        lineTAstart.visible = false;
        scene.add(lineTAstart);
        var createLineTA = (x, y) => {
          scene.remove(lineTA);
          lineTA = new THREE.Group;

          let caleLineTA = Math.sqrt(Math.pow(300 * y / x, 2));
          lineTAstart.scale.set(caleLineTA, caleLineTA, caleLineTA);
          lineTAstart.position.set(300, 150 * y / x, 2);
          lineTAstart.rotation.z = Math.PI / 2;
          ...
          textT.position.set(320, 300 * y / x, 0);

          //T点
          pointT.position.set(300, 300 * y / x, 3);
          lineTA.add(dashLine);
        ...
        var createObj = () => {
          ...
          textP = createText('P(x,y)', STARTX + 50, STARTY + 30, 0, '#000', 28);
          ...
          textT = createText('T', 320, 300 * STARTY / STARTX, 0, '#000', 25);
          textT.visible = false;
          createLineTA(STARTX, STARTY);
          ...
          pointP.position.set(STARTX, STARTY, 5);

          angleA = new THREE.Group();
          angleA = drawAngle(0, 60, 25, '#258269', '#5EAA97', 'α', -2);

          scene.add(circle, touchP, pointP, textP, textA, pointT, angleA, textT);
        };
        createObj();
        ...
        var drawLine = () => {
          scene.remove(angleA);
          let obj = intersection.sub(offset), x = parseInt(obj.x), y = obj.y, x1, y1, x11 = x;
        ...
        var resetWidget = () => {
          ...
          angleA = drawAngle(0, 60, 25, '#258269', '#5EAA97', 'α', -2);
          scene.add(angleA)
          pointT.position.set(300, 519.5, 3);
        };
        var TO = function () {
          return {
            reset: resetWidget,
