<template>
  <div id="app" class="noselect">
    ...
    <div class="ctrl">
      <ui-btn type="switch" v-model="checked">余割函数线</ui-btn>
    </div>
    <!--侧边按钮区-->
  </div>
</template>
  ...
  methods: {
    ...
    init() {
      ...
      var initLine = () => {
        ...
        lineObj.on = common.unitLine({ color: "#00CD43", width: 4 });
        lineObj.texta1 = common.createText('α', 0, 0, 2, '#BD00D4', 24);
        lineObj.textn = common.createText('N', 0, 0, 0, '#000', 24);
        lineObj.textcsca = common.createText('cscα', 0, 0, 0, '#000', 28);
      ...
      var drawSecantLine = (alpha) => {
        ...
        lineObj.on = common.scaleLine([0, 0, 1], [0, y, 1], lineObj.on);
        lineObj.textcsca.position.set(-40, y > 500 ? 250 : y / 2, 5);
        secGroup.add(lineObj.on, lineObj.textcsca);
        lineObj.pn = common.scaleLine([P.x, P.y, 1], [0, y, 1], lineObj.pn);
        secGroup.add(lineObj.pn);
        lineObj.circlen.position.set(0, y, 2);
        secGroup.add(lineObj.circlen);
        lineObj.textn.position.set(15, y + 15, 0);
