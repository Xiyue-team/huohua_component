...
export class RandomDot {
    ...
    // 一个位置移动到另一个位置的移动动画函数
    moveAnimate(startX: number, startY: number, endX: number, endY: number, visible: boolean) {
        ...
        const thiz = this;
        function ani() {
            if ((endX - startX) < 0.1 && (endY - startY) < 0.1) {
                thiz.box.position.set(endX, endY, 0);
                thiz.box.visible = visible;
