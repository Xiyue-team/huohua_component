...
export class Model extends ThreeBase {
    ...
    //添加反应方程式
    private equation = ThreeUtil.createImg(162.9, 41.4, equation, 0, -230, 2);
    //添加反应区域圆形背景
    ...
    initObject() {
        this.initialMoveAni(8, 2, 138, 168, 1);
        this.runAnimation(168);
        this.decomposeEvent(168, 1000);
    ...
    // 创建分子函数
    creatMolecule(molecule: any, width: number, height: number, img: any, name: string, radius1: number, radius2: number, ) {
        const randomX1 = Math.random() * radius1 * Math.cos(Math.random() * 360 / 180 * Math.PI);
        const randomY1 = Math.random() * radius1 * Math.sin(Math.random() * 360 / 180 * Math.PI);
        molecule = new RandomDot(randomX1, randomY1, 0, width, height, img, name);
        molecule.selfMoveAnimate(randomX1, randomY1, 0, 0, radius2);
        this.moleculeArr1.push(molecule, );
        molecule.addSince(this.scene);
    }
    // 创建初始状态下压强为中等时反应区域内分子组
    initialMoveAni(num: number, num1: number, radius1: number, radius2: number, scaleNum: number) {
        for (let i = 0; i < num; i++) {
            this.creatMolecule(this.activeA, 38, 38, activeA, 'activeA', radius1, radius2);
            this.creatMolecule(this.activeB, 25, 25, activeB, 'activeB', radius1, radius2);
        }
        for (let i = 0; i < num1; i++) {
            this.creatMolecule(this.normalA, 20, 20, normalA, 'normalA', radius1, radius2);
            this.creatMolecule(this.normalB, 15, 15, normalB, 'normalB', radius1, radius2);
        }
        this.circle.scale.set(scaleNum, scaleNum, 1);
        this.circleBg.scale.set(scaleNum, scaleNum, 1);
    }
    // 获取两个分子或离子之间的距离函数
    ...
    // A/B分子反应生成AB分子
    mediumEvent(index: number, i: number, radius: number) {
        if (this.moleculeArr1[i].box.name === 'activeA' && this.moleculeArr1[index].box.name === 'activeB') {
            ...
            this.activeAB.selfMoveAnimate((this.moleculeArr1[i].box.position.x + this.moleculeArr1[index].box.position.x) / 2,
                (this.moleculeArr1[i].box.position.y + this.moleculeArr1[index].box.position.y) / 2, 0, 0, radius);
    ...
    // 让AB分子消失并生成A/B分子函数
    decomposeEvent(radius: number, time: number) {
            ...
            for (let i = 0, L = this.moleculeArr1.length; i < L; i++) {
                if (this.moleculeArr1[i].box.name === 'activeAB') {
                    if ((this.moleculeArr1[i].box.material as any).opacity <= 0) {
                        ...
                        this.activeA.selfMoveAnimate(randomX, randomY, 0, 0, radius);
                        this.moleculeArr1[i].removeSince(this.scene);
            ...
            for (let i = 0, L = this.moleculeArr1.length; i < L; i++) {
                if (this.moleculeArr1[i].box.name === 'activeAB1') {
                    if ((this.moleculeArr1[i].box.material as any).opacity <= 0) {
                        ...
                        this.activeB.selfMoveAnimate(randomX, randomY, 0, 0, radius);
                        this.moleculeArr1[i].removeSince(this.scene);
    ...
    // 反应中分子及离子三种不同的运动函数
    runAnimation(radius: number) {
        const thiz = this;
        function ani() {
            for (let i = 0, L = thiz.moleculeArr1.length; i < L; i++) {
                thiz.runIntoAni(i, radius);
    ...
    lowPressureEvent() {
        this.clearTimer();
        this.anewMolecule();
        this.initialMoveAni(8, 2, 170, 202, 1.2);
        this.runAnimation(202);
        this.decomposeEvent(202, 1000);
    ...
    highPressureEvent() {
        this.clearTimer();
        this.anewMolecule();
        this.initialMoveAni(8, 2, 90, 130, 0.8);
        this.runAnimation(130);
        this.decomposeEvent(130, 500);
