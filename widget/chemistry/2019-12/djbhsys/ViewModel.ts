...
import { DjbhsysViewHandler } from './services/DjbhsysViewHandler';
...
export class ViewModel extends Vue {
  ...
  created() {
    ...
    ViewController.getInstance(new DjbhsysViewHandler(this), viewOption);
  ...
  private setBgColor() {
    if (window.env.browserInfo.os === 'Mac OS X') {
      document.getElementById('background').style.backgroundColor = '#4d4d4d';
    } else if (window.env.browserInfo.isIpad) {
      document.getElementById('background').style.backgroundColor = '#434343';
      ...
      document.getElementById('background').style.backgroundColor = '#444345';
    }
  }
