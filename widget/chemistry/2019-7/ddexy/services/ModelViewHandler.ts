...
export class ModelViewHandler extends CommonViewHandler implements ViewHandler {
    ...
    domReady() {
        ...
        this.tipText = $('.tip_text');
        this.play1.on('timeupdate', () => {
            if (this.play1.currentTime() >= 1.2) {
                this.tipText.text(this.text[0]);
            }
        });
        this.play2.on('timeupdate', () => {
            if (this.play2.currentTime() >= 1.9) {
                this.tipText.text(this.text[1]);
            }
        });
    }
    resize(): void {
    ...
    getEvent1(index: number) {
        if (index === 1) {
            ...
            (this.viewModel as any).tipImg = require('../sub_static/UI/solution.jpg');
        } else if (index === 2) {
            this.buttonEvent((this.viewModel as any).active2, this.play2);
            (this.viewModel as any).active2 = true;
            (this.viewModel as any).tipImg = require('../sub_static/UI/colloid.jpg');
        }
    }
