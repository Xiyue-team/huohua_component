...
import * as atomCa from '../sub_static/Ca.png';         //Ca离子
import * as atomNa from '../sub_static/Na.png';     //Na离子
import * as atomOh from '../sub_static/Oh.png';       //Oh离子
import * as atomK from '../sub_static/K.png';     //K离子
...
export class Model extends ThreeBase {
  ...
  private atomCa            = ThreeUtil.createImg(18, 18, atomCa, 0, 0, 0);
  private atomNa            = ThreeUtil.createImg(18, 18, atomNa, 0, 0, 0);
  private atomOh            = ThreeUtil.createImg(18, 18, atomOh, 0, 0, 0);
  private atomK             = ThreeUtil.createImg(18, 18, atomK, 0, 0, 0);
  ...
  */
  ...
  initImage () {
    //三个烧杯
    const beaker1 = ThreeUtil.createImg(213, 241, beaker, -300, 0, 1);
    const beaker2 = beaker1.clone();
    beaker2.position.set(0, 0, 1);
    const beaker3 = beaker1.clone();
    beaker3.position.set(300, 0, 1);
    //烧杯对应的公式
    const formulaImage1   = ThreeUtil.createImg(195, 19, formula1, -300, -150, 1);
    const formulaImage2   = ThreeUtil.createImg(231, 25, formula2, 305, -150, 1);
    const formulaImage3   = ThreeUtil.createImg(175, 19, formula3, 5, -150, 1);
    ...
    this.scene.add(beaker1, beaker2, beaker3);
  ...
  //初始化放大镜背后背景
  initCircle() {
    const geometry = new THREE.CircleBufferGeometry(90, 60);
  ...
  //初始化烧杯溶液文字
  initText() {
    const beakerTextLeft = ThreeUtil.createNewRomanEnText(this.acidtText[0], -300, 150, 0, '#ffffff', 0.4);
    const beakerTextCenter = ThreeUtil.createNewRomanEnText(this.acidtText[1], 0, 150, 0, '#ffffff', 0.4);
    const beakerTextRight = ThreeUtil.createNewRomanEnText(this.acidtText[2], 300, 150, 0, '#ffffff', 0.4);
  ...
  initHydrochlorideModel() {
    for (let i = 0; i < 6; i++) {
      this.createMolecule(this.activeA, 'activeA', 1, 1.35, -295, -10, this.atomOh, 1, this.moleculeArr1);
      this.createMolecule(this.activeB, 'activeB', 1, 0.95, -295, -10, this.atomNa, 1, this.moleculeArr1);
    }
  }
  ...
  initSulphateModel() {
    for (let i = 0; i < 6; i++) {
      this.createMolecule(this.activeA, 'activeA', 2, 1.35, 5, -10, this.atomOh, 1, this.moleculeArr1);
      this.createMolecule(this.activeB, 'activeB', 2, 1.25, 5, -10, this.atomK, 1, this.moleculeArr1);
    }
  }
  ...
  initNitricModel() {
    for (let i = 0; i < 3; i++) {
      this.createMolecule(this.activeA, 'activeA', 3, 1.35, 305, -10, this.atomOh, 1, this.moleculeArr1);
      this.createMolecule(this.activeA, 'activeA', 3, 1.35, 305, -10, this.atomOh, 1, this.moleculeArr1);
      this.createMolecule(this.activeB, 'activeB', 3, 1.1, 305, -10, this.atomCa, 1, this.moleculeArr1);
    }
  }
