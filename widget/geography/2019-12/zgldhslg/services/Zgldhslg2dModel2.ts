...
export class Zgldhslg2dModel {
    ...
    private initBaseButton(): fabric.Group {
        ...
        this.baseInfos.forEach(item => {
            const text = new fabric.Text(
                item.text, {
                fontFamily: '微软雅黑',
                fontSize: parent.basePanelFontScale * window.innerWidth
            });
            ...
            if (item.type === '01') {
                text.top = window.innerHeight / 2 - 20 - rect.height + (rect.height - text.height) / 2;
                rect.top = window.innerHeight / 2 - 20 - rect.height;
                parent.ctrlOnsitePanelTop = rect.top;

            } else {
                text.top = window.innerHeight / 2 + 20 + (rect.height - text.height) / 2;
                rect.top = window.innerHeight / 2 + 20;
            }
    ...
    private initNationButton(content: string, type: string): fabric.Group {
        ...
        text.left = window.innerWidth * 0.11;
    ...
    */
    ...
    private initName() {
        ...
        spaceInfos.forEach(item => {
            const name = new fabric.Text(
                item.text, {
                fontFamily: '微软雅黑',
                fontSize: this.windowWidth * this.nameFontScale,
                selectable: false,
                left: item.left * this.imgWidth,
                top: item.top * this.imgHeight,
            });
            item.kind = 'name';
            name.data = item;
    ...
    private initSpace(type: string) {
        ...
        spaceInfos.forEach(item => {
            const space = new fabric.Rect({
                ...
                stroke: 'rgba(0,0,0,0.10)',
                strokeWidth: 0.5,
                left: Math.floor(item.left * this.imgWidth),
                top: Math.floor(item.top * this.imgHeight),
                selectable: false
            });
    ...
    private initMap() {
        ...
        fabric.Image.fromURL(mapPng, function (img) {
            ...
            parent.imgWidth = img.width;
            parent.imgHeight = img.height;
            parent.initZoom = parent.mapCanvas.getWidth() / img.width;
            parent.mapCanvas.setZoom(parent.initZoom);
            parent.initName();
...
*/
    ...
    resize() {
        this.reset();
        this.mapCanvas.setZoom(this.mapCanvas.getWidth() / this.mapImg.width);
        this.mapCanvas.setHeight(window.innerHeight);
        ...
        this.onsiteBtn.width = window.innerWidth * 0.31;
        this.onsiteBtn.setCoords();
        this.offshoreBtn.width = window.innerWidth * 0.31;
        this.offshoreBtn.setCoords();
    }
}
...
export class ScaleValue {
    public scale: number;
    constructor() {
        const mapScale = 1 - 0.65;
        const ratio = window.innerWidth * mapScale / window.innerHeight;
        if (ratio > (16 / 9)) {
            this.scale = window.innerWidth * mapScale / 1200;
        } else {
            this.scale = window.innerHeight / 550;
        }
    }
}
