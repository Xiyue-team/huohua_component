...
export class Threejs3dModel extends ThreeBase {
  ...
  render = () => {
    ...
    const y1 = 300 + 235 * -(this.camera.position.z - 500) / 350;
  ...
  addStaticImage() {
    ...
    this.map = ThreeUtil.createImg(this.mapWidth / 3 * 2, this.mapHeight / 3 * 2, backgroundImage, x, y);
  ...
  createTextImage(image: any, config: any) {
    const mesh = ThreeUtil.createImg(
      config.width / 3 * 2,
      config.height / 3 * 2, image,
      (config.left / 3 - this.mapWidth / 3 / 2 + config.width / 6) * 2,
      (-config.top / 3 + this.mapHeight / 3 / 2 - config.height / 6) * 2,
      1
    );
