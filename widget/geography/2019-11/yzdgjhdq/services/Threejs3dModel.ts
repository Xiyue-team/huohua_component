...
export class Threejs3dModel extends ThreeBase {
  ...
  isShowRegion = [false, false, false, false, false, false];
  ...
  render = () => {
    ...
    const x1 = 380 + 275 * -(this.camera.position.z - 500) / 350;
    const x2 = -370 + -275 * -(this.camera.position.z - 500) / 350;

    const y1 = 350 + 235 * -(this.camera.position.z - 500) / 350;
    const y2 = -370 + -236 * -(this.camera.position.z - 500) / 350;
  ...
  addTextImageSrc() {
    ...
    this.regionImageSrc = (new LeftImageStorage()).regionImageSrc;
  }
  ...
  addStaticImage() {
    ...
    this.map = ThreeUtil.createImg(this.mapWidth / 3 * 1, this.mapHeight / 3 * 1, backgroundImage, x, y);
    this.scene.add(this.map);

    for (let i = 0; i < this.regionImageSrc.length; i++) {
      this.regionArray[i] = ThreeUtil.createImg(this.mapWidth / 3 * 1, this.mapHeight / 3 * 1, this.regionImageSrc[i], x, y);
  ...
  createTextImage(image: any, config: any) {
    const mesh = ThreeUtil.createImg(
      config.width / 3 * 1,
      config.height / 3 * 1, image,
      (config.left / 3 - this.mapWidth / 3 / 2 + config.width / 6) * 1,
      (-config.top / 3 + this.mapHeight / 3 / 2 - config.height / 6) * 1,
      1
    );
  ...
  cameraChangeAnim(i: number) {
    ...
    this.cameraAnim = TweenMax.to(tween, 1, {
      x: this.textArray[i].position.x,
      y: this.textArray[i].position.y,
      z: 150,
      onUpdate: () => {
  ...
  hideRegion() {
    for (let i = 0; i < this.regionImageSrc.length; i++) {
      this.regionArray[i].visible = this.isShowRegion[i];
    }
  }
  ...
  showPlateauText(isShow: boolean) {
    this.isShowRegion[3] = isShow;
    this.hideRegion();
  }

  // 山地
  showHillyAreaText(isShow: boolean) {
    this.isShowRegion[4] = isShow;
    this.hideRegion();
  }

  // 平原
  showPlainText(isShow: boolean) {
    this.isShowRegion[5] = isShow;
    this.hideRegion();
  }

  // 高原2
  showPlateauText2(isShow: boolean) {
    this.isShowRegion[0] = isShow;
    this.hideRegion();
  }

  // 山地2
  showHillyAreaText2(isShow: boolean) {
    this.isShowRegion[1] = isShow;
    this.hideRegion();
  }

  // 平原2
  showPlainText2(isShow: boolean) {
    this.isShowRegion[2] = isShow;
    this.hideRegion();
  }
