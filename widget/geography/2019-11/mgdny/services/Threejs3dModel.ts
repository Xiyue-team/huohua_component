...
export class Threejs3dModel extends ThreeBase {
  ...
  addRegionImage() {
    ...
    for (let i = 0; i < this.regionImageSrc.length; i++) {
      this.regionArray[i] = ThreeUtil.createImg(this.mapWidth / 3, this.mapHeight / 3, this.regionImageSrc[i], x, y);
  ...
  createTextImage(image: any, config: any) {
    const mesh = ThreeUtil.createImg(
      ...
      (config.left - 285) / 3 - this.mapWidth / 3 / 2 + config.width / 6,
      -(config.top - 323) / 3 + this.mapHeight / 3 / 2 - config.height / 6,
      1
    );
