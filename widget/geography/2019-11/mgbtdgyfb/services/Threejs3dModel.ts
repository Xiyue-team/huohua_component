...
import eastImage from '../sub_static/img/leftImage/east.png';
import southImage from '../sub_static/img/leftImage/south.png';
import westImage from '../sub_static/img/leftImage/west.png';
...
export class Threejs3dModel extends ThreeBase {
  ...
  eastBackground: THREE.Mesh;
  southBackground: THREE.Mesh;
  westBackground: THREE.Mesh;
  ...
  render = () => {
    ...
    const x1 = 180 + 275 * -(this.camera.position.z - 1000) / 350;
    const x2 = -270 + -275 * -(this.camera.position.z - 1000) / 350;

    const y1 = 20 + 235 * -(this.camera.position.z - 1000) / 350;
    const y2 = -70 + -236 * -(this.camera.position.z - 1000) / 350;
  ...
  addStaticImage() {
    ...
    this.map = ThreeUtil.createImg(this.mapWidth / 3, this.mapHeight / 3, backgroundImage, x, y);
    //创建切换的三张背景图片
    this.eastBackground = ThreeUtil.createImg(this.mapWidth / 3, this.mapHeight / 3, eastImage, x, y);
    this.southBackground = ThreeUtil.createImg(this.mapWidth / 3, this.mapHeight / 3, southImage, x, y);
    this.westBackground = ThreeUtil.createImg(this.mapWidth / 3, this.mapHeight / 3, westImage, x, y);
    this.eastBackground.visible = false;
    this.southBackground.visible = false;
    this.westBackground.visible = false;
    this.scene.add(this.map, this.eastBackground, this.southBackground, this.westBackground);
  }

  backgroundControl(value: number) {
    switch (value) {
      case 1:
        this.eastBackground.visible = true;
        this.southBackground.visible = false;
        this.westBackground.visible = false;
        break;
      case 2:
        this.eastBackground.visible = false;
        this.southBackground.visible = false;
        this.westBackground.visible = true;
        break;
      case 3:
        this.eastBackground.visible = false;
        this.southBackground.visible = true;
        this.westBackground.visible = false;
        break;
      default:
        this.eastBackground.visible = false;
  ...
  buttonControl() {
    (window as any).viewHandler.viewModel.$data.buttonActive1 = false;
    (window as any).viewHandler.viewModel.$data.buttonActive2 = false;
    (window as any).viewHandler.viewModel.$data.buttonActive3 = false;
