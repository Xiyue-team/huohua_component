...
export class Threejs3dModel extends ThreeBase {
  ...
  addStaticImage() {
    ...
    this.map = ThreeUtil.createImg(this.mapWidth / 1.8, this.mapHeight / 1.8, backgroundImage, x, y);
    (this.map.material as any).transParent = true;
  ...
  addTextImage() {
    for (let i = 0; i < mapJson.textConfig.length; i++) {
      ...
      this.regionImageArray[i] = ThreeUtil.createImg(this.mapWidth / 1.8, this.mapHeight / 1.8, this.regionImageSrc[i], 0, 0);
      this.map.add(this.textArray[i]);
      this.regionImageArray[i].visible = false;
      this.scene.add(this.regionImageArray[i]);
    }
  }
  ...
  createTextImage(image: any, config: any) {
    const mesh = ThreeUtil.createImg(
      config.width  / 1.8,
      config.height  / 1.8, image,
      (config.left - this.mapWidth / 2 + config.width / 2) / 1.8,
      (-config.top + this.mapHeight / 2 - config.height / 2) / 1.8,
      1
    );
  ...
  cameraChangeAnim(i: number) {
    ...
    this.cameraAnim = TweenMax.to(tween, 1, {
      ...
      z: 250,
      onStart: () => {
        (this.map.material as any).opacity = 0.5;
        this.regionImageArray[i].visible = true;
      },
  ...
  reverseCamera() {
    ...
    this.cameraAnim = TweenMax.to(tween, 1, {
      ...
      onComplete: () => {
        (this.map.material as any).opacity = 1;
        for (let i = 0; i < this.regionImageArray.length; i++) {
          this.regionImageArray[i].visible = false;
        }
      },
      paused: true,
  ...
  showHillyAreaText(isShow: boolean) {
    for (let i = 0; i < this.regionImageArray.length; i++) {
      this.regionImageArray[i].visible = isShow;
    }
