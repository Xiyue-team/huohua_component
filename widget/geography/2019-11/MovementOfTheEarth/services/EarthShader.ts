...
export class EarthShader {
    ...
    static earthFragment =
        ...
        void main(void) {
            ...
            vec3 lightVectorW = normalize(vLightPosition - vSelfPosition);
            float ndl = clamp( dot(vNormalW, lightVectorW) * 10.,0.,1.);
            float ndlt = max(0., dot(vNormalW, lightVectorW)) * 5.;
            vec3 angleW = normalize(viewDirectionW + lightVectorW);
            float specComp = max(0., dot(vNormalW, angleW));
            specComp = pow(specComp, max(1., 64.)) *0.1;
            if (nightcolor.r*(1.-ndlt)>=0.3) {
                nightcolor.rgb=nightcolor.rgb*2.;
