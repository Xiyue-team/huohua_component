...
import { RightImageStorage } from './RightImageStorage';
...
export class RightCradCanvas {
  ...
  initContainer() {
    ...
    this.app.renderer.resize(768 * this.scale, 510 * this.scale);
  ...
  initImage() {
    ...
    add(rightImageStorage.imageSrc4).
    add(rightImageStorage.imageSrc5).
  ...
  async addCard(res: PIXI.LoaderResource) {
    ...
    for (let i = 0; i < this.config.smallCradImage.length; i++) {
      ...
      if (!(res as any)[src + '/five/cardImage' + (i + 1) + '.png']) {
        srcFive = (res as any)[src + '/one/cardImage' + (i + 1) + '.png'].texture;
      } else {
        srcFive = (res as any)[src + '/five/cardImage' + (i + 1) + '.png'].texture;
      }

      if (!(res as any)[src + '/four/cardImage' + (i + 1) + '.png']) {
        srcFour = (res as any)[src + '/one/cardImage' + (i + 1) + '.png'].texture;
      } else {
        srcFour = (res as any)[src + '/four/cardImage' + (i + 1) + '.png'].texture;
      }

      if (!(res as any)[src + '/three/cardImage' + (i + 1) + '.png']) {
        srcThree = (res as any)[src + '/one/cardImage' + (i + 1) + '.png'].texture;
      } else {
        srcThree = (res as any)[src + '/three/cardImage' + (i + 1) + '.png'].texture;
      }

      if (!(res as any)[src + '/two/cardImage' + (i + 1) + '.png']) {
  ...
  addMask() {
    ...
    for (let i = 0; i < this.card.length; i++) {
      ...
      this.maskArray[i].lineStyle(1, 0x000000, 0.06);
      let beginFill = 0x000000;
      if (!this.config.smallCradImage[i].color) {
        beginFill = 0x000000;
      } else {
        beginFill = this.config.smallCradImage[i].color;
      }
      this.maskArray[i].beginFill(beginFill, 1);
      ...
      this.maskArray[i].on('pointerdown', (event: any) => {
        ...
        this.three3dModel.riverTextImageArray[i][0].visible = true;
        this.animIndex = i;
        this.card[this.animIndex].children[5].text = window.env.browserInfo.lang.cardText[i].sub_title[0];
      ...
      // 添加文字
      const text = this.createSmallImageText(lang.smallCradText[i], this.config.smallImageText[i]);
  ...
  createSmallImageText(textString: string, config: any) {
    // 添加文字内容
    const textStyle = new PIXI.TextStyle(config);
    const text = new PIXI.Text(textString, textStyle);
    text.position.x = this.config.cardConfig.x + config.x;
    text.position.y = this.config.cardConfig.y + config.y;
    return text;
  }
  ...
  addCardMask() {
    ...
    this.bigImageMask.on('pointerdown', (event: any) => {
      ...
      this.timer1 = setTimeout(() => {
        // 显示卡片中的文字
        this.card[this.animIndex].children[5].renderable = true;
        this.card[this.animIndex].children[6].renderable = true;
        // 显示第二段动画放大蒙板显示 可点击
  ...
  async addScaleIcon(res: PIXI.LoaderResource) {
    ...
    this.narrowIcon.position.x = this.config.bigImageMask.width - 48;
    this.narrowIcon.position.y = this.config.bigImageMask.height - 48;
  }
  ...
  reverseAnim() {
    ...
    this.card[this.animIndex].children[3].renderable = false;
    this.card[this.animIndex].children[4].renderable = false;
    ...
    this.timer2 = setTimeout(() => {
      ...
      this.three3dModel.showPlateauText( (window as any).viewHandler.viewModel.$data.isShowPlateauText );
      this.three3dModel.showPlainText( (window as any).viewHandler.viewModel.$data.isShowPlainText );
      this.three3dModel.showHillyAreaText( (window as any).viewHandler.viewModel.$data.isShowHillyAreaText );
  ...
  previousPage() {
    ...
    this.three3dModel.riverTextImageArray[this.animIndex][this.pageIndex + 1].visible = false;
    this.card[this.animIndex].children[5].text = window.env.browserInfo.lang.cardText[this.animIndex].sub_title[this.pageIndex];
  }
  ...
  nextPage() {
    ...
    this.three3dModel.riverTextImageArray[this.animIndex][this.pageIndex].visible = true;

    this.card[this.animIndex].children[5].text = window.env.browserInfo.lang.cardText[this.animIndex].sub_title[this.pageIndex];

    (window as any).viewHandler.viewModel.$data.isButtonGrayPreviousPage = false;

    if (this.pageIndex === (this.card[this.animIndex].children.length - (7 - this.cardNumber[this.animIndex]) - 1)) {
  ...
  reset() {
    ...
    this.card[this.animIndex].children[5].text = window.env.browserInfo.lang.cardText[this.animIndex].sub_title[0];
    // 显示所有第一段动画小蒙板显示可点击
    for (let i = 0; i < this.card.length; i++) {
      this.card[i].renderable = false;
      this.maskArray[i].interactive = true;
  ...
  resize() {
    ...
    this.app.renderer.resize(768 * scale, 515 * scale);
