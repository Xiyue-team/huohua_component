...
export class Threejs3dModel extends ThreeBase {
  ...
  addTextImageSrc() {
    ...
    this.regionImageSrc = [
      regionImage1, regionImage2, regionImage3, regionImage4, regionImage5
    ];
  }
  ...
  // 添加静态图片
  addStaticImage() {
    this.map = ThreeUtil.createImg(this.mapWidth / 3 * 2, this.mapHeight / 3 * 2, backgroundImage, 0, 0);
  ...
  addRegionImage() {
    for (let i = 0; i < this.regionImageSrc.length; i++) {
      this.regionArray[i] = ThreeUtil.createImg(this.mapWidth / 3 * 2, this.mapHeight / 3 * 2, this.regionImageSrc[i], 0, 0);
  ...
  createTextImage(image: any, config: any) {
    const mesh = ThreeUtil.createImg(
      ...
      config.height / 3 * 2,
      image,
      config.left / 3 * 2 - this.mapWidth / 3 + config.width / 3,
      -config.top / 3 * 2 + this.mapHeight / 3 - config.height / 3,
      1
    );
