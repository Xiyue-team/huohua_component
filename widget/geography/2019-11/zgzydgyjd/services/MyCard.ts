...
export class MyCard {
  ...
  // 创建右侧的说明文字卡片
  createCard(img: any, title: string, content: string, img2?: any, img3?: any, img4?: any, img5?: any) {
    const image = new PIXI.Sprite(img);
    ...
    const image4 = new PIXI.Sprite(img4);
    image4.width = this.config.cardImage.width;
    image4.height = this.config.cardImage.height;
    image4.renderable = false;

    const image5 = new PIXI.Sprite(img5);
    image5.width = this.config.cardImage.width;
    image5.height = this.config.cardImage.height;
    image5.renderable = false;
    ...
    card.addChild(image4);
    card.addChild(image5);
    card.addChild(textTitle);
    card.addChild(textContent);
    // card.renderable = false;

    // textTitle.renderable = false;
    // textContent.renderable = false;
  ...
  createAnim(card: any, maskArray: any, maskGroup: PIXI.Container, index: number, smallMask: PIXI.Graphics,
    ...
    const anim = TweenMax.to(tween, 1, {
      ...
      onStart: () => {
        ...
        card[index].children[5].renderable = true;
        card[index].children[6].renderable = true;
      ...
      onUpdate: () => {
        ...
        card[index].children[5].alpha = tween.opacity;
        card[index].children[6].alpha = tween.opacity;
  ...
  createAnim2(card: any, index: number, bigMask: PIXI.Graphics, narrowIcon: PIXI.Sprite, rightWidth: number, scale: number) {
    ...
    const anim = TweenMax.to(tween, 1, {
      ...
      onStart: () => {
        ...
        card[index].renderable = true;
        card[index].children[5].renderable = false;
        card[index].children[6].renderable = false;
      },
      onUpdate: () => {
