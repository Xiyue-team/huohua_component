...
export class RightCradCanvas {
  ...
  async addCard(res: PIXI.LoaderResource) {
    ...
    let src = '/static/widget/geography/2019-10/yzddx/sub_static/img/right';
    if ((process.env.component as any).config.dev) {
      src = '/static/widget/geography/2019-10/yzddx/sub_static/img/right';
    } else {
      src = './widget/geography/2019-10/yzddx/sub_static/img/right';
    }
    ...
    for (let i = 0; i < this.config.smallCradImage.length; i++) {
      this.card[i] = await this.myCard.createCard(
        ...
        (res as any)[src + '/two/cardImage' + (i + 1) + '.png'].texture,
        (res as any)[src + '/three/cardImage' + (i + 1) + '.png'].texture
      );
