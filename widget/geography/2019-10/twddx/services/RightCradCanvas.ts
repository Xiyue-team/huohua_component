...
export class RightCradCanvas {
  ...
  addCardPosition(scale?: number) {
    ...
    for (let i = 0; i < this.card.length; i++) {
      ...
      this.card[i].children[0].position.x = this.config.smallCradImage[i].left;
      this.card[i].children[0].position.y = this.config.smallCradImage[i].top;
    }
  }
  ...
  createSmallImageText(textString: string, config: any) {
    ...
    textContainer.position.x = this.config.cardConfig.x + config.x;
    textContainer.position.y = this.config.cardConfig.y + config.y;
