...
export class Threejs3dModel extends ThreeBase {
    ...
    private sinPoint: THREE.Vector2[];
    ...
    createSinFunction() {
        ...
        for (let i = -Math.PI * 4; i <= 4 * Math.PI; i += Math.PI * 0.1) {
            this.sinPoint.push(new THREE.Vector2(i, Math.sin(i)));
        }
        const sin = this.curveLineHelper.createCurveLine({
        pointList: this.sinPoint,
        lineWidth: 0.2,
        style: 2,
        color: '#FFD621',
      });
        this.axisGroup.add(sin);
    }
    ...
    createMonotonicArrow() {
        ...
        for (let i = 0; i < 4; i++) {
          ...
          this.redArrow[i].rotation.z =  Math.atan(Math.cos((-2.5 * Math.PI) + (i * 2 * Math.PI)));
          this.greenArrow[i].rotation.z = Math.atan(Math.cos((-3.5 * Math.PI) + (i * 2 * Math.PI)));
          this.redArrow[i].position.set((-2.5 * Math.PI) + (i * 2 * Math.PI), Math.sin((-2.5 * Math.PI) + (i * 2 * Math.PI)), 0);
          this.greenArrow[i].position.set((-3.5 * Math.PI) + (i * 2 * Math.PI), Math.sin((-3.5 * Math.PI) + (i * 2 * Math.PI)), 0);
    ...
    createMonotonicAnimation() {
      const startPosition = -3.5 * Math.PI;
      const endPosition = -2.8 * Math.PI;
      const startPosition2 = -2.5 * Math.PI;
      const endPosition2 = -1.8 * Math.PI;
      this.moveAnimation[0] = this.helper.createArrowMoveAnimation(this.greenArrow, startPosition, endPosition, 1);
      this.moveAnimation[1] = this.helper.createArrowMoveAnimation(this.redArrow, startPosition2,
        endPosition2, 2, () => {this.moveAnimation[0].play(); });
    }
    ...
    createDashFunctionLine() {
        this.dashFunctionLine = this.curveLineHelper.createCurveLine({
          pointList: this.sinPoint,
          lineWidth: 0.2,
          style: 1,
          dashArray: 0.05,
        ...
        this.dashFunctionLine.visible = false;
        this.axisGroup.add(this.dashFunctionLine);
    }
    ...
    createCenter() {
        ...
        for (let i = 0; i < 4; i++) {
            points[i].position.x = (1 + i) * Math.PI;
            points[i + 4].position.x =  -(1 + i) * Math.PI;
            this.centerGroup.add(points[i], points[i + 4]);
        }
        this.centerGroup.add(point);
    ...
    createPeriodicDashLine() {
        ...
        for (let i = -6 * Math.PI; i < 4 * Math.PI; i += Math.PI * 0.1) {
            dashLinePoint.push(new THREE.Vector2(i, Math.sin(i)));
        }
    ...
    parityEvent(boolean: boolean) {
      this.dashFunctionLine.visible = boolean;
