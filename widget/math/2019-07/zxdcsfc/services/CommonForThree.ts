...
export default class CommonForThree {
  ...
  // 画圆线
  static createStrokeCircle(radius: number, { color = '#000', width = 1 } = {}, a: number, b: number, c: number) {
    ...
    const line = CommonForThree.drawDashOrLine(vertices, { color, width });
    group.position.set(a, b, c);
    group.add(line);
    return group;
  }

  // 画直角标识（带面）
  static drawRightAng(width: number, height: number, {
    color = '#000',
    opacity = 1,
    ...
    const SquareM = new THREE.MeshBasicMaterial({ color, transparent: false, opacity, depthTest });
    const SquareG = new THREE.PlaneGeometry(width, height);
    const Square = new THREE.Mesh(SquareG, SquareM);
    Square.position.set(position[0], position[1], position[2]);
    const group = new THREE.Group();
    if (text) {
      const texture = CommonForThree.createText(text, [width + dataShift, height * 0.8, zIndex + 1]);
      const texture1 = CommonForThree.createText(90 + '°', [width + dataShift1, height * 0.8, zIndex + 1], { isItalic: false });
  ...
  static drawAngle(startAngle: number, endAngle: number, {
    ...
    if (text) {
      const texture = CommonForThree.createText(text, [
        (size + 5) * Math.cos((startRad - endRad) / 2 + endRad) + dataShift,
        (size + 5) * Math.sin((startRad - endRad) / 2 + endRad) + dataShift2,
        zIndex + 1
      ]);
      const texture1 = CommonForThree.createText(endAngle + '°', [
        (size + 5) * Math.cos((startRad - endRad) / 2 + endRad) + dataShift1,
        (size + 5) * Math.sin((startRad - endRad) / 2 + endRad) + dataShift2,
        zIndex + 1
      ], { isItalic: false });
      if (startAngle + endAngle === 180) {
        texture.position.x = (size + 15) * Math.cos(Math.PI - endRad / 2);
        texture.position.y = (size + 5) * Math.sin(Math.PI - endRad / 2) + dataShift2;
        texture1.position.x = (size + 15) * Math.cos(Math.PI - endRad / 2) + dataShift1;
        texture1.position.y = (size + 5) * Math.sin(Math.PI - endRad / 2) + dataShift2;
      }
      // group.add(texture, texture1);
       group.add(texture);
    }
  ...
  // 创建文字及数字字母字符串
  static createText(text: string, position: Array<number> = [0, 0, 0], { color = '#A0E758', isItalic = true } = {}) {
  ...
  static drawSector(startPos: Array<number>, orginPos: Array<number>, endPos: Array<number>, {
    ...
    if (endAngle > 180 && startAngle === 0) {
      startAngle = 360;
    } else if (endAngle > 180 && startAngle > 0 && startAngle < 180) {
  ...
  static getVecPos(startPos: Array<number>, endPos: Array<number>, xiShu: number = 10) {
    const dis = Math.hypot(endPos[0] - startPos[0], endPos[1] - startPos[1]);
    const vecPos = [0, 0, 0];
    vecPos[0] = startPos[0] + (endPos[0] - startPos[0]) / dis * xiShu;
    vecPos[1] = startPos[1] + (endPos[1] - startPos[1]) / dis * xiShu;
    return vecPos;
  }

  static getLineAngle(startPos: Array<number>, endPos: Array<number>, name ?: String) {
    let angle;
  ...
  static sectorShape(pos1: Array<number>, pos2: Array<number>, pos3: Array<number>, pos4: Array<number>,
                     pos5: Array<number>, color = '#0f0') {
    const shape = new THREE.Shape();
    shape.moveTo(0, 0);
    ...
    shape.lineTo(0, 0);
    const G = new THREE.ShapeBufferGeometry(shape, 5);
    const M = new THREE.MeshBasicMaterial({ color: color, transparent: true, depthTest: true });
