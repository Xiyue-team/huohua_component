...
export class CoordinateSystem{
     ...
     * @param endY 
     */
    drawDotLine(endX:number,endY:number):object[]{
        const min = Math.min(this.width,this.height)
        const space = (min - 40) / this.length
        const x1Topx = space * (endX + this.length / 2) + (this.width - min) / 2 + 20
        const y1Topx = space * (this.length / 2 - endY) + (this.height - min) / 2 + 20
        const origin = this.getOrigin();

        const dotLine = {
            points: [origin[0], origin[1], x1Topx, y1Topx],
            stroke: 'black',
            strokeWidth: 2,
            lineJoin: 'round',
            /*
