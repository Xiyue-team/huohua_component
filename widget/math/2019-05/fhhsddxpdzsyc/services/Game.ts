...
export class Game {
  ...
  constructor(canvas: HTMLCanvasElement, formula: any) {
    ...
    //初始化参数
    this.params.addParam('k', new Param(-5, 5, 1));
    this.params.addParam('m', new Param(-5, 5, 2));
  ...
  reset() {
    ...
    this.relation = false;
    this.fg = false;
    this.formula.g.setPoint(new Point(-1));
    this.drawCanvas();
  }
  ...
  drawFormulas(): void {
    ...
    this.formula.g.point.y = this.formula.g.rule(this.formula.g.point.x, this.params) * devicePixelRatio;
    this.formula.fg.point.x = this.formula.g.point.x  * devicePixelRatio;
