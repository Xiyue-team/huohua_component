import {
  ...
  */
  export class SliderControlLine {
    controlPoint: any;
    //Dom的宽高
    private domWidth = document.getElementById('3dModel').getBoundingClientRect().width;
    private domHeight = document.getElementById('3dModel').getBoundingClientRect().height;
    private moveFlag = true;
    dragControls: any;
    private noDrag: boolean;
    private vmCenter: any;
    //构造函数
    /**
     * @param line 直线
     ...
     */
    constructor(controlPoint: any, vmCenter: Jtzh33DModel, noDrag?: boolean) {
    ...
    private initDragEvent(controlPoint: any, camera: Camera, renderer: Renderer) {
        ...
        this.dragControls.addEventListener('dragstart', (obj: any) => {
            ...
            this.vmCenter.downHandle(mousePosArr, object.name);
        });
        this.dragControls.addEventListener('drag', (obj: any) => {
            ...
            mousePosArr.x = object.position.x;
            mousePosArr.y = object.position.y;
            this.vmCenter.moveHandle(mousePosArr, object.name);
        });
        this.dragControls.addEventListener('dragend', (obj: any) => {
            const mousePosArr = {} as any;
            const object = obj.object;
            mousePosArr.x = object.position.x;
            mousePosArr.y = object.position.y;
            this.vmCenter.MouseUp(mousePosArr, object.name);
        });
    }
    ...
    MouseMove(camera: Camera, event: any, name: string) {
        ...
        this.vmCenter.moveHandle(mousePosArr, name);
    }
    //滑点触摸开始事件
