    ...
    function initSlider(){
        ...
        sliderOne = new slider({
            ...
            ondrag:(obj)=>{
                ...
                //此种情况下y = 0, 函数无意义
                if(sliderNumTwo === 0 && sliderNumOne === 0){
                    setPointDisable(0);
                    resetFormulaBtn();
                }else{
    ...
    //设置公式
    function setFormula(){ 
        var value1= sliderNumTwo / 2;
        var param1 = Math.round(value1 * 100) / 100;
        var value2 = sliderNumOne - (sliderNumTwo / 2);
        var param2 = Math.round(value2 * 100) / 100;
        var formulaSymbol = '';
        ...
        var formulaString1 = param1 +'x^2';
        var formulaString2 = formulaSymbol + param2 +'x';
        ...
        if(value2 === 1){
            formulaString2 = formulaSymbol + 'x';
        } else if(value2 === 0){
            formulaString2 = ''; 
        }

        var formulaHtml = formulaString1 + formulaString2;
        var html = katex.renderToString(formulaHtml, {
            throwOnError: false
        });
	  ...
	  function initDesmos(){
        ...
        if(window.innerHeight < 610 || window.innerWidth <610){
            ...
            sequenceImage.style.height  = parseInt(rightWidth * (112 / 282)) + 'px';
            sequenceImage.style.bottom  = parseInt(120 * scale) + 'px';
            formulaDetail.style.width   = parseInt(200 * scale) + 'px';
            formulaDetail.style.height  = parseInt(200 * scale * 107 / 200)  + 'px';
            formulaDetail.style.left    = parseInt(80 * scale)  + 'px';
            formulaDetail.children[0].style.width = parseInt(200 * scale * 107 / 200 * (176 / 107))  + 'px';

            
            formulaBtn.style.transform  = 'scale('+ scale + ')';
            formulaBtn.style.bottom     = parseInt(-formulaBtn.clientHeight * (1- scale) / 2 + (200 * scale * 107 / 400))  + 'px';
    ...
    function createFormulaLine() {
        //函数1
        calculator.setExpression({id: "formula1",latex: "y=\\frac{d}{2}x^2+\\left(a-\\frac{d}{2}\\right)x",color: "#9BF23B",hidden:true});
        //滑条a
		    calculator.setExpression({id: "formula2",latex: "a=1",sliderBounds: { min: "-10", max: "10", step: "0.1" }});
		    //滑条q
		    calculator.setExpression({id: "formula3",latex: "d=-1",sliderBounds: { min: "-10", max: "10", step: "0.1" }});
        //函数1的点1'
		    calculator.setExpression({id: "point1",color: "#fff",latex: "\\left(1,\\frac{d}{2}\\cdot1^2+\\left(a-\\frac{d}{2}\\right)\\cdot1\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: false,showLabel: false});
		    //函数1的点2'
		    calculator.setExpression({id: "point2",color: "#fff",latex: "\\left(2,\\frac{d}{2}\\cdot2^2+\\left(a-\\frac{d}{2}\\right)\\cdot2\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点3'
		    calculator.setExpression({id: "point3",color: "#fff",latex: "\\left(3,\\frac{d}{2}\\cdot3^2+\\left(a-\\frac{d}{2}\\right)\\cdot3\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点4'
		    calculator.setExpression({id: "point4",color: "#fff",latex: "\\left(4,\\frac{d}{2}\\cdot4^2+\\left(a-\\frac{d}{2}\\right)\\cdot4\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点5'
		    calculator.setExpression({id: "point5",color: "#fff",latex: "\\left(5,\\frac{d}{2}\\cdot5^2+\\left(a-\\frac{d}{2}\\right)\\cdot5\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点6'
		    calculator.setExpression({id: "point6",color: "#fff",latex: "\\left(6,\\frac{d}{2}\\cdot6^2+\\left(a-\\frac{d}{2}\\right)\\cdot6\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点7'
		    calculator.setExpression({id: "point7",color: "#fff",latex: "\\left(7,\\frac{d}{2}\\cdot7^2+\\left(a-\\frac{d}{2}\\right)\\cdot7\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点8'
		    calculator.setExpression({id: "point8",color: "#fff",latex: "\\left(8,\\frac{d}{2}\\cdot8^2+\\left(a-\\frac{d}{2}\\right)\\cdot8\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点9'
		    calculator.setExpression({id: "point9",color: "#fff",latex: "\\left(9,\\frac{d}{2}\\cdot9^2+\\left(a-\\frac{d}{2}\\right)\\cdot9\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
		    //函数1的点10'
		    calculator.setExpression({id: "point10",color: "#fff",latex: "\\left(10,\\frac{d}{2}\\cdot10^2+\\left(a-\\frac{d}{2}\\right)\\cdot10\\right)",pointStyle: "POINT",dragMode: "NONE",hidden: true,showLabel: false});
    }
        ...
        formulaBtn.addEventListener("click",(event)=>{
            if(sliderNumOne != 0 && sliderNumTwo != 0){
              isClick = !isClick;
			      ...
			      if(isClick && sliderNumOne != 0 && sliderNumTwo != 0){
