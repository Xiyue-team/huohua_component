...
import {Vector3, WebGLRenderer, Mesh, PerspectiveCamera} from 'three';
...
export class Yzqxdsdy3dModel extends ThreeBase {
    ...
    createtext() {
      ...
      const textk1 = this.planeTexture(k1, 9, 9);
      const textk2 = this.planeTexture(k2, 9, 9);

      const textk3 = this.planeTexture(k1, 9, 9);
      const textk4 = this.planeTexture(k2, 9, 9);
    ...
    createPoint1() {
      ...
      this.point1 = ThreeUtil.createPoint(1.5, '#0199FF', -50, 0, 1);
      this.point1.position.z = 4;
    ...
    createPoint2() {
      ...
      this.point2 = ThreeUtil.createPoint(1.5, '#0199FF', 50, 0, 1);
      this.point2.position.z = 4;
    ...
    createPointM1() {
      ...
      this.pointM1 = ThreeUtil.createPoint(1.5, '#0199FF', 0, 50, 1);
      this.pointM1.position.z = 4;
      this.scene.add(this.pointM1);
    ...
    createPointM2() {
      ...
      this.pointM2 = ThreeUtil.createPoint(1.5, '#0199FF', 0, -50, 1);
      this.pointM2.position.z = 4;

      this.scene.add(this.pointM2);
    ...
    createPoint5() {
      ...
      this.point5 = ThreeUtil.createImg(15, 15, s2, 40, -10 * 4.3);
      const point5 = ThreeUtil.createPoint(1,  '#000', 0, 0 , 0);
      this.point5.add(point5);
      this.scene.add(this.point5);
      this.point5.position.z = 3;
      // this.point5.visible  = false;

      this.dargControls1 = new dragcontrols([this.point5], this.camera, this.renderer.domElement);
      this.dargControls1.addEventListener( 'drag', () => {
        if (!this.canDrag) {
          return;
        }
        if (this.point5.position.x  > 70) {
          this.point5.position.x = 70;
          this.point5.visible = true;
        } else if (this.point5.position.x < -70) {
          this.point5.position.x = -70;
          this.point5.visible = true;
        }
        this.point5.position.y =  - Math.sqrt(920 * (1 + (Math.pow(this.point5.position.x, 2) /
          Math.pow(Math.sqrt(Math.pow(this.pointM2.position.y, 2) - 920), 2))));
        console.log(this.point5.position.y, this.point5.position.x);

        // const y1 = (-22 * (this.point5.position.y / 10)) / ((this.point5.position.x / 10 ) + 3);
        const y1 = ((-25 * (this.point5.position.y / 10 + 3)) / (this.point5.position.x / 10 )) - 3;

        const y2 = ((25 * (this.point5.position.y / 10 + 3)) / (this.point5.position.x / 10 )) - 3;
        this.endM1 = ThreeUtil.createPoint(1, '#000', -250, 10 * y1, 1 );
        this.topM1 = ThreeUtil.createPoint(1, '#000',  250, 10 * y2, 1);

        const y3 = ((-25 * (this.point5.position.y / 10 - 3)) / (this.point5.position.x / 10 )) + 3;
        const y4 = ((25 * (this.point5.position.y / 10 - 3)) / (this.point5.position.x / 10 )) + 3;

        this.endM2 = ThreeUtil.createPoint(1, '#000', -250, 10 * y3, 1);
        this.topM2 = ThreeUtil.createPoint(1, '#000', 250 , 10 * y4, 1);

        this.removeLine(this.line3);
        this.line3 = this.drawLine({x: this.endM1.position.x, y: this.endM1.position.y , z: 1},
          {x: this.topM1.position.x, y: this.topM1.position.y, z: 1}, 1000, '#FF4747');
        this.scene.add(this.line3);

        this.removeLine(this.line4);
        this.line4 = this.drawLine({x: this.endM2.position.x, y: this.endM2.position.y , z: 1},
          {x: this.topM2.position.x, y: this.topM2.position.y, z: 1}, 1000, '#0199FF');
    ...
    createLine3() {
      ...
      this.line3 = createLine.createLine({
        startPoint: new THREE.Vector3(-250, -10 * (25 * ((3 - Math.sqrt(112 / 9)) / 1.6) + 3)  , 1),
        endPoint: new THREE.Vector3(250, 10 * (25 * ((3 - Math.sqrt(112 / 9))  / 1.6) - 3), 1),
        lineWidth: 1000,
        color: '#FF4747',
        lineWidthScale: 1 / 500
    ...
    createLine4() {
      const createLine = new Line();
      this.line4 = createLine.createLine({
        startPoint: new THREE.Vector3(-250, -10 * (25 * ((3 - Math.sqrt(112 / 9)) / 0.29) - 3) , 1),
        endPoint: new THREE.Vector3(250, 10 * (25 * ((3 - Math.sqrt(112 / 9)) / 0.29) + 3), 1),
        lineWidth: 1000,
        color: '#1099FF',
        lineWidthScale: 1 / 500

      });
      this.line4.position.z = 3;
      this.scene.add(this.line4);
    ...
    createslider1() {
      ...
      this.slider1 = ThreeUtil.createPoint(1.5, '#0199FF', -30, 0, 1);
    ...
    createslider2() {
      ...
      this.slider2 = ThreeUtil.createPoint(1.5, '#0199FF', 30, 0, 1);
    ...
    createsliderM1() {
      ...
      this.sliderM1 = ThreeUtil.createPoint(1.5, '#0199FF', 0, 30, 1);
      this.sliderM1.position.z = 3;

      const textS1Pos = commonForThree.createText(`(0,3)`, [70, 45, 0], {'color': '#000', 'isItalic': false, fontSize: 40});
      textS1Pos.position.set(12, -12, 0);

      this.sliderM1.add(textS1Pos);
      this.scene.add(this.sliderM1);
    }

    //创建下面线2与x轴交点slider2
    createsliderM2() {

      this.sliderM2 = ThreeUtil.createPoint(1.5, '#0199FF', 0, -30, 1);
      this.sliderM2.position.z = 3;
      const textS2Pos = commonForThree.createText(`(0,-3)`, [70, 45, 0], {'color': '#000', 'isItalic': false, fontSize: 40});
      textS2Pos.position.set(14, -12, 0);

      this.sliderM2.add(textS2Pos);
      this.scene.add(this.sliderM2);
    ...
    animationImg4() {
      ...
      this.animationimg4 = TweenMax.to(tween, 5, {
        ...
        onComplete: () => {
          this.removePoint(this.pointGroup);
          (window as any).viewHandler.viewModel.$data.disable2 = false;
        },
        paused: true
      });
    ...
    changeFoucsPoint(val: any) {
      ...
      this.f = val;
      this.point1.position.x = -10 * Math.round(Math.sqrt((9 * (this.f * this.f - 1)) + 9));
      this.point2.position.x =  10 * Math.sqrt((9 * (this.f * this.f - 1)) + 9);

      this.stops();

     (window as any).viewHandler.viewModel.$data.disable = false;
     (window as any).viewHandler.viewModel.$data.disable2 = true;
     (window as any).viewHandler.viewModel.$data.color1 = false;
     (window as any).viewHandler.viewModel.$data.color2 = false;
     ...
     if (this.f < 1.33) {
       ...
       this.pointM1.visible = false;
       this.pointM2.visible = false;
       this.sliderM1.visible = false;
       this.sliderM2.visible = false;
       this.point5.visible = false;
    ...
    reset() {
      ...
      this.textk1Pos.text = `= 0.504`;
      this.textk2Pos.text = `= 3.528`;
      this.createLines1();
