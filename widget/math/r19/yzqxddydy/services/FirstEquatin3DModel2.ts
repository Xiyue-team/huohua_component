...
export class FirstEquatin3DModel2 {
  scene: any;
  camera: any;
  renderer: any;
  ...
  // 初始化f点
  initFPoint() {
    const point1 = ThreeUtil.createPoint(1.5, '#0199FF', 0, 40, 1);
    this.firstEquatin.add(point1);

    const point2 = ThreeUtil.createPoint(1.5, '#0199FF', 0, -40, 1);
  ...
  initMPoint() {
    ...
    this.mPoint.position.set(30, 0, 0);
  }
  ...
  initText() {
    ...
    const f1Text = ThreeUtil.createNewRomanText('F₂', -25, -40, 0, '#000000', 0.12);
    this.firstEquatin.add(f1Text);
    const f1Text2 = ThreeUtil.createNormalText('(0, -4)', -13, -40, 0, '#000000', 0.12);
    this.firstEquatin.add(f1Text2);

    const f2Text = ThreeUtil.createNewRomanText('F₁', 5, 40, 0, '#000000', 0.12);
    this.firstEquatin.add(f2Text);
    const f2Text2 = ThreeUtil.createNormalText('(0, 4)', 16, 40, 0, '#000000', 0.12);
    this.firstEquatin.add(f2Text2);
  }

  // 添加红蓝辅助线
  initTwoLine() {
    this.line1 = this.helper.drawLine({x: 0, y: -40, z: 1},
      {x: this.mPoint.position.x, y: this.mPoint.position.y, z: 1}, 3, '#FF4747');

    this.line2 = this.helper.drawLine({x: 0, y: 40, z: 1},
  ...
  initDragEvent() {
    ...
    dargControls1.addEventListener( 'drag',  (pos: any) => {
      ...
      if (pos.object.position.x > 0) {
        pointX = this.sliderPoint(30, 50, pos.object.position.x, pos.object.position.y);
