...
export class ControlBar {
    ...
    updateProgress(currentTime: number , index: number ) {
        ...
        //前一步骤的总时间
        const previousStepTime = this.index === 0 ? 0 : this.stepVideo.steps[this.index - 1].end;
        ...
        if (percent >= 100 ) {
            document.getElementById('jdImg' + this.index).style.left = '93%';
        } else {
            document.getElementById('jdImg' + this.index).style.left = (percent - 7 ).toString() + '%';
        }
    }

    //视频播放时间是不是达到了当前步骤总时间，达到之后视频暂停，设置播放时间从头开始
