...
export class Create3DUtil {
    ...
    addPoint(pointArray: any, height: number) {
        const pyramidPoint = [];
        ...
        for (let i = 0; i < pointArray.length * 2; i += 2 ) {
            ...
            pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 0, height - 100, -pointArray[i / 2][1] * 0);
        }
        pyramidPoint[pointArray.length * 2] = new THREE.Vector3(0, -200 / 2, 0);
        return pyramidPoint;
    }

    addPyramid (pointArray: any, height: number) {
    ...
    addLine(pointArray: any, height: number) {
    ...
    addVertices(pointArray: any, height: number, name: any) {
        ...
        if (name === '棱锥') {
            ...
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, -200 / 2 - 0.5, -pointArray[i / 2][1] * 100.1);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 25.1, height - 99.8, -pointArray[i / 2][1] * 25.1);
            }
        } else if (name === '棱柱') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, -200 / 2 - 0.5, -pointArray[i / 2][1] * 100.1);
