<template>
    <div class="aspectration bg_white root_div_container" data-ratio="16:9">
        ...
        <span id="title" class="title">棱台的构建</span>
...
export default Vue.extend({
    ...
    methods: {
        resetEvent(){
            ( ViewController.getInstance().viewHandler as any).prismModel.reset(this.sliderNum);
            this.sliderNum = 4;
            this.sliderNum4 = 100;
            this.polygon = 1;
        }
    ...
    watch:{
        sliderNum:function (sliderNum:any) {
            ...
            (ViewController.getInstance().viewHandler as any).prismModel.dragPolygon(sliderNum);
            return setTimeout(function () {
                (ViewController.getInstance().viewHandler as any).prismModel.dragChangePyramid(sliderNum);
            },0);
        },

        sliderNum4:function (sliderNum4:number) {
            ( ViewController.getInstance().viewHandler as any).prismModel.dragEventHeight(sliderNum4,this.sliderNum);
        },

        polygon:function (value) {
            let sliderNum = this.sliderNum;
            ( ViewController.getInstance().viewHandler as any).prismModel.judge(Number.parseInt(value) ,sliderNum);
        }
    }
})
</script>
