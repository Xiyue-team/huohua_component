    ...
    <div class="aspectration bg_white root_div_container" data-ratio="16:9">
        <div class="control-panel_div_rt">
            ...
            <div class="control-block_div_border edge_height_slider" style="margin-top:10px;">
                <div class="model_name">模型</div>
                <vue-slider ref="slider4" v-model="model_name" v-bind="modelOption" style="margin:0 auto;position: relative;top:40px;">

                </vue-slider>

                <div class="edge_text">边数</div>
                <vue-slider ref="slider4" v-model="sliderNum" v-bind="sliderOption" style="margin:0 auto;position: relative;top:85px;">

                </vue-slider>

                <div class="height_text">高度</div>
                <vue-slider ref="slider4" v-model="sliderNum4" v-bind="sliderMathOption" style="margin:0 auto; position: relative; top:125px; margin-bottom: 14px;">
        ...
        <span id="title" class="title">棱锥、棱台、棱柱的变化关系</span>
...
export default Vue.extend({
    ...
    methods: {
        resetEvent(){
            ( ViewController.getInstance().viewHandler as any).ztzbhgx.reset(this.sliderNum);
            this.sliderNum = 4;
            this.sliderNum4 = 100;
            this.polygon = 1;
            this.model_name = '棱锥';
    ...
    watch:{
        model_name:function (model_name:any) {
            ( ViewController.getInstance().viewHandler as any).ztzbhgx.dragModelName(model_name, this.sliderNum);
        },
        ...
        sliderNum:function (sliderNum:any) {
            ...
            (ViewController.getInstance().viewHandler as any).ztzbhgx.dragPolygon(sliderNum);
            return setTimeout(function () {
                (ViewController.getInstance().viewHandler as any).ztzbhgx.dragChangePyramid(sliderNum);
            },0);
        },

        sliderNum4:function (sliderNum4:number) {
            ( ViewController.getInstance().viewHandler as any).ztzbhgx.dragEventHeight(sliderNum4,this.sliderNum);
        },

        polygon:function (value) {
            let sliderNum = this.sliderNum;
            ( ViewController.getInstance().viewHandler as any).ztzbhgx.judge(Number.parseInt(value) ,sliderNum);
        }
    }
})
</script>
