...
export class Prism3dModel extends ThreeBase {
    ...
    //棱台上底面的宽度
    private topWidth = 0.01;
    //棱台下底面的宽度
    private bottomWidth = 200;
    //棱台的高度
    private prismHeight = 200;
    ...
    // 模型的种类
    private modelName: any = '棱锥';

    private circlePlan: any;

    private ConeLine: any = [];
    ...
    private render = () => {
        ...
        if (this.orbit.object.position.x < 1.5 && this.orbit.object.position.x > -1.5
            && this.orbit.object.position.y > 55 && this.orbit.object.position.z < 1.5 && this.modelName === '棱锥') {
    ...
    judge(value: number, sliderNum: any) {
        ...
        if (sliderNum !== '+∞') {
            ...
            if (value === 1) {
                ...
                this.pyramid.geometry = this.create3DUtil.addPyramid(pointXY, this.prismHeight, this.modelName);

                this.deleteLine(this.obj[0]);
                this.deleteLine(this.obj[1]);
                this.scene.remove(this.obj[0]);
                this.scene.remove(this.obj[1]);
                this.obj = this.create3DUtil.createLine2(pointXY, this.prismHeight, this.modelName);
    ...
    addPyramid() {
        ...
        const geometry = this.create3DUtil.addPyramid(pointXY, this.prismHeight, this.modelName);
    ...
    // 模型滑块控制模型种类
    dragModelName(name: any, value: any) {
        this.modelName = name;
        if (value === '+∞') {
            if (name === '棱锥') {
                this.topWidth = 0.01;
                ...
                this.topWidth = this.bottomWidth;
            }
            //边数无穷大时   给32个边假装是一个圆
            this.number = 32;
            this.pyramid.geometry.dispose();
            ...
            this.pyramid.geometry.dispose();
            this.pyramid.geometry = this.create3DUtil.addPyramid(this.pointArray, this.prismHeight, this.modelName);

            this.deleteLine(this.obj[0]);
            this.deleteLine(this.obj[1]);
            this.scene.remove(this.obj[0]);
            this.scene.remove(this.obj[1]);
            this.obj = this.create3DUtil.createLine2(this.pointArray, this.prismHeight, this.modelName);
            this.scene.add(this.obj[0]);
            this.scene.add(this.obj[1]);
        }
    }
    ...
    dragChangePyramid(value: any) {
        ...
        if (this.modelName === '棱锥') {
            this.topWidth = 0.01;
        } else if (this.modelName === '棱台') {
            this.topWidth = this.bottomWidth / 2;
        } else if (this.modelName === '棱柱') {
            this.topWidth = this.bottomWidth;
        }
        ...
        if (value === '+∞') {
            ...
            // this.updateLines1th(this.pyramid.geometry);
            this.number = value;
