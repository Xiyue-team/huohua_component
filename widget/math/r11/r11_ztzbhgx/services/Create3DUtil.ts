...
export class Create3DUtil {
    ...
    addPoint(pointArray: any, height: number, name: any) {

        const pyramidPoint = [];
        const length = 0.4;
        if (name === '棱锥') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100, -200 / 2 - length, -pointArray[i / 2][1] * 100);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 0, height - 100 - length, -pointArray[i / 2][1] * 0);
            }
        } else if (name === '棱台') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100, -200 / 2 - length, -pointArray[i / 2][1] * 100);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 50, height - 100 - length, -pointArray[i / 2][1] * 50);
            }
        } else if (name === '棱柱') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100, -200 / 2 - length, -pointArray[i / 2][1] * 100);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 100, height - 100 - length, -pointArray[i / 2][1] * 100);
            }
        }

        pyramidPoint[pointArray.length * 2] = new THREE.Vector3(0, -200 / 2 - length, 0);
        return pyramidPoint;
    }

    addPyramid (pointArray: any, height: number, name: any) {
        const geometry = new THREE.Geometry();

        this.vertices = [];
        this.faces = [];

        this.vertices = this.addPoint(pointArray, height, name);
    ...
    addLine(pointArray: any, height: number, name: any) {

        const vertices = this.addPoint(pointArray, height, name);
    ...
    // 此方法提供根据给出的两个点画一条线
    createLine(startPoint: any, endPoint: any, color: any) {
    ...
    // 添加画边框用的点
    addVertices(pointArray: any, height: number, name: any) {
        const pyramidPoint = [];
        if (name === '棱锥') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, -200 / 2 - 0.5, -pointArray[i / 2][1] * 100.1);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 0, height - 99.5, -pointArray[i / 2][1] * 0);
            }
        } else if (name === '棱台') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, -200 / 2 - 0.5, -pointArray[i / 2][1] * 100.1);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 50.1, height - 99.5, -pointArray[i / 2][1] * 50.1);
            }
        } else if (name === '棱柱') {
            for (let i = 0; i < pointArray.length * 2; i += 2 ) {
                pyramidPoint[i] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, -200 / 2 - 0.5, -pointArray[i / 2][1] * 100.1);
                pyramidPoint[i + 1] = new THREE.Vector3(pointArray[i / 2][0] * 100.1, height - 99.5, -pointArray[i / 2][1] * 100.1);
            }
        }
    ...
    // 此方法同时画实线和虚线
    createLine2(pointArray: any, height: number, name: any) {
        let line;
        let line2;
        let line3;
        let vertices;
    ...
    // 同时画实线圆 和 虚线圆
    createLine3(topRadius: number, bottomRadius: number, color: any, topHeight: number, bottomHeight: number) {
        let line1;
        let line2;
        let line3;
        let line4;
        ...
        line1 = this.createEllipseLine(topRadius + 0.1, color, 3);
        obj1.add(line1);
        line2 = this.createEllipseLine(topRadius + 0.1, color, 2);
        obj2.add(line2);
    ...
    // 生产圆形线
    createEllipseLine(radius: any, color: any, style: number) {
