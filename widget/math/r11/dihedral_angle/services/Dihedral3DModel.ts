...
import {CylinderBufferGeometry, DoubleSide, Geometry, Mesh, WebGLRenderer} from 'three';
...
export class Dihedral3DModel extends ThreeBase {
    ...
    //棱的默认色
    private edgeColor = 0x602205;
    private planeColor = 0x4A90E2;

    //旋转中心点
    centerMesh: Mesh;

    //上面动画开关
    topAnimationEnable: Boolean = false;
    topPlanePosition: Array<number> = [ 2.59, 4.48, 0];


    //下底面动画开关
    bottomAnimationEnable: Boolean = false;
    bottomPlanePosition: Array<number> = [2.6, 1];

    //角度线开关
    lineAnimationEnable: Boolean = false;

    private rightAngleLine: any = [];
    ...
    initCamera(): void {
       ...
       this.camera =  new THREE.OrthographicCamera( window.innerWidth / - 50, window.innerWidth / 50, window.innerHeight / 50,
            window.innerHeight / -50, 1, 1000);
...
/*        const lineEdge = new THREE.EdgesGeometry(arcGeometry, 90);
        ...
        alphaText.material.depthTest = !1;
    ...
    prismAnimation() {
        ...
        if ((window as any).viewHandler.viewModel.$data.animationIsRunning  === true) {
            (window as any).viewHandler.viewModel.$data.animationIsRunning = false;
    ...
    showAngleLine() {
        ...
        const line1 = this.scene.getObjectByName('line1') as Mesh;
        line1.geometry.dispose();
        line1.geometry = new THREE.CylinderBufferGeometry( 0.1, 0.1, 0.5 );
        line1.scale.y = 1;

        const line2 = this.scene.getObjectByName('line2') as Mesh;
    ...
    /** 旋转平面 **/
    rotatePlane(sliderNum: number, currentNum: number) {
        ...
        //修改角度
        if (currentNum === 90) {
            const arcMesh = this.scene.getObjectByName('arcAngle') as Mesh;
        ...
        (angleTxt as SpriteText2D).text = currentNum + '°';
