...
export class LsCanvas extends SimpleKonvaTemplate {
    ...
    ellipseLine2DragEvent() {
      ...
      // 改变实际圆B位置
      this.ellipse2.x(this.ellipseLine2.x());
      this.ellipse2.y(this.ellipseLine2.y());
      // 改变重叠色位置
      this.overlappingColors.x(this.ellipseLine2.x());
      this.overlappingColors.y(this.ellipseLine2.y());

      this.textB.x(this.ellipseLine2.x() - this.config.textB.fontSize / 2);
      this.textB.y(this.ellipseLine2.y() - this.config.textB.fontSize / 2);
    }
