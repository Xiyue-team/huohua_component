...
import pai from '../sub_static/pai.png';
...
export default class FormulaFourCanvas {
    ...
    // 添加直角
    initRightAngle() {
      const radius = 30 * this.config.scale;

      const x1 = radius * Math.sin((45) * Math.PI / 180) + this.config.width * 0.343;
      const y1 = this.config.height * 0.5 - radius * Math.cos((45) * Math.PI / 180);

      const x2 = radius * Math.sin((135) * Math.PI / 180) + this.config.width * 0.343;
      const y2 = this.config.height * 0.5 - radius * Math.cos((135) * Math.PI / 180);

      const radius2 = Math.sqrt(2) * radius;

      const x0 = radius2 * Math.sin((90) * Math.PI / 180) + this.config.width * 0.343;
      const y0 = this.config.height * 0.5 - radius2 * Math.cos((90) * Math.PI / 180);

      const line1 = new fabric.Line([x0, y0, x1, y1], this.config.rightAngle);
      const line2 = new fabric.Line([x0, y0, x2, y2], this.config.rightAngle);

      const x3 = radius * Math.sin((-45) * Math.PI / 180) + this.config.width * 0.343;
      const y3 = this.config.height * 0.5 - radius * Math.cos((-45) * Math.PI / 180);

      const x4 = radius * Math.sin((-135) * Math.PI / 180) + this.config.width * 0.343;
      const y4 = this.config.height * 0.5 - radius * Math.cos((-135) * Math.PI / 180);

      const x5 = radius2 * Math.sin((-90) * Math.PI / 180) + this.config.width * 0.343;
      const y5 = this.config.height * 0.5 - radius2 * Math.cos((-90) * Math.PI / 180);

      const line3 = new fabric.Line([x5, y5, x3, y3], this.config.rightAngle);
      const line4 = new fabric.Line([x5, y5, x4, y4], this.config.rightAngle);
      line3.set('visible', false);
      line4.set('visible', false);

      this.rightAngle = new fabric.Group([line1, line2, line3, line4], {selectable: false, visible: true});
      this.group.push(this.rightAngle);
    }
    ...
    async initText() {
      ...
      this.paiText = new fabric.Text('-α', this.config.paiText as any);
      this.group.push(this.paiText);
      this.pai2Text = await FabricUtil.loadImage(pai as any, this.config.pai);
      this.group.push(this.pai2Text);
    }

    initDragEvent() {
      FabricUtil.limitButtonScope(60, this.smallCircleDragPoint, this.config.circleLine.radius,
    ...
    updateLine(angle: number) {
      ...
      this.helper.updateBlueLine(this.blueLine, 90 + (90 - angle) - 90, this.config.circleLine.radius);
      this.helper.updateYellowLine(this.yellowLine, 90 + (90 - angle) - 90, this.config.circleLine.radius);
      this.helper.updateVioletLine(this.violetDottedLine, 90 + (90 - angle) - 90, this.config.circleLine.radius * 5);
      this.helper.updateVioletLine(this.violetSolidLine2, 90 + (90 - angle), this.config.circleLine.radius * 5);

      this.rightAngle.rotate((90 - angle) - 45);
    }
    ...
    updateText(angle: number) {
      ...
      this.helper.updateText(this.aText, 90 - (90 - angle) / 2, 20 * this.scale + (90 - angle) * 0.04 * this.scale);
      this.helper.updateText(this.paiText, 90 + (90 - angle) / 2, 20 * this.scale + (90 - angle) * 0.04 * this.scale);
      this.helper.updateText(this.pai2Text, 90 + (90 - angle) - 45, 60 * this.config.scale);
    }
