...
export default class FormulaOneCanvas {
    ...
    initLine() {
      ...
      this.violetDottedLine.set('visible', false);
    ...
    initText() {
      ...
      this.paiText = new fabric.Text('2π', this.config.paiText as any);
    ...
    initDragEvent() {
      ...
      this.smallCircleDragPoint.on({'moving': (e: any) => {
          ...
          this.updateScrewBlueLine(angle, 360);
    ...
    updateLine(angle: number) {
      ...
      this.helper.updateBlueLine(this.blueLine, angle, this.config.circleLine.radius);
      this.helper.updateYellowLine(this.yellowLine, angle, this.config.circleLine.radius);
      ...
      if (angle < 0 && this.greenLine.get('visible')) {
        this.violetDottedLine.set('visible', true);
      } else {
        this.violetDottedLine.set('visible', false);
      }
    }
