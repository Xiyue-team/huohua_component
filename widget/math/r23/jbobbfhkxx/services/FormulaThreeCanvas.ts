...
export default class FormulaThreeCanvas {
    ...
    updateLine(angle: number) {
      ...
      this.helper.updateBlueLine(this.blueLine, 90 + (90 - angle), this.config.circleLine.radius);
      this.helper.updateYellowLine(this.yellowLine, 90 + (90 - angle), this.config.circleLine.radius);
      this.helper.updateVioletLine(this.violetDottedLine, 90 + (90 - angle), this.config.circleLine.radius * 5);
      this.helper.updateVioletLine(this.violetSolidLine2, 90 + (90 - angle) - 180, this.config.circleLine.radius * 5);

      if (angle < 0 && this.greenLine.get('visible')) {
        this.violetSolidLine2.set('visible', true);
      } else {
        this.violetSolidLine2.set('visible', false);
      }
    }
    ...
    updateText(angle: number) {
      ...
      this.helper.updateText(this.paiText, 90 + (90 - angle) / 2, 18 * this.scale + (90 - angle) * 0.04 * this.scale);
    }
