<template>
  <div class="aspectration bg_white root_div_container" data-ratio="16:9">
    <fullScreensLayout>
      <template slot="viewBox">
        <div class="title_text">{{title_text}}</div>
        <div id="Container" style="width: 100%; height: 100%;">
          <div id="Container3d">
            <canvas
              id="renderCanvas"
              touch-action="none"
              width="1920"
              height="1080"
              tabindex="1"
              style="opacity: 1;"
            ></canvas>
          </div>
                  <div class="buttonG">
          <div class="button_style1" @click="ButtonEvent(1)">
            <buttonPrimary
              style="width: 100%"
              v-bind:title="buttonTitle[0]"
              v-bind:actived="buttonActived===1"
              type="ellipse"
            ></buttonPrimary>
          </div>
          <div id="tip" v-if="buttonActived === 1" :style="`background-image: url(${pictip})`"></div>
          <div class="button_style2" @click="ButtonEvent(2)">
            <buttonPrimary
              style="width: 100%"
              v-bind:title="buttonTitle[1]"
              v-bind:actived="buttonActived===2"
              type="ellipse"
            ></buttonPrimary>
          </div>
          <div class="button_style2" @click="ButtonEvent(3)">
            <buttonPrimary
              style="width: 100%"
              v-bind:title="buttonTitle[2]"
              v-bind:actived="buttonActived===3"
              type="ellipse"
            ></buttonPrimary>
          </div>
        </div>
        </div>

      </template>

      <template slot="controlPanel">
        <div id="controlPanel"></div>
      </template>
    </fullScreensLayout>
  </div>
</template>

<script lang="ts">
import Vue from 'vue';
import '../../../../src/assets/css/core.css';
import '../../../../src/assets/css/layout.css';
import fullScreensLayout from '../../../../src/component/layout/fullScreens_layout.vue';
import Component from 'vue-class-component';
import { ViewModel } from './ViewModel';
import buttonPrimary from '../../../babylon/template/ui/buttonPrimary.vue';
@Component({
  components: {
    fullScreensLayout,
    buttonPrimary
  },
  mixins: [ViewModel]
})
export default class App extends Vue {}
</script>

<style scoped="scoped">
body {
  overflow: hidden !important;
}
#renderCanvas {
  width: 100%;
  height: 100%;
  outline: 0;
}
.title_text {
  font-size: 24px;
  line-height: 24px;
  color: #fff;
  position: absolute;
  top: 24px;
  left: 24px;
  z-index: 3;
  pointer-events: none;
}

#Container3d{
  position: absolute;
  left: 0;
  right: 324px;
  height: 100%;
}
.buttonG {
  position: absolute;
  width: 200px;
  height: 102px;
  bottom: 50%;
  right: 24px;
}
.button_style1 {
  margin-top: 51px;
  width: 100%;
  height: 42px;
}

.button_style2 {
  width: 100%;
  height: 42px;
  margin-top: 18px;
}

#tip {
  width: 300px;
  height: 125px;
  margin-top: 18px;
  margin-left: -100px;
  background-repeat: no-repeat;
  background-position: right top;
  background-size: contain;
  pointer-events: none;
}

@media screen and (min-height: 600px) and (max-height: 800px) {
  .buttonG {
    right: 24px;
  }
  #Container3d{
    right: 324px;
  }
}

@media screen and (min-height: 450px) and (max-height: 599px) {
 .buttonG {
    top: 80px;
  }
  #Container3d{
    right: 324px;
  }
}

@media screen and (min-height: 300px) and (max-height: 449px) {
  #Container3d{
    right: 224px;
  }
  .buttonG {
    width: 160px;
    top:80px;
    right: 24px;
  }
  .button_style1 {
    height: 28px;
    margin-top: 0px;
  }

  .button_style2 {
    height: 28px;
    margin-top: 8px;
  }

  #tip {
    width: 200px;
    margin-left: -40px;
    height: 80px;
    margin-top: 8px;
  }
}

#Container {
  background-color: #333333;
}

</style>

<style>
/*右侧面板*/
.control-panel_div_floatRight {
 padding: 0px !important;
}
</style>
