<template>
    <div class="aspectration bg_white root_div_container" data-ratio="16:9">
        <div class="control-panel_div_rt">
            ...
            <span id="cylinder" class="cylinder">241.89</span>
            <span id="busbarLength" class="busbarLength">5.39</span>
            <div class="gongshi">
                <img src="./sub_static/gongshidingli.png"/>
            </div>
        </div>

        <span class="title">圆台的表面积</span>
...
export default Vue.extend({
    ...
    methods: {
        async openEvent(){
            ...
            const animation = await  (ViewController.getInstance().viewHandler as any).table3dModel.openEvent(this.sliderNum);
            (ViewController.getInstance().viewHandler as any).table3dModel.showLine();
        },

        resetEvent(){

            (ViewController.getInstance().viewHandler as any).table3dModel.reset();
            this.sliderNum = [3 , 6];
            this.sliderNum4 = 10;
        }
    },
    watch:{
        sliderNum:function (sliderNum: number) {
            (ViewController.getInstance().viewHandler as any).table3dModel.dragCircleGeometry(sliderNum, this.sliderNum4);
            (ViewController.getInstance().viewHandler as any).table3dModel.changeSlider(sliderNum, this.sliderNum4);
        },

        sliderNum4:function (sliderNum4:number) {
//            (ViewController.getInstance().viewHandler as any).prismStructure.dragHeight(sliderNum4);
            (ViewController.getInstance().viewHandler as any).table3dModel.dragCircleGeometry(this.sliderNum, sliderNum4);
            (ViewController.getInstance().viewHandler as any).table3dModel.changeSlider(this.sliderNum, sliderNum4);
        },
