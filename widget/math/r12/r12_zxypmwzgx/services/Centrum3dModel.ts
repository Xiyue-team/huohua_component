...
export class Centrum3dModel extends ThreeBase {
   ...
   /* cameraControl: boolean = false;*/
    ...
    initCamera(): void {
        ...
        this.camera.position.set(-30,  50,  40);
    }
    ...
    resetCamera() {
        ...
        this.orbit.object.position.set(-30,  23,  33);
        this.orbit.reset();
    }
    ...
    * */
    ...
    //页面初始场景
    initThreeDScene() {
        this.geometryLine = new THREE.CylinderBufferGeometry( 1, 1, 250 );
        this.materialLine = new THREE.MeshBasicMaterial( {color: 0xff1f3a} );
        this.line =  new THREE.Mesh( this.geometryLine, this.materialLine );
        this.scene.add(this.line);
        this.line.add(this.text);
        this.line.rotateX(Math.PI / 180 * 90);
        this.line.rotateZ(Math.PI / 180 * -45);
        this.Geometry = new THREE.PlaneBufferGeometry( 440, 440, 32 );
        this.Material = new THREE.MeshBasicMaterial( {color: 0x4a90e2, transparent: true, opacity: 0.4, side: THREE.DoubleSide} );
    ...
    createWord() {
        const text1 = this.createText('α' , -200, 30 , 200, '#000000');
        text1.material.depthTest = false;
        this.scene.add(text1);
        this.text = this.createText('l', 0, -110, -20, '#ff1f3a');
