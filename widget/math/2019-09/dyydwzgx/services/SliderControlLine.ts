...
export class SliderControlLine {
    controlPoint: any;
    private Line3dModelCavas: Line3dModel;
    //Dom的宽高
    ...
    setCanvas(Line3dModelCavas: Line3dModel) {
        this.Line3dModelCavas = Line3dModelCavas;
    }
    //初始化滑点鼠标事件
    ...
    private initDragEvent(controlPoint: any, camera: Camera, renderer: Renderer) {
        ...
        this.dragControls.addEventListener('dragstart', (obj: any) => {
            this.Line3dModelCavas.downHandle(obj.object.name);
        });
        this.dragControls.addEventListener('drag', (obj: any) => {
            ...
            this.Line3dModelCavas.moveHandle(mousePosArr, object.name);
        });
    ...
    MouseMove(camera: Camera, event: any, name: string) {
        ...
        this.Line3dModelCavas.moveHandle(mousePosArr, name);
    }
    //滑点触摸开始事件
