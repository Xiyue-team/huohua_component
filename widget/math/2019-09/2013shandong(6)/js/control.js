     ...
     (function() {
        ...
        function initStyle(){
            //初始化题目的高度
            var height = window.innerWidth * (224 / 1200 );
            ...
            for(var index=0; index < leftContainer.children.length - 1; index++){
                leftContainer.children[index].style.height = window.innerWidth * 0.38 * (196 / 460) + 'px';
                leftContainer.children[index].style.marginTop = window.innerHeight * 0.36 + 'px';
            }            
            vueElement = new Vue({
                el: '#app',
                data: {
                    ...
                    switchModel : function(value){
                        ...
                        switch (value){
                            case switchDataText[0]:
                                ...
                                calculator.setExpression({id: "point3",hidden:true,showLabel: false});
                            ...
                            case switchDataText[1]: 
                                ...
                                calculator.setExpression({id: "point3",hidden:false,showLabel: true});
        ...
        function createFormulaLine() {  
            ...
            //函数区域1
            calculator.setExpression({id: "area1",color: "#00AAFF",latex: "2x-y-2\\ge0",hidden: false,});
            //函数区域2
            calculator.setExpression({id: "area2",color: "#8BC052",latex: "x+2y-1\\ge0",hidden: false,});
            //函数区域3
            calculator.setExpression({id: "area3",color: "#FFD621",latex: "3x+y-8\\le0",hidden: false,});
            //函数区域4
            calculator.setExpression({id: "area4",color: "#ffffff",latex: "\\operatorname{polygon}\\left(\\left(2,2\\right),\\left(3,-1\\right),\\left(1,0\\right)\\right)",});
        
            //函数1
            calculator.setExpression({id: "function1",color: "#00AAFF",latex: "2x-y-2=0",lineStyle: "SOLID", hidden:false});
            //函数2
            calculator.setExpression({id: "function2",color: "#8BC052",latex: "x+2y-1=0",lineStyle: "SOLID", hidden:false});
            //函数3
            calculator.setExpression({id: "function3",color: "#FFD621",latex: "3x+y-8=0",lineStyle: "SOLID", hidden:false});
            //函数4
            calculator.setExpression({id: "function4",color: "#FF5A5A",latex: "y=zx",lineStyle: "DASHED", hidden:true});

            //公式点1
            calculator.setExpression({id: "functionPoint1",color: "#00AAFF",latex: "\\left(4.5,5\\right)",label:katex.renderToString('2x-y-2=0', {throwOnError: false}),hidden:true,showLabel: true});
            //公式点2
            calculator.setExpression({id: "functionPoint2",color: "#8BC052",latex: "\\left(-6,4.5\\right)",label:katex.renderToString('x+2y-1=0', {throwOnError: false}),hidden:true,showLabel: true});
            //公式点3
            calculator.setExpression({id: "functionPoint3",color: "#FFD621",latex: "\\left(0,5\\right)",label:katex.renderToString('3x+y-8=0', {throwOnError: false}),hidden:true,showLabel: true});

            //圆点1
            calculator.setExpression({id: "point1",color: "#fff",latex: "\\left(2,2\\right)",label:katex.renderToString('A', {throwOnError: false}),hidden:true,showLabel: false});
            //圆点2
            calculator.setExpression({id: "point2",color: "#fff",latex: "\\left(3,-1\\right)",label:katex.renderToString('B', {throwOnError: false}),hidden:true,showLabel: false});
            //圆点3
            calculator.setExpression({id: "point3",color: "#fff",latex: "\\left(1,0\\right)",label:katex.renderToString('C', {throwOnError: false}),hidden:true,showLabel: false});
            //圆点4
            calculator.setExpression({id: "point4",color: "#fff",latex: "\\left(0.4,0.4z\\right)",hidden:true,showLabel: false});
        ...
        function checkSliderNumber(){
            ...
            sliderObject.observe("numericValue", function() {
                //为了突出答案，做吸附显示    
                if(sliderObject.numericValue > -0.45 && sliderObject.numericValue < -0.25){
                    calculator.setExpression({id: "slider1",latex: "z = -0.333333333333"}); //数值是1/3
                    document.querySelector('.bottomcontrol').innerHTML = katex.renderToString('-\\frac 13', {throwOnError: false});
                    document.querySelector('.bottomcontrol').style.fontSize = '24px';
                }else{
                    document.querySelector('.bottomcontrol').innerHTML = sliderObject.numericValue;
                    document.querySelector('.bottomcontrol').style.fontSize = '18px';
                }
            });
        }
