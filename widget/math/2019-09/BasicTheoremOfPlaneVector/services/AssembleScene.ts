...
import Vue from 'vue';
import {
    Color3, Vector3, LinesMesh, Mesh, Scene, AbstractMesh, Engine, Vector2
...
export class AssembleScene extends Base2DScene implements VectorMoveHandler {
    ...
    vectorAArrowPos = new Vector3(8, 7, 0);

    vectorE1: Vector; //向量e1
    vectorE1Pos = new Vector3(-3, 1, 0);
    vectorE1ArrowPos = new Vector3(9, 12, 0);

    vectorE2: Vector; //向量e2
    vectorE2Pos = new Vector3(-1, -2, 0);
    vectorE2ArrowPos = new Vector3(8, -10, 0);
    ...
    initValue(advancedTexture: GUI.AdvancedDynamicTexture, scene: Scene) {
        ...
        const lightmaterialA = MaterialLab.CreateLightMaterial(this.colorBlue, scene);
        ...
        this.vectorE1 = new Vector('tipE1', this.edgesWidth, scene, this, null, tempMesh)
            ...
            .initValue(this.vectorE1Pos, this.vectorE1ArrowPos, this.colorBlue, lightmaterialA)
        ...
        this.vectorE2 = new Vector('tipE2', this.edgesWidth, scene, this, null, tempMesh)
            .setAdvancedDynamicTexture(advancedTexture)
            .initValue(this.vectorE2Pos, this.vectorE2ArrowPos, this.colorBlue, lightmaterialA)
            .initDotLabel(dot, ImageLabelOption)
        ...
        if (currentMesh.name.indexOf('tip') !== -1) {
            if (currentMesh === this.vectorE1.getArrow()) {
                this.vectorE1.updateDataWithAngle(currentMesh, startingPoint, this.vectorE2.getArrowPos(),
                    this.orthoX, this.orthoY, this.offset);
                ...
                this.vectorE1.updateDataWithLength(currentMesh, startingPoint, this.orthoX, this.orthoY, this.offset);
                this.vectorE2.updateDataWithLength(currentMesh, startingPoint, this.orthoX, this.orthoY, this.offset);
            }
    ...
    updateVector(): void {
        ...
        const r1 = re1.length() / this.vectorE1.getArrowPos().length();
        const r2 = re2.length() / this.vectorE2.getArrowPos().length();
        this.r1 = Vector3.Dot(re1, this.vectorE1.getArrowPos()) < 0 ? '-' + r1.toFixed(2) : r1.toFixed(2);
        this.r2 = Vector3.Dot(re2, this.vectorE2.getArrowPos()) < 0 ? '-' + r2.toFixed(2) : r2.toFixed(2);
        this.r1 = this.r1 === '1.00' ? '1' : this.r1 === '-1.00' ? '-1' : this.r1;
        this.r2 = this.r2 === '1.00' ? '1' : this.r2 === '-1.00' ? '-1' : this.r2;
        this.setR1Text(`${this.r1 === '0.00' || this.r1 === '-0.00' ? '0' : this.r1}`);
        this.setR2Text(`${this.r2 === '0.00' || this.r2 === '-0.00' ? '0' : this.r2}`);
    }
    ...
    moveEnd(v: Vector): void {
        ...
        if (this.hasFinished === 3) {
            ...
            this.lineV = this.line.isVisible = this.line2.isVisible = true;
            this.updateVector();
            this.hasFinished = 0;
        }
    }
     ...
     * 关系按钮
     */
    buttonImageEvent2(): void {
        if (this.viewModel.buttonImageActived2) {
            this.vectorA.setPickAble(false).setRootLabelVisible(false).setArrowLabelVisible(false);
            this.vectorE1.setPickAble(false).setRootLabelVisible(false).setArrowLabelVisible(false);
            this.vectorE2.setPickAble(false).setRootLabelVisible(false).setArrowLabelVisible(false);
            ...
            this.lineV = this.line.isVisible = this.line2.isVisible = false;
            this.vectorA.notCheck().setPickAble(true).setRootLabelVisible(true).setArrowLabelVisible(true).updateRotation();
            this.vectorE1.notCheck().setPickAble(true).setRootLabelVisible(true).setArrowLabelVisible(true).updateRotation();
            this.vectorE2.notCheck().setPickAble(true).setRootLabelVisible(true).setArrowLabelVisible(true).updateRotation();
        }
    }
     ...
     */
    reset(): void {
        this.viewModel.buttonImageActived2 = false;
        this.vectorRE1.setPickAble(false).setArrowPickAble(false).setVisible(false).setLabelVisible(false);
        this.vectorRE2.setPickAble(false).setArrowPickAble(false).setVisible(false).setLabelVisible(false);
        this.lineV = this.line.isVisible = this.line2.isVisible = false;
    ...
    setR1Text(text: string) {
        this.viewModel.r1text = text;
    }

    setR2Text(text: string) {
        this.viewModel.r2text = text;
    }
}
