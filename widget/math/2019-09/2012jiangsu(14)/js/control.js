     ...
     (function() {
        ...
        function initStyle(){
            //初始化题目的高度
            var height = window.innerWidth * (182 / 1200 );
            ...
            for(var index=0; index < leftContainer.children.length - 1; index++){
                leftContainer.children[index].style.height = window.innerWidth * 0.38 * (456 / 459) + 'px';
                leftContainer.children[index].style.marginTop = window.innerHeight * 0.07 + 'px';
            }
        }
        ...
        function createFormulaLine() {  
            ...
            //函数区域1
            calculator.setExpression({id: "area1",color: "#FFD621",latex: "y-e^x\\ge0",hidden: false,});
            //函数区域2
            calculator.setExpression({id: "area2",color: "#00AAFF",latex: "3x+y-5\\ge0",hidden: false,});
            //函数区域3
            calculator.setExpression({id: "area3",color: "#9BF23B",latex: "x+y-4\\le0",hidden: false,});
            //函数区域4
            calculator.setExpression({id: "area4",color: "#ffffff",latex: "e^{x}\\le y\\le4-x\\left\\{5-3x<e^{x}\\right\\}",});
            //函数区域5
            calculator.setExpression({id: "area5",color: "#ffffff",latex: "5-3x\\le y\\le4-x\\left\\{5-3x>e^{x}\\right\\}",});
        
            //函数1
            calculator.setExpression({id: "function1",color: "#FFD621",latex: "y=e^x",lineStyle: "SOLID", hidden:false});
            //函数2
            calculator.setExpression({id: "function2",color: "#00AAFF",latex: "3x+y-5=0",lineStyle: "SOLID", hidden:false});
            //函数3
            calculator.setExpression({id: "function3",color: "#9BF23B",latex: "x+y-4=0",lineStyle: "SOLID", hidden:false});
            //函数4
            calculator.setExpression({id: "function4",color: "#FF5A5A",latex: "zx=y",lineStyle: "DASHED",hidden:true});

            //公式点1
            calculator.setExpression({id: "functionPoint1",color: "#FFD621",latex: "\\left(2.5,6\\right)",label:katex.renderToString('y=e^x', {throwOnError: false}),hidden:true,showLabel: true});
            //公式点2
            calculator.setExpression({id: "functionPoint2",color: "#00AAFF",latex: "\\left(4.5,-5\\right)",label:katex.renderToString('3x+y-5=0', {throwOnError: false}),hidden:true,showLabel: true});
            //公式点3
            calculator.setExpression({id: "functionPoint3",color: "#9BF23B",latex: "\\left(-3,6\\right)",label:katex.renderToString('x+y-4=0', {throwOnError: false}),hidden:true,showLabel: true});

            //圆点1
            calculator.setExpression({id: "point1",color: "#fff",latex: "\\left(0.5,3.5\\right)",label:katex.renderToString('A(\\frac12,\\frac72)', {throwOnError: false}),hidden:true,showLabel: false});
            //圆点2
            calculator.setExpression({id: "point2",color: "#fff",latex: "\\left(1,e\\right)",label:katex.renderToString('P(1,e)', {throwOnError: false}),hidden:true,showLabel: false});      
            //圆点4
            calculator.setExpression({id: "point4",color: "#fff",latex: "\\left(0.2,0.2z\\right)",dragMode:'AUTO',hidden:true,showLabel: false});
        ...
        function checkSliderNumber(){
            ...
            sliderObject.observe("numericValue", function() {
                //为了突出答案，做吸附显示    
                if(sliderObject.numericValue > 6 && sliderObject.numericValue < 8){
                    calculator.setExpression({id: "slider1",latex: "z = 7"});
                    document.querySelector('.bottomcontrol').innerHTML = sliderObject.numericValue;
                }  
                else if(sliderObject.numericValue >2 && sliderObject.numericValue < 3){
                    calculator.setExpression({id: "slider1",latex: "z = 2.71828182846"}); //e 的精确值
                    document.querySelector('.bottomcontrol').innerHTML = 'e';
                }
                else
                document.querySelector('.bottomcontrol').innerHTML = sliderObject.numericValue;
            });
