...
export class DashLine {
  ...
  addLine(pointArr: any, color: any, linewidth: number, dashLine: any) {
      const contentW = window.innerWidth * 0.25;
      ...
      const resolution = new THREE.Vector2(window.innerWidth - contentW, window.innerHeight - 144);
      let  matLine = null;

      const parr = [];
      parr.push( pointArr[0].x);
      parr.push( pointArr[0].y);
      parr.push( pointArr[0].z);
      parr.push( pointArr[1].x);
      parr.push( pointArr[1].y);
      parr.push( pointArr[1].z);
    // @ts-ignore
    const geometryLine = new THREE.LineGeometry();
           geometryLine.setPositions(parr);
      ...
      // @ts-ignore
      const lineMesh = new THREE.Line2(geometryLine, matLine);
