...
export default class CommonForThree {
    ...
    // 缩放线段
    static scaleLine(startPos: any, endPos: any, line: THREE.Mesh, isCube: boolean = true, proportion: number = 1) {
        ...
        if (isCube === false) {
            const radX = Math.atan2(z - z1, y - y1);
            line.rotation.x = radX;
        }
    ...
    //画虚线
    static drawDresh(x: any, x1: any, color: any) {
        const G = new THREE.Geometry();
        G.vertices.push(new THREE.Vector3(x, 0, 0));
        G.vertices.push(new THREE.Vector3(x1, 0, 0));
    ...
    static createStrokeCircle(radius: number, { color = '#000' } = {}) {
        ...
        const line = CommonForThree.drawDashOrLine(vertices, { color: '#ffa400', width: 1, isDash: false});
        group.add(line);
        return group;
    }
    //中点坐标公式
