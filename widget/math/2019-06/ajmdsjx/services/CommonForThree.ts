...
export default class CommonForThree {
  ...
  //创建面
  static creatPlane(vertices: any, w: number, h: number, {color = '#282828'} = {}) {
  ...
  static createStrokeCircle(radius: number, { color = '#000' } = {}) {
    ...
    const line = CommonForThree.drawDashOrLine(vertices, { color, width: 0.5 });
    group.add(line);
    return group;
  }
  ...
  static midPos(startPos: any, endPos: any) {
    ...
    const midPos: Array<number> = [0, 0, 0];
    midPos[0] = disX / 2;
    midPos[1] = disY / 2;
  ...
  //文字
  static createText(text: string, position: Array<number> = [0, 0, 0], { color = '#fff', isItalic = true } = {}) {
