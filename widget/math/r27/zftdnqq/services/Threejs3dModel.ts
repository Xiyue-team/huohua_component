...
export class Threejs3dModel extends ThreeBase {
  ...
  // 添加完整球和两个半球
  addBall() {
    const frontSphereGeometry = new THREE.SphereBufferGeometry( 50, 32, 32, 0, Math.PI );
    const material = new THREE.MeshPhongMaterial({color : '#ff7c77', specular: '#ffcbc9',  side: THREE.DoubleSide});
    this.frontSphere = new THREE.Mesh( frontSphereGeometry.clone(), material.clone() );
    this.scene.add( this.frontSphere );

    const afterSphereGeometry = new THREE.SphereBufferGeometry( 50, 32, 32, 0, -Math.PI );
    ...
    this.sphere = new THREE.Mesh( sphereGeometry.clone(), material.clone());
  ...
  addDashLine() {
    // 添加虚线
    this.dashLine = (new DrowBorderLineHelper()).createDashLine(100, 0.25, '#000');
    this.dashLine.rotateZ(Math.PI / 2);
