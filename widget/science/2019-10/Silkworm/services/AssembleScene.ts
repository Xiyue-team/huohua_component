...
import * as silkworm from '../sub_static/silkworm.glb';
...
export class AssembleScene extends Base3DScene {
    ...
    createScene(engine: Engine): Scene {
        ...
        this.camera.position = new Vector3(0, 0, -4);
        scene.activeCameras.push(this.camera);
        
        this.dD3Control = new D3Control(this.option);
        SceneLoader.ImportMesh('', silkworm.replace('silkworm.glb', ''), 'silkworm.glb', scene, (v) => { this.importMeshSuccess(v); });
        this.reset();
        return scene;
    }
