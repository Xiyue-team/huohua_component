...
export class TemplateViewHandler extends CommonViewHandler implements ViewHandler {
    ...
    resize():  void {
        ...
        const scaleX = window.innerWidth / (918);
        const scaleY = window.innerHeight / (534);
        if ((534) * scaleX <= window.innerHeight) {

          this.titleContent.style.transform = 'scale(' + (scaleX + 0.1) + ')';
          this.show_posterContent.style.transform = 'scale(' + (scaleX - 0.2) + ')';

        } else if ((918) * scaleY <= window.innerWidth) {

          this.titleContent.style.transform = 'scale(' + (scaleY + 0.1) + ')';
          this.show_posterContent.style.transform = 'scale(' + (scaleY - 0.2) + ')';
