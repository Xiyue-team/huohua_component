...
export class Dhdc2dModel {
    ...
    constructor(container: string) {
        ...
        this.initLayer();
        this.chargeArray.push(new PointCharge(1, {
        ...
        for (let i = 0; i < chargeArray.length; i++) {
            ...
            imageObj.onload = () => {
                ...
                charge.on('dragmove', (e) => {
                    ...
                    const obj = this.getPositonAndJudgeIsDrage(target.id(), target.x(), target.y());
                    if (obj.isDrage) {
                        // 设置点的新坐标
                        this.chargeArray[obj.index].setPosition({x: Number(target.x()), y:  Number(target.y())});
                        this.drawChangeLines();
                    } else {
                        target.setAttrs({
                            x: obj.x,
    ...
    private getPositonAndJudgeIsDrage(id: string, x: number, y: number) {
        ...
        for (let i = 0; i < this.chargeArray.length; i++) {
            ...
            const distance = Math.abs(Math.sqrt(Math.pow(x - positon.x, 2) + Math.pow(y - positon.y, 2)));
            if (distance < 40 || x < 10 || y < 10 || x > window.innerWidth - 10 || y > window.innerHeight - 10) {
                isDrage = false;
            }
        }
    ...
    private drawChangeLines() {
        ...
        for (let i = 0; i < chargeArray.length; i++) {
            ...
            for (let j = 0; j < 12; j++) {
                ...
                // 计算线段终点
                let startX = position.x + polarity * distanceLine * Math.cos($sm);
                let startY = position.y + polarity * distanceLine * Math.sin($sm);
                let e = this.calcElectricFieldStrength({x: startX, y: startY});
                // 计算改点到其他各点的电势
                let index = 0;
                while (startX >= -20 && startY >= -20 && startX <= this.stage.width() + 20 && startY <= this.stage.height() + 20) {
                    const endX = startX + polarity * distanceLine * e.fieldStrengthX / e.fieldStrength;
                    const endY = startY + polarity * distanceLine * e.fieldStrengthY / e.fieldStrength;
    ...
    private calcElectricFieldStrength(point: Position) {
        ...
        for (let i = 0; i < chargeArray.length; i++) {
            ...
            const r = Math.sqrt((Math.pow((point.x - position.x), 2)) + (Math.pow((point.y - position.y), 2))); 
             //电场强度  E=kQ/(r*r)
            const e = this.electrostaticForce * currentElement.getChargeStrength() * currentElement.getPolarity()
             / (Math.pow(r, 2));
