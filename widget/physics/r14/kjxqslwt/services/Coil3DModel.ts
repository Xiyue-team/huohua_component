...
import * as xqBin from '../sub_static/photo/xianquan.bin';
import * as xqGltf from '../sub_static/photo/xianquan.gltf';
...
import * as xqImage5 from '../sub_static/photo/image05.jpg';
import * as xqImage6 from '../sub_static/photo/image06.png';
...
export class Coil3DModel extends ThreeBase {
    ...
    static preload() {
        const modelArray = [xqBin, xqGltf, xqImage1, xqImage2, xqImage3, xqImage4, xqImage5, xqImage6];
    ...
    async initGltfLoader()  {
        ...
        jt.scene.traverse((child: any) => {
            if (child instanceof Scene) {
                // 设置磁铁上半部分透明
                (child.children[0].children[0] as any).material.transparent = true;
                (child.children[0].children[0] as any).material.opacity = 0.8;
                // 设置磁铁下半部分透明
                (child.children[0].children[1] as any).material.transparent = true;
                (child.children[0].children[1] as any).material.opacity = 0.8;
    ...
    addDashLine() {
        ...
        geometryLine.vertices = [new THREE.Vector3(0, 50, 0), new THREE.Vector3(0, -130, 0)];
        ...
        this.scene.add(dottedLine);
    }

    // 添加字母
    addText() {
        const otext =  Coil3DModel.createText('O', -15, 60, 0, '#000000');
        this.scene.add(otext);

        const o2text =  Coil3DModel.createText('O\'', 15, -100, 0, '#000000');
        this.scene.add(o2text);

        const numbertext =  Coil3DModel.createText('30°', 64, 15, 0, '#000000');
        this.scene.add(numbertext);

        const btext =  Coil3DModel.createText('B', 110, -40, 0, '#000000');
