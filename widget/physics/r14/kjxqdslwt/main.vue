...
import {PelletViewHandler} from './services/PelletViewHandler';
...
export default Vue.extend({
    ...
    created() {
        const config = {
            ...
            stepArray : [
                { coverImage: analytic1Img , call: () => {
                        ...
                        (ViewController.getInstance().viewHandler as PelletViewHandler).gltf.hideObj();
                        // (ViewController.getInstance().viewHandler as PelletViewHandler).gltf.showObj1();
                        if (this.resetCamera) {
                            (ViewController.getInstance().viewHandler as PelletViewHandler).gltf.resetCamera();
                ...
                { coverImage: analytic3Img , call: () => {
                        ...
                        (ViewController.getInstance().viewHandler as PelletViewHandler).gltf.hideObj();
                        (ViewController.getInstance().viewHandler as PelletViewHandler).gltf.showObj2();
                        this.showImageOne = false;
        ...
        this.exerciseOption.resizeCall = () => { (ViewController.getInstance().viewHandler as PelletViewHandler).moveDiv() };

        (window as any).noMobile = true;
        ViewController.getInstance(new PelletViewHandler(this));
    ...
    mounted() {
        ...
        (ViewController.getInstance().viewHandler as PelletViewHandler).moveDiv();
    ...
    methods: {
        resetEvent() {
            (ViewController.getInstance().viewHandler as PelletViewHandler).reset();
        },
    }
});
</script>
