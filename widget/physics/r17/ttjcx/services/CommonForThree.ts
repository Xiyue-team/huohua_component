...
export default class CommonForThree {
    ...
    static drawUnitLine({
        ...
        if (!isDash) {
            const geometry = new THREE.CylinderGeometry(0.2,0.2,1,10,1,true)
    ...
    static createText(text: string, position: Array<number> = [0, 0, 0], { color = '#000', isItalic = true,size=40 } = {}) {
        const str = isItalic ? 'italic' : '';
        const textStyle = { font: `${str} ${size}px "Times New Roman"`, fillStyle: color, antialias: true };
