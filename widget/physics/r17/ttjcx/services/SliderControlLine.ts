...
export class SliderControlLine {
     ...
     */
    constructor(controlPoint: any, noDrag ?: boolean) {
    ...
    private initDragEvent(controlPoint: any, camera: Camera, renderer: Renderer) {
        ...
        this.dragControls.addEventListener('dragstart', (obj: any) => {
            Line3dModel.downHandle(obj.object.name);
        });
        this.dragControls.addEventListener('drag', (obj: any) => {
            const mousePosArr = {} as any;
            if (obj.object.name === '') {
                if (obj.object.position.x >= 50) {
                    obj.object.position.x = 50;
                } else if (obj.object.position.x <= 10) {
                    obj.object.position.x = 10
                }
                // obj.object.material = new THREE.MeshBasicMaterial({transparent: true,
                //     opacity:0})
            } else {
                if (obj.object.position.x <= -20) {
                    obj.object.position.x = -20;
                } else if (obj.object.position.x >= 20) {
                    obj.object.position.x = 20;
                }
            }
            const object = obj.object;
            obj.object.position.y = 0;
            obj.object.position.z = 0;
            mousePosArr.x = object.position.x;
            // mousePosArr.y = object.position.y;
            Line3dModel.moveHandle(mousePosArr, object);


        })
        this.dragControls.addEventListener('dragend', (obj: any) => {
            // this.controlPointDragEnd(controls);
            Line3dModel.upHandle();
        });
    }
