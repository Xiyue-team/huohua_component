...
const bgImg = require('../sub_static/background.png');
export class Dlxcyl2dModel {
    ...
    private showFreeElectronStatus = false; // 是否显示自由电子
    ...
    constructor(container: string) {
        ...
        // 加载背景图片
        fabric.Image.fromURL(bgImg, (image: fabric.Image) => {
    ...
    private createArrowGroup() {
        fabric.Image.fromURL(arrowImg, (image: fabric.Image) => {
            const upText = new fabric.Text(window.env.browserInfo.lang.current, {
                left: 220,
                top: 11,
                fontSize: 24,
            ...
            const upGroup = new fabric.Group([image, upText], {
                top: this.top - 54,
            ...
            this.canvas.add(upGroup);
        });

        fabric.Image.fromURL(arrow2Img, (image: fabric.Image) => {
            const downText = new fabric.Text(window.env.browserInfo.lang.freeElectron, {
                top: 11,
                fontSize: 24,
                fontFamily: 'SourceHanSansSC-Medium',
                fill: 'white'
            ...
            const downGroup = new fabric.Group([downText, image], {
                top: this.top + 293 + 8,
    ...
    freeElectronEvent() {
        let i = 0;
        let flag = false;
        if (this.showFreeElectronStatus) { // 已经在展示直接跳出
            return;
        }
        const interval = setInterval(() => {
            ...
            if (i === 10) {
                ...
                this.showFreeElectronStatus = !this.showFreeElectronStatus;
            }
            flag = !flag;
        }, 200, this);
        this.showFreeElectronStatus = !this.showFreeElectronStatus;
     ...
     * @param fe 
     */
    moveFreeElectron(fe: fabric.Object) {
