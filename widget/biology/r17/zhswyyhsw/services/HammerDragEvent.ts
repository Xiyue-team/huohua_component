...
//拖拽事件
    onPanend(vm: any) {
        ...
        if (x < box && y < topBoxBottom &&
            ~['image0', 'image1', 'image5', 'image6', 'image8'].indexOf(this.el.id)) {
            if (ACQUIRE_PRICE[0] === '') {
        ...
        if (x < box && y < topBoxBottom &&
            ~['image2', 'image3', 'image4', 'image7', 'image9'].indexOf(this.el.id)) {
    ...
    changeBackground(vm: any, index: any, num: number) {
        ...
        if (W > 1000) {
            ...
            $(`#${this.el.id}`).parent().hide();
        }

        // $(`#${this.el.id}`).next().find('.textText').remove();
        // $(`#${this.el.id}`).parent().remove();
    }
