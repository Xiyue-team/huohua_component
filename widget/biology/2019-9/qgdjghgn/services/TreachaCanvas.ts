...
export default class TreachaCanvas {
  ...
  async initLine() {
    ...
    this.line[0] = await FabricUtil.loadImage(line_step_oneImg as any, this.config.lineImgConfig);
  ...
  //初始化文字标注
  initAnnotationTexts() {
    this.step_one_annotation_text[0] = new fabric.Text(window.env.browserInfo.lang.tracheaTitle,
      (this.config.annotation_TextConfig[0] as any));
    this.step_one_annotation_text[1] = new fabric.Text(window.env.browserInfo.lang.esophagusTitle,
      (this.config.annotation_TextConfig[1] as any));
    this.step_one_annotation_text[2] = new fabric.Text(window.env.browserInfo.lang.throatTitle,
      (this.config.annotation_TextConfig[2] as any));

    this.step_two_annotation_text[0] = new fabric.Text(window.env.browserInfo.lang.esophagusTitle,
      (this.config.annotation_TextConfig[3] as any));
    this.step_two_annotation_text[1] = new fabric.Text(window.env.browserInfo.lang.tracheaTitle,
      (this.config.annotation_TextConfig[4] as any));

    this.step_three_annotation_text[0] = new fabric.Text(window.env.browserInfo.lang.mucosaTitle,
      (this.config.annotation_TextConfig[5] as any));
    this.step_three_annotation_text[1] = new fabric.Text(window.env.browserInfo.lang.submucosaTitle,
      (this.config.annotation_TextConfig[6] as any));
    this.step_three_annotation_text[2] = new fabric.Text(window.env.browserInfo.lang.cartilageTitle,
      (this.config.annotation_TextConfig[7] as any));
    this.step_three_annotation_text[3] = new fabric.Text(window.env.browserInfo.lang.adventitiaTitle,
      (this.config.annotation_TextConfig[8] as any));

    this.myCanvas.add(this.step_one_annotation_text[0]);
    this.myCanvas.add(this.step_one_annotation_text[1]);
    this.myCanvas.add(this.step_one_annotation_text[2]);
    ...
    this.myCanvas.add(this.step_three_annotation_text[3]);
  }
  ...
  showOrHideAnnotationTexts(index: number) {
    switch (index) {
      case 1:
        for (let i = 0; i < this.step_one_annotation_text.length; i++) {
          this.step_one_annotation_text[i].set('visible', false);
        }
    ...
    this.zoomOutAnimation = TweenMax.to(tween,  1, {
      ...
      onComplete: () => {
        if (index > 2) {
          ...
          (window as any).viewHandler.viewModel.$data.videoActive_start = true;
  ...
  resetImageConfig() {
    ...
    for (let i = 0; i < this.step_one_annotation_text.length; i++) {
      this.step_one_annotation_text[i].set('visible', true);
    }
    for (let i = 0; i < this.step_two_annotation_text.length; i++) {
      this.step_two_annotation_text[i].set('visible', false);
    }
