...
export class Utils {
    ...
    static createText2(texts: string, x: number, y: number, z: number, color: string) {
        const textStyle = {font: '40px "SourceHanSansSC-Medium"', fillStyle: color, antialias: true};
        const text = new SpriteText2D(texts, textStyle);
    ...
    //创建数字的方法
    static createNumber(texts: string, x: number, y: number, z: number, color: string, scale: number) {
