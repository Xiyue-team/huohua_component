...
import { Scene, WebGLRenderer, Vector2, Vector3} from 'three';
...
export class RongqiCanvas extends ThreeBase {
  private rongqi = commit.createImg([-230, -25, -3], 210, 225, bg);
  private deng = commit.createImg([-64.6, -15, 2], 90, 241.2, wuguang);
  private guangxian = commit.createImg([-248, -31.5, 1], 328, 342.5, guangxian);
  private text = commit.createText('浓度为1%mol/L', [-220, -140, -3], {color: ""});
  private text1 = commit.createText1('CO2', [-265, -147, -3], {color: '#FFF'});
  private leaf: any = [];
  ...
  initEve() {
    this.initScene1(ye);
    this.scene.add(this.rongqi, this.deng, this.text, this.text1);
  }
  hafePos() {
    ...
    $('#button1').css({
      left: vector_x * halfWidth * 5.3 + halfWidth,
      top: vector_y * halfHeight * 12 + halfHeight 
    });
    $('#button2').css({
      left: vector_x * halfWidth * 4.3 + halfWidth,
      top: vector_y * halfHeight * 12 + halfHeight 
    });
    $('#button3').css({
      left: vector_x * halfWidth * 3.3 + halfWidth,
      top: vector_y * halfHeight * 12 + halfHeight 
    });
    $('.threeCanvas').css({
      left: vector_x * halfWidth * 4 + halfWidth,
      top: -vector_y * halfHeight * 12 + halfHeight 
    });
  }
  addLight() {
...
//产生叶片
  getLeaf(obj: any) {
    for (let i = 0; i < 10; i++) {
      ...
      this.leaf[i] = new Leaf([-310 + 19 * i, -128, 5], obj1, i, this.scene);
...
//控制叶片上浮个数
  contrialLeaf(numb: number) {
    for (let i = 0; i < 10; i++) {
      if (numb === 1) {
        if (~[0, 1, 2, 4, 5, 7].indexOf(i)) {
          this.leaf[i].leafAnimation(i);
        }
      } else if (numb === 2) {
        this.leaf[i].leafAnimation(i);
      } else if (numb === 3) {
        if (~[0, 1, 2, 4].indexOf(i)) {
          this.leaf[i].leafAnimation(i);
        }
      }
    }
