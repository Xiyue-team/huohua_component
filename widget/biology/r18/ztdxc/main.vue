    ...
    <div class="aspectration bg_white root_div_container" >
        <fullScreensLayout>
            <template slot="viewBox" slot-scope="viewBox">
                <span class="title_style">蔗糖的形成</span>
                <div id="3dContainer" style="width: 100%;height: 100%;display: flex;flex-direction: column;justify-content: space-between;align-items: center;">
                    ...
                    <!--方程式-->
                    <div class="imgContain" v-bind:class="{'imgContainMobile': isMobile, 'imgContainIpad': isIpad}">
                            <img src="./sub_static/zuoce.png" v-bind:style="img2style" class="img1Style" v-bind:class="{'img1StyleMobile': isMobile, 'img1StyleIpad': isIpad}"/>

                            <img src="./sub_static/youce.png" v-bind:style="img2style" class="img2Style" v-bind:class="{'img2StyleMobile': isMobile, 'img2StyleIpad': isIpad}"/>

                            <img src="./sub_static/zongfanyingshi.png" v-bind:style="imgstyle" class="img3Style" v-bind:class="{'img3StyleMobile': isMobile}"/>
                    </div>
                    <!--滑块-->
                    <div class="slider-content control-block_div_border sliderContentStyle"  v-bind:class="{'sliderContentStyleMobile': isMobile}" >
                        <vue-slider ref='sliderC' v-model='sliderNum' v-bind='sliderOption' class="sliderStyle" v-bind:class="{'sliderStyleMobile': isMobile}" @mousedown.native="pause()" @touchstart.native="pauses()">
                        </vue-slider>
                        <!--开始暂停按钮-->
                        <div class="play_btn playStyle" @click="isPlay = !isPlay" v-bind:class="{'playStyleMobile': isMobile}" >
                            <img v-show="!isPlay"  src="./sub_static/play_icon.png" alt="">
                            <img v-show="isPlay"   src="./sub_static/pause_icon.png" alt="">
                        </div>
                    </div>
                </div>
            </template>
...
<script lang="ts">
  ...
  import {ZtdxcViewHandler} from './services/ZtdxcViewHandler';
  ...
  export default Vue.extend({
    ...
    created() {
      ...
      ViewController.getInstance(new ZtdxcViewHandler(this), viewOption);
      ViewController.getInstance(new ZtdxcViewHandler(this));
    ...
    watch: {
      ...
      sliderNum: function(sliderNum: number) {
        ...
        (ViewController.getInstance().viewHandler as ZtdxcViewHandler).ztdxc.modelanimation.action.time = sliderNum / 44;

        if (this.isPlay) {
          (ViewController.getInstance().viewHandler as ZtdxcViewHandler).PlayAnimation(true);
          ...
          (ViewController.getInstance().viewHandler as ZtdxcViewHandler).PlayAnimation(false);
          this.imgstyle.opacity = sliderNum / 100;
          if (sliderNum === 0) {
            this.img2style.opacity = 1;
          } else {
