...
import * as l1 from '../sub_static/model/putaotang_L.fbx';
import * as l2 from '../sub_static/model/putaotang_R.fbx';
import * as l3 from '../sub_static/model/animation.fbx';
import * as l4 from '../sub_static/model/mayatang.fbx';
import * as l5 from '../sub_static/model/shui.fbx';
import * as l6 from '../sub_static/model/RUTANG.png';

export class Mytdxc3dModel extends ThreeBase {
    ...
    private geometry: any = new THREE.SphereGeometry( 100, 100, 100 );
    ...
    initCamera(): void {
      ...
      // alert(screen.width === 854);
      // alert(screen.height === 534);
      ...
      if (BrowserUtil.getBrowserInfo().isSmallDevice) {
        ...
        left = this.width / -3.6;
        right = this.width / 3.6;
        top = this.height / 3.6;
        bottom = this.height / -3.6;
      }
    ...
    initLight(): void {
      this.lights = [];
      this.scene.add( new THREE.AmbientLight( 0x666666, 1.4 ) );
      this.lights[0] = new THREE.DirectionalLight( 0xdfebff, 1.4 );
      this.lights[0].position.set( 200, 0, 250 );
      this.lights[0].position.multiplyScalar( 1.3 );

      this.lights[1] = new THREE.DirectionalLight( 0xdfebff, 1.4);
      this.lights[1].position.set( -200, 0, -250 );
    ...
    async initmodel() {
      ...
      this.group.position.set(-112, 6, 0);
      ...
      this.group2.position.set(111.3, 0, 10);
      ...
      this.group3.position.set(-95.5, 11.5, 0);
      ...
      const cube4 = new THREE.Mesh( geometry, this.material );
      this.group4.add(cube4);
      this.scene.add(this.group4);
      this.group4.position.set(125.3, 2, 0);
      this.group4.visible = false;
      this.modelRotate2 = new ModelRotateHelper(this.group4, null, null, this.group3);
      this.modelRotate2.initEvent();

      let length = screen.height / 10;

      if ((window as any)['env'].browserInfo.isIpad) {
        length = screen.height / 20;
      }


      this.group.position.y += length;
      this.group2.position.y += length;
      this.group3.position.y += length;
      this.group4.position.y += length;
      this.model3.position.y += length;
