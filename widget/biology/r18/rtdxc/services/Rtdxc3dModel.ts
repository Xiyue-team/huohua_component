...
export class Rtdxc3dModel extends ThreeBase {
    ...
    initCamera(): void {
      ...
      if (screen.width === 854) {
        const scale = 1.8;
        left = this.width / -scale;
        right = this.width / scale;
        top = this.height / scale;
        bottom = this.height / -scale;
      }
    ...
    async initmodel() {
      ...
      //模型一和触碰旋转区域
      this.ballOne = await this.fbxLoader(l1 as any);
      ...
      this.group1.add(this.ballOne);
      this.group1.add(cube);
      this.modelRotateOne = new ModelRotateHelper(this.group1, null, null, this.group2);
      this.modelRotateOne.initEvent();
      this.group1.visible = true;
      this.group1.position.set(-113, -17.5, 0);
      this.scene.add(this.group1);

      //模型二和触碰旋转区域
      this.ballTwo = await this.fbxLoader(l2 as any);
      ...
      this.group2.add(this.ballTwo);
      this.group2.add(cube2);
      this.modelRotateTwo = new ModelRotateHelper(this.group2, null, null, this.group1);
      this.modelRotateTwo.initEvent();
      this.group2.visible = true;
      this.group2.position.set(117, -6.8, 0);
      this.scene.add(this.group2);

      //模型三动画
      this.ballThree = await this.fbxLoader(l3 as any);
      this.scene.add(this.ballThree);
      this.modelanimation = new Model3dAnimation(this.ballThree);
      this.ballThree.visible = false;


      //模型四和触碰旋转区域
      this.ballFour = await this.fbxLoader(l4 as any);
      ...
      this.group4.add(this.ballFour);
      this.group4.add(cube4);
      this.modelRotateFour = new ModelRotateHelper(this.group4, null, null, this.group5);
      this.modelRotateFour.initEvent();
      this.group4.visible = false;
      this.group4.position.set(-64, -12.5, 0);
      this.scene.add(this.group4);

      //模型五和触碰旋转区域
      this.ballFive = await this.fbxLoader(l5 as any);
      ...
      this.group5.add(this.ballFive);
      this.group5.add(cube5);
      this.modelRotateFive = new ModelRotateHelper(this.group5, null, null, this.ballFour);
      this.modelRotateFive.initEvent();
