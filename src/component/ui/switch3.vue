<template>
  <div v-bind:id="id"  class="button-switch" v-bind:class="{'vertical':vertical}">
    ...
    <div class="switch-sign" v-bind:style="{width: width,left:left,}" v-bind:class="{ 'switch-padding': ready}"
      style="top: 0;bottom: 0;margin: auto;"></div>
...
export default Vue.extend({
  ...
  mounted: function()  {
      console.log('this.datas',this.maxW)
      const maxWidth  = this.getMaxWidthItem();
      this.ready = true;
      this.width = maxWidth + 'px';
      this.images.indexOf(this.value);
      setTimeout( () => {
  ...
  watch : {
      activeIndex(newVal, oldVal) {
          ...
          console.log('移动了', this.left, this.itemPosition);
      },
      value(newVal) {
