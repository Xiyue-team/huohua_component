  ...
  >
    <div ref="elem" aria-hidden="true" class="vue-slider" :style="[elemStyles, bgStyle]">
      <template v-if="isRange">
        ...
        >
          <!-- 默认当前点 -->
          <div class="vue-slider-dotPoint"> </div>
  ...
  export default {
    ...
    methods: {
      ...
      wrapClick (e) {
        ...
        if (this.isDisabled || !this.clickable || this.processFlag ||  (this.backDisabled ) ) return false
      ...
      moving (e) {
        ...
        console.log(processSign1.pos[0] + this.getPos(e.targetTouches[0]) - processSign1.start)*/


        if( this.backDisabled && (e.movementX < 0  || this.getPos((e.targetTouches && e.targetTouches[0]) ? e.targetTouches[0] : e) < this.currentPosition )){
            return;
            //this.flag =  false
        }
        //保存当前位置
        this.currentPosition = this.getPos((e.targetTouches && e.targetTouches[0]) ? e.targetTouches[0] : e);
...
<style>
  ...
  /*重新设置滑动点大小 */
  .vue-slider-component .vue-slider-dotPoint{
      width: 24px;
      height: 24px;
      border-radius: 50%;
