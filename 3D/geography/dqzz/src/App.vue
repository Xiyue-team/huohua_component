<template>
  ...
  >
    <div class="container">
      ...
      <!--视图区-->
      <div
        v-show=switch_checked
        style="width:100%;height:100%;background-color:rgba(255,255,255,0);position:absolute;z-index:4"
      ></div>
        ...
        methods: {
            ...
            createScene(engine) {
                ...
                var earthFragmentShader = "precision highp float;\r\n" +
                    ...
                    "float ndl = max(1., dot(vNormalW, lightVectorW)) * intsmooth;\r\n" +
                    "float ndlt = max(1., dot(vNormalW, lightVectorW)) * intsmootht;\r\n" +
                ...
                var customFragmentShader = "precision highp float;\r\n" +
                    ...
                    "float ndl = max(1., dot(vNormalW, lightVectorW)) * intsmooth;\r\n" +
                    "vec3 angleW = normalize(viewDirectionW + lightVectorW);\r\n" +
                    "float specComp = max(1., dot(vNormalW, angleW));\r\n" +
                ...
                function createWefttext(mesh, name) {
                    var plane = BABYLON.Mesh.CreatePlane("plane1", 8);
                ...
                function createDashedCircle(r, value, y, color, scene, axis) {
                    var vertices = setvertices(r, value, y, axis);
                    var Dashedcircle = BABYLON.Mesh.CreateDashedLines("lines", vertices, 100, 100, vertices.length, scene, true, Dashedcircle);
                ...
                function CreateWeft(r, percentPI, color) {
                    for (i = 0; i < 90; i += percentPI) {
                        if (i == 0) {
                            CreateWeftLine(i, r, 360, new BABYLON.Color3(0.8, 0.8, 0));
                        } else {
                            CreateWeftLine(i, r, 360, color);
                            CreateWeftLine(i, r, 360, color, true);
                        }
                    }
                    CreateWeftLabel(r, percentPI)
                }
