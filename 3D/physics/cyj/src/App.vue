  ...
  methods: {
    ...
    bm(){
      ...
      this.mark1 = !this.mark1
       ...
       this.mm= this.mark1 ? '取消标记' : '显示标记'
     
      this.object.children[4].visible = this.mark1
            this.object.children[3].visible = this.mark1
    ...
    //初始化
    init() {
      var ac = new THREE.TextureLoader().load("./static/jq/aluminum_C.jpg");
       var am = new THREE.TextureLoader().load("./static/jq/aluminum_M.jpg");
        var ar = new THREE.TextureLoader().load("./static/jq/aluminum_R.jpg");

         var bc = new THREE.TextureLoader().load("./static/jq/black_C.jpg");
          var bm = new THREE.TextureLoader().load("./static/jq/black_M.jpg");
           var br = new THREE.TextureLoader().load("./static/jq/black_R.jpg");

            var pc = new THREE.TextureLoader().load("./static/jq/penyouguan_C.jpg");
             var pm = new THREE.TextureLoader().load("./static/jq/penyouguan_M.jpg");
              var pr = new THREE.TextureLoader().load("./static/jq/penyouguan_R.jpg");

               var rc = new THREE.TextureLoader().load("./static/jq/red_C.jpg");
                var rm = new THREE.TextureLoader().load("./static/jq/red_M.jpg");
                 var rr = new THREE.TextureLoader().load("./static/jq/red_R.jpg");

                 var ev = new THREE.TextureLoader().load("./static/jq/env.png");
...
node.lookAt(this.camera.position)
            ...
            if (node.isMesh) {
              ...
              child.material.side = 2
                // child.material
              if(child.name == 'aluminum_m'){
                
              child.material.envMap = ev
              child.material.metalnessMap = am
               child.material.map = ac
                child.material.roughnessMap = ar
             
              }
               if(child.name == 'black_m'){
                 console.log(child)
             child.material.envMap = ev
              child.material.metalnessMap = bm
               child.material.map = bc
                // child.material.roughnessMap = br
              }
              if(child.name == 'penyouguan_m'){
                 console.log(child)
            child.material.envMap = ev
             child.material.metalnessMap = pm
               child.material.map = pc
                child.material.roughnessMap = pr
              }
              if(child.name == 'red_m'){
                 console.log(child)
           child.material.envMap = ev
            child.material.metalnessMap = rm
              //  child.material.map = rc
                // child.material.roughnessMap = rr
              }
  ...
  this.nv = false
      ...
      initScene('./static/jq/chaiyouji.gltf');
      var onWindowResize = () => {
