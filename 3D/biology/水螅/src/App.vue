<template>
  ...
  >
    ...
    >两侧对称：经过身体的纵轴，只有一个切面将身体分为对称的两部分。</p>
    <div
      id="container"
      ref="renderCanvas"
    ></div>
...
export default {
  ...
  data() {
    return {
      ...
      show:false,
      object:null,
      img2:new THREE.TextureLoader().load("./static/qyj/ci_normals.bmp")
    ...
    this.camera.add(light2);
        ...
        loader.load(url, (data) => {
          ...
          this.object.traverse((node) => {
           if(node.name == 'benti'){
 node.traverse((node) => {
    if (node.isMesh && node.name!="transform1") {
            node.material.normalMap = this.img2
    }
 })
           }
            if(node.name == "zi"){
...
node.lookAt(this.camera.position)
         ...
         node.material.opacity = 0.8
             ...
             console.log(node.name.indexOf('pCube'))
              if(node.name.indexOf('pCube')>=0){
                console.log(node)
          ...
          let b  = this.object.children[4];
          let c =  this.object.children[5];
          let d =  this.object.children[6];
          this.a = new THREE.Group()

         this.a.add(a,b,c,d)
          this.a.scale.set(0.5,0.4,0.5);
          this.a.position.y = this.a.position.y +2
          this.object.position.y =  this.object.position.y -3
          this.a.visible = false
          scene.add(this.object, this.a)
         console.log(this.object, this.a)
      ...
      initScene('./static/qyj/shuixi.gltf');
      var onWindowResize = () => {
