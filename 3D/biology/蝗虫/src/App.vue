<template>
  ...
  >
    ...
    <div class="btnG"> <button
        @click="bm(1)"
        :class="{ active: mark1 }"
      >&nbsp &nbsp头&nbsp &nbsp </button>
      <button
        @click="bm(2)"
        :class="{ active: mark2 }"
      > &nbsp &nbsp胸&nbsp &nbsp </button>

      <button
        @click="bm(3)"
        :class="{ active: mark3 }"
      > &nbsp &nbsp腹 &nbsp &nbsp</button>
    </div>
    <div
      v-show="loadModelF"
      id="changeSence"
...
export default {
  ...
  data() {
    return {
      ...
      show:false,
      object:null,
      img1:new THREE.TextureLoader().load("./static/qyj/HC_toubu.png"),
      img3:new THREE.TextureLoader().load("./static/qyj/HC_fu_g.png"),
      img2:new THREE.TextureLoader().load("./static/qyj/HC_xiong_g.png"),
      img4:new THREE.TextureLoader().load("./static/qyj/HC.png")
 ...
 this.object.children[0].visible = this.mark1
        this.object.children[1].visible = this.mark2
        this.object.children[2].visible = this.mark3
              this.object.children[4].material.map = this.img4;
    },
    loopInit() {
    ...
    if(i ==1){
      if(this.mark1){
        ...
        this.mark3 = !this.mark1
        this.mm= this.mark1 ? '隐藏标记' : '显示标记'
     this.object.children[5].material.map = this.img1;
        this.object.children[0].visible = this.mark1
        this.object.children[1].visible = this.mark2
        this.object.children[2].visible = this.mark3
    }else if(i ==2) {
      if(this.mark2){
        ...
        this.mm= this.mark1 ? '隐藏标记' : '显示标记'
     this.object.children[4].material.map = this.img2;
        this.object.children[0].visible = this.mark1
        this.object.children[1].visible = this.mark2
        this.object.children[2].visible = this.mark3
    }else{
      if(this.mark3){
        ...
        this.mark1 = !this.mark3
       
        this.mm= this.mark1 ? '隐藏标记' : '显示标记'
       this.object.children[3].material.map = this.img3;
      //  this.object.children[0]. children[0] .material.map.offset.x = 0.1
      //  this.object.children[0]. children[0] .material.map.offset.y = -0.1
         
         console.log( this.object.children[0])
    ...
    this.camera.add(light2);
        ...
        loader.load(url, (data) => {
          ...
          //  this.object.children[0].visible = this.mark1
           this.object.scale.set(1.7, 1.7,1.7);
            this.object.children[0].visible = this.mark1
             this.object.children[1].visible = this.mark2
             this.object.children[2].visible = this.mark3
             this.object.position.set(0,-6,0)
           this.object.children[1].traverse((node) => {
 ...
 if (node.isMesh) {
   ...
   if(node.name.indexOf('pCylinder')<0){
         console.log(node)
...
if(node.name.indexOf('pCylinder')<0){
      node.lookAt(this.camera.position)
           ...
           this.object.children[2].traverse((node) => {
...
if(node.name.indexOf('pCylinder')<0){
      node.lookAt(this.camera.position)
            ...
            this.object.children[0].traverse((node) => {
...
if(node.name.indexOf('pCylinder')<0){
      ...
      initScene('./static/qyj/huangchong.fbx');
      var onWindowResize = () => {
